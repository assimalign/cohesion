...
public sealed class ResourceManifestSdkIntegrationTests
{
    ...
    public async Task Build_EnabledWebReferencesEnabledDatabase_GeneratesValidManifestsAndTypedAccessors()
    {
        ...
        webControlPlaneSource.ShouldContain(
            "global::Assimalign.Cohesion.ApplicationModel.WebResourceControlPlane.Create()");
        ...
        databaseControlPlaneSource.ShouldContain(
            "global::Assimalign.Cohesion.ApplicationModel.DatabaseResourceControlPlane.Create()");
    ...
    /// <summary>
    /// Verifies the area SDK resolves the sibling ApplicationModel SDK at its own version
    /// without requiring another global.json pin.
    /// </summary>
    /// <returns>A task representing the package-boundary build.</returns>
    [Fact(DisplayName = "Cohesion Test [Sdk.ApplicationModel] - enabled area resource builds with only base and area SDK pins")]
    public async Task Build_EnabledAreaWithBaseAndAreaPinsOnly_ShouldResolveApplicationModelSdk()
    {
        // Arrange
        using var timeout = new CancellationTokenSource(TimeSpan.FromMinutes(3));
        using ConsumerWorkspace workspace = ConsumerWorkspace.Create("MinimalResource");
        Dictionary<string, string> availablePins = ConsumerWorkspace.CreateMatchingSdkPins();
        var pins = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ["Assimalign.Cohesion.Sdk"] = availablePins["Assimalign.Cohesion.Sdk"],
            ["Assimalign.Cohesion.Sdk.Web"] = availablePins["Assimalign.Cohesion.Sdk.Web"]
        };
        ...
        // Assert
        result.ExitCode.ShouldBe(0, result.Output);
        string globalJson = File.ReadAllText(Path.Combine(workspace.RootDirectory, "global.json"));
        globalJson.ShouldNotContain("Assimalign.Cohesion.Sdk.ApplicationModel", Case.Sensitive);
    }
    ...
    private static string GeneratedOutput(string projectDirectory, string fileName)
    {
        string path = Path.Combine(
            ...
            "cohesion",
            fileName);
        string[] candidates = Directory.Exists(Path.Combine(projectDirectory, "obj"))
            ? Directory.EnumerateFiles(
        ...
        File.Exists(path).ShouldBeTrue(
            $"Expected generated output '{path}'. Candidates: '{string.Join("', '", candidates)}'.");
        return path;
    }
