...
| 2026-07-12 | #166 | (fable5 service-surface PR) | Stage 4, Lane H — **the database host composition**. `Database.Hosting` build-out on the `libraries/Hosting` execution menu: `DatabaseApplication : Host<DatabaseApplicationContext>` composes `WriteAheadFlushService` + `PageWriterService` (`DedicatedThreadService`) then the composition root's endpoint (and other) `IHostService`s, so endpoints start last and **drain first** on stop. **COHRES002 discovery + resolution:** the hosting module may reference no same-area library but the root, so it cannot name `IDatabaseServer` to run the accept loop — the scaffold's in-Hosting `QueryEndpointService` would fail the build. Resolved by homing the endpoint adapter **with the server**: new `Database.Server` `DatabaseServer.CreateHostService(IDatabaseServer)` → internal `DatabaseServerHostService : BackgroundService` (starts the server, parks, drains on the server's own budget); `Database.Server` gains a non-area `Assimalign.Cohesion.Hosting` ref (allowed); `Database.Hosting` composes the returned `IHostService` generically and still refs only root + non-area Hosting. **Engine-worker descope (R10):** `WriteAheadFlushService`/`PageWriterService` are documented no-op placeholders that park until shutdown — the SQL engine flushes synchronously at commit and owns its durability (embedded consumers must get it without a host), so there is no host-driven flush/page-writer work yet; the engine-owned background-worker seam + an `IDatabaseEngine` start/stop lifecycle seam are **filed as #902** under the engine self-sufficiency feature #862 (advances #862, not closed here). Env-var conventions bound by Hosting: new `DatabaseHostConfiguration.FromEnvironment()` reads `COHESION_DATABASE_DATA_PATH`/`_ENDPOINT_PORT`/`_DURABILITY` (the #853 manifest sets the same names). 6 host tests (served round-trip through the composed endpoint over `Connections.InMemory`, default+toggled composition, drain-to-Stopped, env-var bind + malformed-port reject); Server 16 re-verified. Hosting + Server DESIGN/OVERVIEW updated with the placement decision; new tests project wired into 3 slnx files. |
