...
| --- | --- | --- |
| `SecretsEndpointService` | `BackgroundService` (pool-scheduled) | async loop to serve secret material and rotation requests |

The secret store is asynchronous I/O end to end: its loops spend their lives awaiting sockets and queues, so pooled `BackgroundService` is the whole composition - a dedicated thread would sit idle between requests.
...
- No builder or DI surface yet; construct `SecretStoreApplication` with `SecretStoreApplicationOptions` directly. A `CreateBuilder` surface can follow the `WebApplication` pattern when the resource matures.
