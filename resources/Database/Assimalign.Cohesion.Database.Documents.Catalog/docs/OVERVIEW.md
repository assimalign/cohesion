...
commit the caller's transaction. The engine takes collection/document locks, rejects write
conflicts and stale index definitions, and enforces schema ownership before calling the catalog.
...
rebuild indexes. `SearchIndexAsync` performs a B+Tree equality or range seek and resolves only
