...
namespace Assimalign.Cohesion.Database.Testing.Internal;
...
internal sealed class TestBootstrapIdentity : IDisposable
{
    ...
    internal TestBootstrapIdentity(string issuer, string subject)
    {
        Issuer = issuer;
        Subject = subject;
    ...
    internal string Issue(string audience, TimeSpan? lifetime = null, DateTimeOffset? issuedAt = null, bool includeId = true)
    {
        DateTimeOffset now = issuedAt ?? DateTimeOffset.UtcNow;
        var descriptor = new JsonWebTokenDescriptor
        {
            Id = includeId ? Guid.NewGuid().ToString("N") : null,
            Issuer = Issuer,
    ...
    private static byte[] CreatePublicKey(ECDsa key, out string keyId)
    {
        ...
        var jwk = new ArrayBufferWriter<byte>();
        using (var writer = new Utf8JsonWriter(jwk))
        {
            writer.WriteStartObject();
            writer.WriteString("kty", "EC");
            writer.WriteString("crv", "P-256");
        ...
        return jwk.WrittenSpan.ToArray();
    }
}
