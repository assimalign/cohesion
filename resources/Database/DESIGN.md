...
| R5 | **Orchestration via the ApplicationModel.** `Database.ApplicationModel` supplies a manifest-backed `DatabaseResource : PlannedResource`, `AddDatabase(manifest, options)`, the platform-neutral Database planner, and the Database default-control-plane factory. It never references the runtime; the runtime never references it. An enabled customer executable's generated code connects both sides through `ResourceRuntime`. | `Database.ApplicationModel` |
...
- **`Database`** — the public contract root: `IDatabase`, `IDatabaseEngine`, `IDatabaseSession`, `IDatabaseTransaction`, `DatabaseException` (the exception root for the contracts and everything above them), `DatabaseNotFoundException` (the exact missing-database signal from `OpenDatabaseAsync`), `DatabaseName`, lifecycle enums, and the immutable `IDatabaseSchema` declaration model. `DatabaseSchema.Create`/`IDatabaseSchemaBuilder` describe custom types, tables (columns, keys, indexes, references), functions, triggers, and database-scoped principals in C#; Hosting retains the completed model for the later schema compiler/migration work (#857–#859). Model-agnostic; every engine, feature library, and service-surface project references it, and it in turn rolls up the child roots (so `TransactionId`, `ProtocolVersion`, and the rest of the base vocabulary arrive transitively).
...
control plane, the builder honors it together with `ResourceRuntime.Current`; when no registration
exists, it behaves as a plain application. There is no Database-specific environment-variable
bridge. Generated `Resource` accessors and the ambient `ResourceContext` carry endpoints, mounts,
settings, references, environment, and the bootstrap credential.

`builder.Provision(engine, name)` registers `DefaultDatabaseProvisioner` as an additional host
service. `builder.AddDatabase(engine, name, schema)` builds the root's C# `IDatabaseSchema`
(tables, types, functions, triggers, and principals), retains it for schema compile/migrations,
and registers the same provisioning step. `DatabaseApplication` materializes **all additional
...
`DatabaseNotFoundException`; every other open failure propagates and aborts startup.
...
`samples/Assimalign.Cohesion.Database.SampleHost` is the in-repo executable proof with
...
invokes the real entry point inside `ResourceRuntime.CreateScope(...)`, while the real-process
E2E obtains the sample apphost through `ReferenceOutputAssembly=false`.
...
- **Health/diagnostics (#168, delivered by #973):** `DatabaseApplicationContext` implements the
  Core-only `IHealthContributor` contract and aggregates each distinct registered or
  ...
  any other registered `IHealthContributor`s join the same generated control plane. Hosting's
  internal admin service exposes the aggregate over private `Web.Health` routes (`/healthz`,
  `/readyz`, `/livez`) and the control-plane endpoints on `admin`. This realizes the observational
  surface without adding a Database-specific health abstraction or treating the wire protocol's
  `Ping` frame as readiness. The generated registration and ambient `ResourceContext` are scoped
...
| Code-first resource schema had no retained model | schema compile/migrations #857–#859 | #973 adds the root `IDatabaseSchema` vocabulary (types, tables, functions, triggers, principals) and `builder.AddDatabase(engine, name, schema)`; the compiler/migration items consume it later |
