...
- **In-process (embedded):** an engine linked directly into an application process — no server, no wire protocol, same engines, same ACID guarantees (`Database.Embedded` is the facade for this mode; post-MVP).
...
| # | Requirement | Where it lands |
|---|---|---|
| R7 | **Database projects.** A consumer creates a *database project* (`<Project Sdk="Assimalign.Cohesion.Sdk.Database">`) holding declarative schema sources. The build validates and compiles the schema into a deployable artifact. *(Assumption: the original requirement — "I want the project to …" — was cut short; it has been read as the DacFx/SSDT-style declarative database-project experience: schema-as-source, build-time validation, artifact output, migration generation by diffing. Flag if wrong.)* | `Sdk.Database` targets + Tasks |
