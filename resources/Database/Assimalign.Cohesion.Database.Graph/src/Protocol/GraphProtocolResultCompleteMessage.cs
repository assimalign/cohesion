...
public sealed record GraphProtocolResultCompleteMessage(long AffectedCount)
{
    /// <summary>
    /// Encodes the message payload.
    /// </summary>
    /// <returns>The payload bytes for a <see cref="GraphProtocolMessageType.ResultComplete"/> frame.</returns>
    public byte[] Encode()
    {
    ...
    public static GraphProtocolResultCompleteMessage Decode(ReadOnlySpan<byte> payload)
    {
        int position = 0;
        return new GraphProtocolResultCompleteMessage(ProtocolPayload.ReadInt64(payload, ref position));
    }
}
