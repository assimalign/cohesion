using System.Collections.Generic;

namespace Assimalign.Cohesion.Database.Graph.Internal;

/// <summary>
/// The Graph server's <see cref="IDatabaseServerContext"/>: the one engine the
/// server fronts and a live view of its active sessions.
/// </summary>
internal sealed class GraphDatabaseServerContext : IDatabaseServerContext
{
    private readonly GraphDatabaseServer _server;

    internal GraphDatabaseServerContext(GraphDatabaseServer server, IDatabaseEngine engine)
    {
        _server = server;
        Engine = engine;
    }

    /// <inheritdoc />
    public IDatabaseEngine Engine { get; }

    /// <inheritdoc />
    public IReadOnlyCollection<IDatabaseServerSession> Sessions => _server.GetSessionsSnapshot();
}
