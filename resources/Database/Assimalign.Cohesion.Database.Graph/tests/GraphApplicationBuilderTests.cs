...
public sealed class GraphApplicationBuilderTests
{
    ...
    {
        string directory = Path.Combine(Path.GetTempPath(), "cohesion-graph-strategy-" + Guid.NewGuid().ToString("N"));
    ...
    private sealed class RecordingStorageStrategy(string directory) : IGraphStorageStrategy, IDisposable
    {
