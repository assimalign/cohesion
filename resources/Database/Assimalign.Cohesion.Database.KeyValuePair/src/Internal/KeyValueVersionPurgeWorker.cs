...
/// </summary>
/// <remarks>
/// Live from the engine's first cut — the key-value MVCC binding is real, so the
/// purge duty is real (the SQL precedent, followed honestly). Passes iterate the
/// engine's database snapshot and tolerate racing a drop. A pass failure flips
/// the engine's observational state to Faulted without stopping service:
/// unpurged versions cost space, never consistency — visibility always filters
/// them. (Aborted writers are normally unlinked inline at rollback, before their
/// locks release — the worker's abort duty is the retry of a failed undo.)
/// </remarks>
internal sealed class KeyValueVersionPurgeWorker : DatabaseEngineWorker
{
    private readonly KeyValueDatabaseEngine _engine;

    internal KeyValueVersionPurgeWorker(KeyValueDatabaseEngine engine)
    {
        _engine = engine;
    }
