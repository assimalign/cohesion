# KeyValuePair wire family

## Ownership and compatibility

`KeyValueProtocol.Family` is fixed when the endpoint accepts a connection. The model
owns `KeyValueProtocolMessageType`, ProtocolExecuteMessage, ProtocolResultHeaderMessage,
...
```mermaid
sequenceDiagram
    participant Client as "KeyValuePair.Client"
    participant Server as "KeyValuePair endpoint"
...
contain one component per header column; null carries its own type tag. PUT outcome sets preserve affectedCount 1 or 0; GET/SCAN query sets use -1. See [command grammar](COMMANDS.md) for parameter names and result columns.
