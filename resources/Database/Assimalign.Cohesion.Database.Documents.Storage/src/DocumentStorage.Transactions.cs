...
/// <summary>Stores document chunks and catalog records on the shared page, journal, and recovery kernel.</summary>
public sealed partial class DocumentStorage : Assimalign.Cohesion.Database.Storage.Storage
{
    ...
    /// <summary>Opens an existing document file set, taking ownership of the streams.</summary>
    /// <param name="data">The data stream.</param>
    /// <param name="journal">The journal stream.</param>
    /// <param name="backup">The backup stream.</param>
    /// <param name="checkpointOnOpen">Whether to checkpoint immediately.</param>
    /// <returns>The recovered storage.</returns>
    public static DocumentStorage Open(Stream data, Stream journal, Stream backup, bool checkpointOnOpen)
        ...
        Func<DocumentContentReference, ValueTask> complete, Func<ValueTask> abort, CancellationToken cancellationToken = default)
    {
        ...
        return new DocumentWriteStream(this, coordinator, context, complete, abort, cancellationToken);
    }
    ...
    public Stream OpenRead(DocumentContentReference content, Func<ValueTask>? onDispose = null)
        => new DocumentReadStream(this, content, onDispose);
        ...
        DocumentContentReference content, CancellationToken cancellationToken = default)
    {
        ...
        while (location != 0)
        {
            ...
            var chunk = DocumentChunkCodec.Decode(bytes, remaining);
        ...
        if (remaining != 0)
        {
            throw new StorageCorruptionException("Document chain ends before its catalog length.");
        }
    }
