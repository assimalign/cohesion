...
public sealed class DatabaseObjectLockedException : DatabaseException
{
    ...
    /// <param name="schemaName">The compiled schema that owns the object.</param>
    /// <param name="operation">The refused operation in the model's own vocabulary.</param>
    public DatabaseObjectLockedException(string objectName, string schemaName, string operation)
        : base($"Object '{objectName}' is owned by schema '{schemaName}' and cannot be changed by {operation}. Alter the schema and redeploy it.")
    {
    ...
    /// <summary>The compiled schema that owns the object.</summary>
