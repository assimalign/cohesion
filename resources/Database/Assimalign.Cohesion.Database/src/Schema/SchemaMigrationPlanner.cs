...
public static class SchemaMigrationPlanner
{
    ...
    /// <exception cref="DatabaseSchemaMigrationException">The schemas are incompatible or a destructive operation is not allowed.</exception>
    public static SchemaMigrationPlan Plan(CompiledSchema? current, CompiledSchema desired)
    {
        ...
        {
            return new SchemaMigrationPlan(current.Hash, desired.Hash, Array.Empty<SchemaMigrationOperation>());
        }

        var operations = new List<SchemaMigrationOperation>();
        PlanTables(current?.Tables ?? Array.Empty<CompiledSchemaTable>(), desired.Tables, operations, identifiers);
        PlanCollections(current?.Collections ?? Array.Empty<CompiledSchemaCollection>(), desired.Collections, operations, identifiers);
        EnsureSupportedMetadataIsUnchanged(current, desired);

        SchemaMigrationOperation? destructive = operations.FirstOrDefault(
            operation => operation.Safety == SchemaMigrationSafety.Destructive);
        ...
        StringComparer identifiers)
    {
        ...
        foreach (CompiledSchemaTable oldTable in current.OrderBy(table => table.Name, StringComparer.Ordinal))
        {
            ...
            foreach (CompiledSchemaIndex oldIndex in oldTable.Indexes.OrderBy(index => index.Name, StringComparer.Ordinal))
            {
                ...
                {
                    operations.Add(new SchemaMigrationOperation(
        ...
        foreach (CompiledSchemaTable newTable in desired.OrderBy(table => table.Name, StringComparer.Ordinal))
        {
            ...
            foreach (CompiledSchemaColumn column in newTable.Columns)
            {
                if (!oldColumns.TryGetValue(column.Name, out CompiledSchemaColumn? previous))
                {
                    operations.Add(new SchemaMigrationOperation(
                        ...
                        column.IsNullable ? SchemaMigrationSafety.Safe : SchemaMigrationSafety.Destructive,
                        column.Name,
                        newTable.Name,
                        column: column));
                }
            ...
            {
                operations.Add(new SchemaMigrationOperation(
                    SchemaMigrationOperationKind.AlterTable,
                    SchemaMigrationSafety.Destructive,
                    newTable.Name,
                    table: newTable));
            }
            ...
            foreach (CompiledSchemaColumn column in oldTable.Columns.OrderBy(value => value.Name, StringComparer.Ordinal))
            {
                ...
                {
                    operations.Add(new SchemaMigrationOperation(
                        SchemaMigrationOperationKind.DropColumn,
                        SchemaMigrationSafety.Destructive,
                        column.Name,
                        oldTable.Name));
                }
        ...
        foreach (CompiledSchemaTable oldTable in current.OrderByDescending(table => table.Name, StringComparer.Ordinal))
        {
            ...
            {
                operations.Add(new SchemaMigrationOperation(
                    SchemaMigrationOperationKind.DropTable,
                    SchemaMigrationSafety.Destructive,
                    oldTable.Name));
            }
        }
    ...
    private static void PlanCollections(
        IReadOnlyList<CompiledSchemaCollection> current,
        IReadOnlyList<CompiledSchemaCollection> desired,
        List<SchemaMigrationOperation> operations,
        StringComparer identifiers)
    {
        var oldCollections = current.ToDictionary(collection => collection.Name, identifiers);
        var newCollections = desired.ToDictionary(collection => collection.Name, identifiers);
        foreach (CompiledSchemaCollection collection in desired.OrderBy(value => value.Name, StringComparer.Ordinal))
        {
            if (!oldCollections.TryGetValue(collection.Name, out CompiledSchemaCollection? previous))
            {
                operations.Add(new SchemaMigrationOperation(
                    SchemaMigrationOperationKind.AddCollection,
                    SchemaMigrationSafety.Safe,
                    collection.Name));
            }
            else if (!CollectionEquals(previous, collection, identifiers))
            {
                operations.Add(new SchemaMigrationOperation(
                    SchemaMigrationOperationKind.AlterCollection,
                    SchemaMigrationSafety.Destructive,
                    collection.Name));
            }
        }

        foreach (CompiledSchemaCollection collection in current.OrderByDescending(value => value.Name, StringComparer.Ordinal))
        {
            if (!newCollections.ContainsKey(collection.Name))
            {
                operations.Add(new SchemaMigrationOperation(
    ...
    private static bool CollectionEquals(
        CompiledSchemaCollection left,
        CompiledSchemaCollection right,
        StringComparer identifiers)
        => identifiers.Equals(left.Name, right.Name)
        && string.Equals(left.EntryType, right.EntryType, StringComparison.Ordinal)
        && KeyEquals(left.Key, right.Key, identifiers)
        && left.Fields.SequenceEqual(right.Fields)
        && left.Indexes.Count == right.Indexes.Count
        && left.Indexes.Zip(right.Indexes).All(pair => IndexEquals(pair.First, pair.Second, identifiers));

    private static void EnsureSupportedMetadataIsUnchanged(CompiledSchema? current, CompiledSchema desired)
    {
        if (current is null)
        {
    ...
    private static DatabaseSchemaMigrationException UnsupportedMetadata(string schemaName)
        => new(
