...
public sealed class ResourceControlPlaneTests
{
    ...
    [Theory(DisplayName = "Cohesion Test [MediaHub.Hosting] - Control plane: managed and standalone resource routes")]
    [InlineData(true)]
    [InlineData(false)]
    public async Task ControlPlane_WithEndpoint_ShouldServeProbesAndManagement(bool managed)
    {
        ...
        await using IMediaHubApplication application = CreateBuilder(typeof(ResourceControlPlaneTests).Assembly).Build();
    ...
    [Fact(DisplayName = "Cohesion Test [MediaHub.Hosting] - Unregistered resource: builds the plain host without a listener")]
    public async Task Build_WithoutRegistration_ShouldLeaveHostUnconfigured()
    {
        ...
        await using IMediaHubApplication application = CreateBuilder(typeof(IMediaHubApplication).Assembly).Build();
    ...
    private static IMediaHubApplicationBuilder CreateBuilder(Assembly assembly)
    {
        MethodInfo method = typeof(MediaHubApplication).GetMethod("CreateBuilder", BindingFlags.Static | BindingFlags.NonPublic,
            binder: null, [typeof(string[]), typeof(Assembly)], modifiers: null)!;
        return (IMediaHubApplicationBuilder)method.Invoke(null, [Array.Empty<string>(), assembly])!;
    }
