...
The area root owns only the contracts that feature packages compose against. `ILogSpaceApplicationBuilder` is the contract-only builder seam, while `ILogSpaceApplication` supplies the hosting-free application lifecycle.
...
The concrete application, builder, and context live in `Assimalign.Cohesion.LogSpace.Hosting`; options and supporting services remain internal.
