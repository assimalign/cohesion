...
`MessageHubApplication` is the standalone host for the message broker resource. Per the Cohesion hosting model, each resource type runs as its own `Host<TContext>` subclass owning its own lifecycle in its own process; this project is that hosting shell, composing the resource's units of work as hosted services.
...
| --- | --- | --- |
| `JournalFlushService` | `DedicatedThreadService` (dedicated OS thread) | blocking loop to flush the message journal with synchronous file I/O |
| `BrokerEndpointService` | `BackgroundService` (pool-scheduled) | async loop to accept broker connections and dispatch messages |
...
- No builder or DI surface yet; construct `MessageHubApplication` with `MessageHubApplicationOptions` directly. A `CreateBuilder` surface can follow the `WebApplication` pattern when the resource matures.
