...
The area root owns only the contracts that feature packages compose against. `IMessageHubApplicationBuilder` is the contract-only builder seam, while `IMessageHubApplication` supplies the hosting-free application lifecycle.
...
The concrete application, builder, and context live in `Assimalign.Cohesion.MessageHub.Hosting`; options and supporting services remain internal.
