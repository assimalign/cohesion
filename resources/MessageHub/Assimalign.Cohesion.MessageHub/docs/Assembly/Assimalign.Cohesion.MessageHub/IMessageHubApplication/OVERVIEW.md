...
`IMessageHubApplication` is the public lifecycle contract for a MessageHub application. It extends `IHost` and adds the run-until-shutdown operation used by executable resources.
...
await using IMessageHubApplication application =
    MessageHubApplication.CreateBuilder(args).Build();
