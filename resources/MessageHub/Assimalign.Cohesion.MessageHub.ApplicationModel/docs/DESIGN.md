# MessageHub ApplicationModel design

MessageHubResource wraps a manifest snapshot and typed MessageHubResourceOptions, and delegates to its internal planner. AddMessageHub returns IMessageHubResourceDescriptor, a thin graph-descriptor wrapper retaining dependencies and the built plan. The planner validates the kind, Deployment workload, declared area endpoint schemes/protocols, and http control plane at /cohesion/v1, then delegates realization to GenericPlanner. Endpoint ports, extra endpoints, secret/configuration mounts, and generic deployer options remain manifest-driven; no platform types are referenced.
