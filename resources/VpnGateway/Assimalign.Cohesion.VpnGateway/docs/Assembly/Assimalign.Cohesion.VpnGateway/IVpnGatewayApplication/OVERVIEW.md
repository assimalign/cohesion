...
`IVpnGatewayApplication` is the public lifecycle contract for a VPN gateway application. It extends `IHost` and adds the run-until-shutdown operation used by executable resources.
...
await using IVpnGatewayApplication application =
    VpnGatewayApplication.CreateBuilder(args).Build();
