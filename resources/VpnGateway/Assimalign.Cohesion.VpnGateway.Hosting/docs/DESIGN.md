...
`VpnGatewayApplication` is the standalone host for the VPN gateway resource. Per the Cohesion hosting model, each resource type runs as its own `Host<TContext>` subclass owning its own lifecycle in its own process; this project is that hosting shell, composing the resource's units of work as hosted services.
...
| --- | --- | --- |
| `TunnelDataPlaneService` | `IHostService` directly (owns one thread per core) | per-core loops to encrypt and forward tunnel traffic |
...
- No builder or DI surface yet; construct `VpnGatewayApplication` with `VpnGatewayApplicationOptions` directly. A `CreateBuilder` surface can follow the `WebApplication` pattern when the resource matures.
