...
public sealed class ResourceControlPlaneTests
{
    ...
    [Theory(DisplayName = "Cohesion Test [VpnGateway.Hosting] - Control plane: managed and standalone resource routes")]
    [InlineData(true)]
    [InlineData(false)]
    public async Task ControlPlane_WithEndpoint_ShouldServeProbesAndManagement(bool managed)
    {
        ...
        await using IVpnGatewayApplication application = CreateBuilder(typeof(ResourceControlPlaneTests).Assembly).Build();
    ...
    [Fact(DisplayName = "Cohesion Test [VpnGateway.Hosting] - Unregistered resource: builds the plain host without a listener")]
    public async Task Build_WithoutRegistration_ShouldLeaveHostUnconfigured()
    {
        ...
        await using IVpnGatewayApplication application = CreateBuilder(typeof(IVpnGatewayApplication).Assembly).Build();
    ...
    private static IVpnGatewayApplicationBuilder CreateBuilder(Assembly assembly)
    {
        MethodInfo method = typeof(VpnGatewayApplication).GetMethod("CreateBuilder", BindingFlags.Static | BindingFlags.NonPublic,
            binder: null, [typeof(string[]), typeof(Assembly)], modifiers: null)!;
        return (IVpnGatewayApplicationBuilder)method.Invoke(null, [Array.Empty<string>(), assembly])!;
    }
