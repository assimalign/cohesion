...
public sealed class ResourceControlPlaneTests
{
    ...
    [Theory(DisplayName = "Cohesion Test [Rezolvr.Hosting] - Control plane: managed and standalone resource routes")]
    [InlineData(true)]
    [InlineData(false)]
    public async Task ControlPlane_WithEndpoint_ShouldServeProbesAndManagement(bool managed)
    {
        ...
        await using IRezolvrApplication application = CreateBuilder(typeof(ResourceControlPlaneTests).Assembly).Build();
    ...
    [Fact(DisplayName = "Cohesion Test [Rezolvr.Hosting] - Unregistered resource: builds the plain host without a listener")]
    public async Task Build_WithoutRegistration_ShouldLeaveHostUnconfigured()
    {
        using IDisposable scope = ResourceRuntime.CreateScope(new ResourceContext(
            endpoints: new Dictionary<string, Uri> { ["admin"] = Uri.CreateEndpoint("http", "127.0.0.1", ReservePort()) }));
        await using IRezolvrApplication application = CreateBuilder(typeof(IRezolvrApplication).Assembly).Build();
    ...
    private static IRezolvrApplicationBuilder CreateBuilder(Assembly assembly)
    {
        MethodInfo method = typeof(RezolvrApplication).GetMethod("CreateBuilder", BindingFlags.Static | BindingFlags.NonPublic,
            binder: null, [typeof(string[]), typeof(Assembly)], modifiers: null)!;
        return (IRezolvrApplicationBuilder)method.Invoke(null, [Array.Empty<string>(), assembly])!;
    }
