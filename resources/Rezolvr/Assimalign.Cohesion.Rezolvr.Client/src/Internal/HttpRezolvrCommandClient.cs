...
namespace Assimalign.Cohesion.Rezolvr.Client.Internal;

internal sealed class HttpRezolvrCommandClient : IRezolvrCommandClient
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="HttpRezolvrCommandClient"/> class.
    /// </summary>
    /// <param name="address">The base address of the Rezolvr command endpoint; commands are sent to its <c>/commands</c> path.</param>
    ...
    public HttpRezolvrCommandClient(Uri address, string bearerToken, HttpMessageInvoker transport, bool ownsTransport)
    {
        _address = address;
    ...
    private async ValueTask<ResourceCommandObservation> SendAsync(ResourceCommand command, HttpMethod method, CancellationToken cancellationToken)
    {
        ...
        string? detail = response.IsSuccessStatusCode ? null : $"Rezolvr command '{command.Kind}' was refused: HTTP {(int)response.StatusCode} {response.ReasonPhrase}.";
