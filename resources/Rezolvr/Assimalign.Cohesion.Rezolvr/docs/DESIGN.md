...
The area root owns only the contracts that feature packages compose against. `IRezolvrApplicationBuilder` is the contract-only builder seam, while `IRezolvrApplication` supplies the hosting-free application lifecycle.
...
The concrete application, builder, and context live in `Assimalign.Cohesion.Rezolvr.Hosting`; options and supporting services remain internal.
