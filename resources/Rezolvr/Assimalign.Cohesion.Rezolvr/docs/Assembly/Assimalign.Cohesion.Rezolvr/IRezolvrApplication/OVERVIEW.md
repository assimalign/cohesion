...
`IRezolvrApplication` is the public lifecycle contract for a Rezolvr application. It extends `IHost` and adds the run-until-shutdown operation used by executable resources.
...
await using IRezolvrApplication application =
    RezolvrApplication.CreateBuilder(args).Build();
