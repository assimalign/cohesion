...
internal sealed class ApiManagerResourceDescriptor : IApiManagerResourceDescriptor
{
    private readonly IApplicationResourceDescriptor _inner;

    internal ApiManagerResourceDescriptor(IApplicationResourceDescriptor inner)
    {
        _inner = inner ?? throw new ArgumentNullException(nameof(inner));
        Resource = inner.Resource as ApiManagerResource ?? throw new ArgumentException(
            "The descriptor must wrap a ApiManagerResource.",
            nameof(inner));
    }

    public ApiManagerResource Resource { get; }
