...
public sealed class ResourceControlPlaneTests
{
    ...
    [Theory(DisplayName = "Cohesion Test [NotificationHub.Hosting] - Control plane: managed and standalone resource routes")]
    [InlineData(true)]
    [InlineData(false)]
    public async Task ControlPlane_WithEndpoint_ShouldServeProbesAndManagement(bool managed)
    {
        ...
        await using INotificationHubApplication application = CreateBuilder(typeof(ResourceControlPlaneTests).Assembly).Build();
    ...
    [Fact(DisplayName = "Cohesion Test [NotificationHub.Hosting] - Unregistered resource: builds the plain host without a listener")]
    public async Task Build_WithoutRegistration_ShouldLeaveHostUnconfigured()
    {
        ...
        await using INotificationHubApplication application = CreateBuilder(typeof(INotificationHubApplication).Assembly).Build();
    ...
    private static INotificationHubApplicationBuilder CreateBuilder(Assembly assembly)
    {
        MethodInfo method = typeof(NotificationHubApplication).GetMethod("CreateBuilder", BindingFlags.Static | BindingFlags.NonPublic,
            binder: null, [typeof(string[]), typeof(Assembly)], modifiers: null)!;
        return (INotificationHubApplicationBuilder)method.Invoke(null, [Array.Empty<string>(), assembly])!;
    }
