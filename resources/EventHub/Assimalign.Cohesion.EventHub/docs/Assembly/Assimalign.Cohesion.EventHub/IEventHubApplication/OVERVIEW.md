...
`IEventHubApplication` is the public lifecycle contract for an event hub application. It extends `IHost` and adds the run-until-shutdown operation used by executable resources.
...
The current filler implementation registers no hosted services by default. Services explicitly added through `IEventHubApplicationBuilder` participate in the same lifecycle. A token that is already cancelled still drives the host through a clean start-and-stop transition; cancellation after startup requests graceful shutdown.
...
await using IEventHubApplication application =
    EventHubApplication.CreateBuilder(args).Build();
