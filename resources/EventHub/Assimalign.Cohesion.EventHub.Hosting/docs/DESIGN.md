...
`EventHubApplication` is the standalone host for the event streaming hub resource. Per the Cohesion hosting model, each resource type runs as its own `Host<TContext>` subclass owning its own lifecycle in its own process; this project is that hosting shell, composing the resource's units of work as hosted services.
...
| --- | --- | --- |
| `PartitionFlushService` | `DedicatedThreadService` (dedicated OS thread) | blocking loop to flush partition logs with synchronous file I/O |
| `IngressEndpointService` | `BackgroundService` (pool-scheduled) | async loop to accept event ingress connections |
...
- No builder or DI surface yet; construct `EventHubApplication` with `EventHubApplicationOptions` directly. A `CreateBuilder` surface can follow the `WebApplication` pattern when the resource matures.
