...
`INatGatewayApplication` is the public lifecycle contract for a NAT gateway application. It extends `IHost` and adds the run-until-shutdown operation used by executable resources.
...
await using INatGatewayApplication application =
    NatGatewayApplication.CreateBuilder(args).Build();
