...
internal static class NatGatewayPlanner
{
    internal static ResourcePlan CreatePlan(PlanContext context)
    {
        ArgumentNullException.ThrowIfNull(context);
        ResourceManifest manifest = context.Manifest;
        if (!string.Equals(manifest.Kind, "NatGateway", StringComparison.Ordinal))
        {
            throw new InvalidOperationException($"NatGateway planner cannot plan resource '{manifest.Name}' with kind '{manifest.Kind}'.");
        }

        if (manifest.Lifecycle.Workload is not WorkloadKind.Deployment)
        {
            throw new InvalidOperationException("NatGateway requires a Deployment workload.");
        }
        if (manifest.ControlPlane.Endpoint != "http" || manifest.ControlPlane.Path != "/cohesion/v1")
        {
            throw new InvalidOperationException("NatGateway requires the 'http' endpoint and '/cohesion/v1' control-plane path.");
        }
    ...
    private static void RequireEndpoint(ResourceManifest manifest, string name, string scheme, string protocol)
    {
        ...
        {
            throw new InvalidOperationException($"NatGateway requires the '{name}' {scheme}/{protocol} endpoint.");
        }
    }
}
