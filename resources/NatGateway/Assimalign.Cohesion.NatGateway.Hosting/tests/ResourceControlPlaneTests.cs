...
public sealed class ResourceControlPlaneTests
{
    ...
    [Theory(DisplayName = "Cohesion Test [NatGateway.Hosting] - Control plane: managed and standalone resource routes")]
    [InlineData(true)]
    [InlineData(false)]
    public async Task ControlPlane_WithEndpoint_ShouldServeProbesAndManagement(bool managed)
    {
        ...
        await using INatGatewayApplication application = CreateBuilder(typeof(ResourceControlPlaneTests).Assembly).Build();
    ...
    [Fact(DisplayName = "Cohesion Test [NatGateway.Hosting] - Unregistered resource: builds the plain host without a listener")]
    public async Task Build_WithoutRegistration_ShouldLeaveHostUnconfigured()
    {
        ...
        await using INatGatewayApplication application = CreateBuilder(typeof(INatGatewayApplication).Assembly).Build();
    ...
    private static INatGatewayApplicationBuilder CreateBuilder(Assembly assembly)
    {
        MethodInfo method = typeof(NatGatewayApplication).GetMethod("CreateBuilder", BindingFlags.Static | BindingFlags.NonPublic,
            binder: null, [typeof(string[]), typeof(Assembly)], modifiers: null)!;
        return (INatGatewayApplicationBuilder)method.Invoke(null, [Array.Empty<string>(), assembly])!;
    }
