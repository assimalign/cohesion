...
The area root owns only the contracts that feature packages compose against. `IEmailHubApplicationBuilder` is the contract-only builder seam, while `IEmailHubApplication` supplies the hosting-free application lifecycle.
...
The concrete application, builder, and context live in `Assimalign.Cohesion.EmailHub.Hosting`; options and supporting services remain internal.
