...
public sealed class ResourceControlPlaneTests
{
    ...
    [Theory(DisplayName = "Cohesion Test [EmailHub.Hosting] - Control plane: managed and standalone resource routes")]
    [InlineData(true)]
    [InlineData(false)]
    public async Task ControlPlane_WithEndpoint_ShouldServeProbesAndManagement(bool managed)
    {
        ...
        await using IEmailHubApplication application = CreateBuilder(typeof(ResourceControlPlaneTests).Assembly).Build();
    ...
    [Fact(DisplayName = "Cohesion Test [EmailHub.Hosting] - Unregistered resource: builds the plain host without a listener")]
    public async Task Build_WithoutRegistration_ShouldLeaveHostUnconfigured()
    {
        ...
        await using IEmailHubApplication application = CreateBuilder(typeof(IEmailHubApplication).Assembly).Build();
    ...
    private static IEmailHubApplicationBuilder CreateBuilder(Assembly assembly)
    {
        MethodInfo method = typeof(EmailHubApplication).GetMethod("CreateBuilder", BindingFlags.Static | BindingFlags.NonPublic,
            binder: null, [typeof(string[]), typeof(Assembly)], modifiers: null)!;
        return (IEmailHubApplicationBuilder)method.Invoke(null, [Array.Empty<string>(), assembly])!;
    }
