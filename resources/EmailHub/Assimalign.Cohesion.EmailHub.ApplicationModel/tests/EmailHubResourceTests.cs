...
namespace Assimalign.Cohesion.EmailHub.ApplicationModel.Tests;

public sealed class EmailHubResourceTests
{
    [Fact(DisplayName = "Cohesion Test [EmailHub.ApplicationModel] - Planning: preserves SDK defaults")]
    public void CreatePlan_WithDefaultManifest_ShouldPreserveAreaShape()
    {
        ResourceManifest manifest = CreateManifest();
        var options = new EmailHubResourceOptions();
        var resource = new EmailHubResource(manifest, options);
        ...
        resource.PlannerName.ShouldBe("EmailHub planner");
    ...
    [Fact(DisplayName = "Cohesion Test [EmailHub.ApplicationModel] - AddEmailHub: returns typed graph descriptor")]
    public void AddEmailHub_WithManifest_ShouldComposeTypedResourceAndDependencies()
    {
        IApplicationBuilder builder = Application.CreateBuilder();
        IEmailHubResourceDescriptor first = builder.AddEmailHub(CreateManifest());
        IEmailHubResourceDescriptor second = builder.AddEmailHub(CreateManifest() with { Name = "second" });

        second.DependsOn(first).ShouldBeSameAs(second);

        first.Resource.ShouldBeOfType<EmailHubResource>();
        second.Dependencies.ShouldContain(first);
        Should.Throw<ArgumentNullException>(() => builder.AddEmailHub(null!));
    }

    [Fact(DisplayName = "Cohesion Test [EmailHub.ApplicationModel] - Control plane: isolates empty command sets")]
    public void CreateControlPlane_ShouldReturnIsolatedEmptyPlanes()
    {
        IResourceControlPlane first = EmailHubResourceControlPlane.Create();
        IResourceControlPlane second = EmailHubResourceControlPlane.Create();
    ...
    [Theory(DisplayName = "Cohesion Test [EmailHub.ApplicationModel] - Planning: rejects incompatible area facts")]
    [InlineData("kind")]
    [InlineData("workload")]
    [InlineData("endpoint")]
    [InlineData("path")]
    public void CreatePlan_WithInvalidFacts_ShouldReject(string invalid)
    {
        ...
        var resource = new EmailHubResource(manifest);

        Should.Throw<InvalidOperationException>(() => resource.CreatePlan(CreateContext(resource)));
    }

    [Fact(DisplayName = "Cohesion Test [EmailHub.ApplicationModel] - Planning: rejects another resource context")]
    public void CreatePlan_WithOtherResourceContext_ShouldReject()
    {
        var resource = new EmailHubResource(CreateManifest());
        var other = new EmailHubResource(CreateManifest() with { Name = "other" });
        Should.Throw<InvalidOperationException>(() => resource.CreatePlan(CreateContext(other)));
    }

    [Fact(DisplayName = "Cohesion Test [EmailHub.ApplicationModel] - Planning: preserves extra endpoints and secret mounts")]
    public void CreatePlan_WithAdditionalTraits_ShouldPreserveThem()
    {
        ResourceManifest source = CreateManifest();
        var resource = new EmailHubResource(source with
        {
    ...
    private static PlanContext CreateContext(EmailHubResource resource) => new(
    ...
    private static ResourceManifest CreateManifest() => new()
    {
        ...
        Kind = "EmailHub",
        Application = "sample",
        ApplicationModel = "Assimalign.Cohesion.EmailHub.ApplicationModel",
        Artifact = new ResourceManifestArtifact { Assembly = "Sample.EmailHub", Composable = true },
        Endpoints =
        [
