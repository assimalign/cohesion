...
internal static class IoTHubPlanner
{
    internal static ResourcePlan CreatePlan(PlanContext context)
    {
        ...
        if (!string.Equals(manifest.Kind, "IoTHub", StringComparison.Ordinal))
        {
            throw new InvalidOperationException($"IoTHub planner cannot plan resource '{manifest.Name}' with kind '{manifest.Kind}'.");
        }

        if (manifest.Lifecycle.Workload is not WorkloadKind.Deployment)
        {
            throw new InvalidOperationException("IoTHub requires a Deployment workload.");
        }
        if (manifest.ControlPlane.Endpoint != "http" || manifest.ControlPlane.Path != "/cohesion/v1")
        {
            throw new InvalidOperationException("IoTHub requires the 'http' endpoint and '/cohesion/v1' control-plane path.");
        }
    ...
    private static void RequireEndpoint(ResourceManifest manifest, string name, string scheme, string protocol)
    {
        ...
        {
            throw new InvalidOperationException($"IoTHub requires the '{name}' {scheme}/{protocol} endpoint.");
        }
    }
}
