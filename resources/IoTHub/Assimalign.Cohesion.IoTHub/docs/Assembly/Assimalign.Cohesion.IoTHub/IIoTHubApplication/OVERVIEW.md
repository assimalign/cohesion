...
The current filler implementation registers no hosted services by default. Services explicitly added through `IIoTHubApplicationBuilder` participate in the same lifecycle. A token that is already cancelled still drives the host through a clean start-and-stop transition; cancellation after startup requests graceful shutdown.
