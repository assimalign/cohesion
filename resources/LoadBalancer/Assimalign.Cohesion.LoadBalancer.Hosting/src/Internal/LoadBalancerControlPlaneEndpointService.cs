...
internal sealed class LoadBalancerControlPlaneEndpointService : IHostService, IDisposable
{
        ...
        LoadBalancerApplicationContext applicationContext)
    {
        ...
        {
            throw new InvalidOperationException("The LoadBalancer control-plane endpoint 'http' requires http or https.");
        }
        ...
        if (string.Equals(endpoint.Scheme, "https", StringComparison.OrdinalIgnoreCase))
        {
            ...
            {
                throw new InvalidOperationException("The LoadBalancer https endpoint 'http' requires its certificate Secret mount (default 'tls').");
            }
