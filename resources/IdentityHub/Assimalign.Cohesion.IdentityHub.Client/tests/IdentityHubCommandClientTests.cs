using System;

using Shouldly;
using Xunit;

namespace Assimalign.Cohesion.IdentityHub.Client.Tests;

public sealed class IdentityHubCommandClientTests
{
    [Fact(DisplayName = "Cohesion Test [IdentityHub.Client] - Create: Disposing a client retains its caller-owned transport")]
    public void Create_WithCallerOwnedTransport_ShouldNotDisposeIt()
    {
        // Arrange
        using var transport = new RecordingHttpMessageInvoker();
        using IIdentityHubCommandClient client = IdentityHubCommandClient.Create(
            new Uri("https://resource.test:8443/custom/control"), "bootstrap-token", transport);

        // Act
        client.Dispose();

        // Assert
        transport.IsDisposed.ShouldBeFalse();
    }
}
