...
public static class IdentityHubCommandClient
{
    ...
    /// <exception cref="ArgumentException">The address is not HTTP(S) or the credential is blank.</exception>
    public static IIdentityHubCommandClient Create(
    ...
    private static IIdentityHubCommandClient Create(
        Uri controlPlaneAddress, string bearerToken, HttpMessageInvoker transport, bool ownsTransport) =>
        new HttpIdentityHubCommandClient(controlPlaneAddress, bearerToken, transport, ownsTransport);
