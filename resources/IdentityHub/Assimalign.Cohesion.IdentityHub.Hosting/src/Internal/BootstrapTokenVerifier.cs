...
internal sealed class BootstrapTokenVerifier : IDisposable
{
    ...
    internal BootstrapTokenVerifier(ResourceContext context)
    {
        ...
        _application = context.ApplicationName ?? throw new InvalidOperationException(
            "A gateway-managed IdentityHub requires an application name.");
        _gateway = context.GatewayName ?? throw new InvalidOperationException(
            "A gateway-managed IdentityHub requires a gateway name.");
        if (context.ApplicationTrustKey.IsEmpty)
        {
            throw new InvalidOperationException(
                "A gateway-managed IdentityHub requires a public application trust key.");
        }
        ...
        {
            throw new InvalidOperationException("The application trust key is not a valid EC P-256 JWK.", exception);
        }
    }
        ...
        DateTimeOffset now)
    {
        if (!JsonWebToken.TryParse(compactToken, out JsonWebToken? token) ||
            token is null || token.SigningInput is null || token.Parts is null ||
        ...
        bool valid = token.Validate(options).Succeeded &&
            ...
            expiresAt > issuedAt && expiresAt > notBefore &&
