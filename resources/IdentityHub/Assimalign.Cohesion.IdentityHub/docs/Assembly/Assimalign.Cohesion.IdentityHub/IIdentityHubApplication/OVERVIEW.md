...
Hosting registers the built-in issuer service after services explicitly added through `IIdentityHubApplicationBuilder`; all participate in the same lifecycle. A token that is already cancelled still drives the host through a clean start-and-stop transition; cancellation after startup requests graceful shutdown.
