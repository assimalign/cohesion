...
| `Assimalign.Cohesion.Web.Api` | Endpoint mapping over the router: plain `Map`/`MapGet` terminal middleware plus source-generated typed-delegate binding (`(int id, IHttpContext) => ...` — route/query/header/body/form + injections, 400/415 outcomes) and an opt-in per-endpoint validation seam; the interceptor generator lives in `analyzers/Assimalign.Cohesion.SourceGeneration.Web` |
