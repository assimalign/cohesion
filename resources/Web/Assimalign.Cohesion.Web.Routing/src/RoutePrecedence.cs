...
using Assimalign.Cohesion.Web.Routing.Template;
...
public static class RoutePrecedence
{
    ...
    private static int ComputeOutboundPrecedenceDigit(RouteTemplateSegment segment)
    {
    ...
    private static int ComputeOutboundPrecedenceDigit(RoutePatternPathSegment pathSegment)
    {
        ...
        var part = pathSegment.Segments[0];
        if (part.IsLiteral)
        {
            return 5;
        }
        else if (part is RoutePatternParameterSegment parameterPart)
        {
    ...
    private static int ComputeInboundPrecedenceDigit(RouteTemplateSegment segment)
    {
        if (segment.Parts.Count > 1)
        {
