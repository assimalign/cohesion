...
| --- | --- | --- | --- |
| `L01.01.NN` (3 seg) | **Area epic** | `[L01.01.11] Foundation - Http` (#314) | parent for a new **feature** |
| `L01.01.NN.MM` (4 seg) | **Feature** | `[L01.01.11.14] …extended connect` | parent for a new **task** |
| `L01.01.NN.MM.PP` (5 seg) | **Task** | `[L01.01.11.18.01] …` (#713) | leaf |

Your branch names the feature in flight: `feature/L01.01.11.14-extended-connect` → feature `L01.01.11.14`.
...
.claude/skills/cohesion-work-items/scripts/New-CohesionWorkItem.ps1 -As task `
  -Title "Validate CONNECT authority form against RFC 9110 origin rules" `
  -Summary "Hardening discovered while wiring extended CONNECT; outside the original feature scope." `
  -Acceptance "Reject malformed authority components.","Add targeted unit tests.","NativeAOT-safe." `
  -Status "In progress"
...
.claude/skills/cohesion-work-items/scripts/New-CohesionWorkItem.ps1 -As feature `
  -Title "Implement HTTP/1.1 informational (1xx) response support" -Wave W02 -Priority P003

# Different area, explicit parent (e.g. Net area epic #324):
.claude/skills/cohesion-work-items/scripts/New-CohesionWorkItem.ps1 -Parent 324 `
  -Title "Add IPv6 scope-id parsing to the endpoint formatter"
...
The repo's de-facto issue body is `## Summary` → `## Acceptance Criteria` → `### Standards and Compliance`.
The script generates this skeleton; fill it with real, testable criteria. For wire-protocol work, cite the
RFC/spec in *Standards and Compliance*; for runtime-contract work, note that no conformance suite applies
(mirrors the existing issues, e.g. #703/#713).
...
- **Only operate on `assimalign/cohesion`** and Project #13. Confirm `gh auth status` has the `project` scope.
