...
<#
.SYNOPSIS
    Create a GitHub work item (feature / task) in the assimalign/cohesion Project #13,
    following the L01.01.NN WBS scheme, native parent/sub-issue links, and the
...
.DESCRIPTION
    The reliable, multi-step intake path used by the `cohesion-work-items` skill:
      ...
      5. Add it to Project #13 and set Status / Priority / Wave / Kind / Area / Origin fields.
...
.EXAMPLE
    ...
    ./New-CohesionWorkItem.ps1 -Search "websocket bootstrap http3"

.EXAMPLE
    # Scope-creep TASK discovered on feature branch feature/L01.01.11.14-extended-connect.
    ./New-CohesionWorkItem.ps1 -As task -Title "Validate CONNECT authority form against RFC 9110 origin rules" `
...
.EXAMPLE
    ...
    ./New-CohesionWorkItem.ps1 -As feature -Title "Implement HTTP/1.1 informational (1xx) responses" -Wave W02

.EXAMPLE
    # Assemble the grouped "Closes #N" block (planned vs discovered) for the current branch's PR body.
    ./New-CohesionWorkItem.ps1 -EmitClosesBlock
#>
...
param(
    ...
    # Explicit parent: an issue number (e.g. 314) or a WBS code (e.g. L01.01.11). Overrides -As inference.
    [Parameter(ParameterSetName = 'Create')]
    [string] $Parent,

    # Explicit WBS code for the new item (e.g. L01.01.02.07) when a design document pre-assigns codes.
...
function Get-BranchWbs {
    ...
    $m = [regex]::Match($branch, 'L\d{2}(?:\.\d{2})+')
...
if ($Parent) {
    if ($Parent -match '^\d+$') {
        ...
        $m = [regex]::Match($parentIssue.title, '^\[(L\d{2}(?:\.\d{2})+)\]')
...
$areaName = $null
if ($areaEpic) {
    $am = [regex]::Match($areaEpic.title, 'Foundation - (.+)$')
