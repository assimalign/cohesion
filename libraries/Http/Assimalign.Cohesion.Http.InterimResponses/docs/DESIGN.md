...
- **The transport** owns the per-version wire emission — an HTTP/1.1 status line,
  ...
  `Content-Length` — behind its internal per-protocol `IHttpExchangeControl`
  implementations it exposes through the interceptor context. It has no interim
  *feature* type.
...
path with zero interim overhead (the exchange control is created only when at
least one response interceptor is registered).
...
- **`IsInterimResponseSupported`** reflects the exchange control's
...
- **`SendInterimResponseAsync`** forwards to the control's
  `WriteInterimResponseAsync`, which validates the status (`1xx`, not `101` — `101 Switching Protocols` belongs to
...
- **The `1xx`-as-final-status guard.** Rejecting a `1xx` set as the final
  ...
  here.
- **Per-version wire framing.** Owned by the transport behind the control.

## AOT posture

No reflection or runtime code generation. The feature is a thin forwarder over the
exchange control; the extensions build a small `HttpHeaderCollection` for the
