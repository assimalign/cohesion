...
public struct KeyValueAccumulator
{
    ...
    public void Append(string key, string value)
    {
        ...
        else
        {
            ...
            _accumulator[key] = new List<string>([value]);
        }
    ...
    public Dictionary<string, List<string>> GetResults()
    {
        if (_expandingAccumulator != null)
        {
            ...
            {
                _accumulator[entry.Key] = new List<string>(entry.Value.ToArray());
            }
        }

        return _accumulator ?? new Dictionary<string, List<string>>(0, StringComparer.OrdinalIgnoreCase);
    }
}