...
{
    private readonly HttpMediaTypeParameter[]? _parameters;
    ...
    private HttpMediaType(string type, string subType, HttpMediaTypeParameter[]? parameters)
    {
        Type = type;
        SubType = subType;
        this._parameters = parameters is { Length: > 0 } ? parameters : null;
    }
    ...
    public HttpMediaType(string value)
    {
        ...
        _parameters = parsed._parameters;
    }
    ...
    public IReadOnlyList<HttpMediaTypeParameter> Parameters
        => _parameters ?? (IReadOnlyList<HttpMediaTypeParameter>)Array.Empty<HttpMediaTypeParameter>();
    ...
    public bool TryGetParameter(string name, out string? value)
    {
        ...
        {
            foreach (HttpMediaTypeParameter parameter in _parameters)
            {
