...
{
    private readonly HttpMediaType[]? _mediaRanges;
    ...
    {
        this._mediaRanges = mediaRanges is { Length: > 0 } ? mediaRanges : null;
    }
    ...
    public HttpAcceptQuery(IEnumerable<HttpMediaType> mediaRanges)
    {
        ...
        this._mediaRanges = buffer.Count == 0 ? null : buffer.ToArray();
    }
    ...
    public IReadOnlyList<HttpMediaType> MediaRanges
        => _mediaRanges ?? (IReadOnlyList<HttpMediaType>)Array.Empty<HttpMediaType>();

    /// <summary>Gets the number of advertised media ranges.</summary>
    public int Count => _mediaRanges?.Length ?? 0;

    /// <summary>Gets a value indicating whether the field advertises no media ranges.</summary>
    public bool IsEmpty => _mediaRanges is null;
    ...
    {
        if (_mediaRanges is null)
        {
            return false;
        }
        foreach (HttpMediaType range in _mediaRanges)
        {
    ...
    public string Serialize()
    {
        ...
        var members = new StructuredFieldMember[_mediaRanges.Length];
        for (int i = 0; i < _mediaRanges.Length; i++)
        {
            members[i] = StructuredFieldMember.FromItem(ToStructuredFieldItem(_mediaRanges[i]));
        }
    ...
    public bool Equals(HttpAcceptQuery other)
    {
        ...
        {
            if (!_mediaRanges![i].Equals(other._mediaRanges![i]))
            {
                return false;
            }
        }
