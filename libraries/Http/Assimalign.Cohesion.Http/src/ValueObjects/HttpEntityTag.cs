...
public readonly struct HttpEntityTag : IEquatable<HttpEntityTag>
{
    ...
    /// parsed).
    /// </summary>
    public bool IsEmpty => _tag is null;
    ...
    public bool Equals(HttpEntityTag other)
        => IsWeak == other.IsWeak && string.Equals(_tag, other._tag, StringComparison.Ordinal);

    /// <inheritdoc />
    public override bool Equals(object? obj) => obj is HttpEntityTag other && Equals(other);

    /// <inheritdoc />
    public override int GetHashCode() => HashCode.Combine(IsWeak, _tag is null ? 0 : string.GetHashCode(_tag, StringComparison.Ordinal));
