...
public readonly struct HttpForwardedNode : IEquatable<HttpForwardedNode>
{
    ...
    public int? PortNumber
        => _port is not null && IsNumericPort(_port.AsSpan()) ? int.Parse(_port) : null;

    /// <summary>Gets a value indicating whether the node is the RFC 7239 <c>unknown</c> sentinel.</summary>
    public bool IsUnknown => _name is not null && string.Equals(_name, UnknownName, StringComparison.OrdinalIgnoreCase);

    /// <summary>
    /// Gets a value indicating whether the nodename is an RFC 7239 &#167; 6.3 obfuscated identifier
    /// (begins with <c>_</c>).
    /// </summary>
    public bool IsObfuscatedName => _name is { Length: > 0 } && _name[0] == '_';

    /// <summary>
    /// Gets a value indicating whether the port is an RFC 7239 &#167; 6.4 obfuscated port
    /// (begins with <c>_</c>).
    /// </summary>
    public bool HasObfuscatedPort => _port is { Length: > 0 } && _port[0] == '_';
    ...
    public override string ToString()
        => IsEmpty ? string.Empty : _port is null ? _name! : string.Concat(_name, ":", _port);

    /// <inheritdoc />
    public bool Equals(HttpForwardedNode other)
        => string.Equals(_name, other._name, StringComparison.OrdinalIgnoreCase)
        && string.Equals(_port, other._port, StringComparison.OrdinalIgnoreCase);
    ...
    public override int GetHashCode()
        => HashCode.Combine(
            ...
            _port is null ? 0 : StringComparer.OrdinalIgnoreCase.GetHashCode(_port));
