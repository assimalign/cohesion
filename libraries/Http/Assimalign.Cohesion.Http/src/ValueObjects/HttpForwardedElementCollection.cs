...
{
    private readonly HttpForwardedElement[]? _elements;
    ...
    public HttpForwardedElement this[int index]
        => _elements is null ? throw new ArgumentOutOfRangeException(nameof(index)) : _elements[index];
    ...
    /// </summary>
    public HttpForwardedElement? Nearest => Count == 0 ? null : _elements![^1];
    ...
    public ReadOnlySpan<HttpForwardedElement> AsSpan() => _elements;
    ...
    {
        if (_elements is null || _elements.Length <= 1)
        {
            return this;
        }
        var reversed = new HttpForwardedElement[_elements.Length];
        for (int i = 0; i < _elements.Length; i++)
        {
            reversed[i] = _elements[_elements.Length - 1 - i];
        }
    ...
    {
        if (_elements is null || _elements.Length == 0)
        {
            return string.Empty;
        }
        ...
        for (int i = 0; i < _elements.Length; i++)
        {
            ...
            builder.Append(_elements[i].Serialize());
        }
    ...
    public IEnumerator<HttpForwardedElement> GetEnumerator()
        => ((IEnumerable<HttpForwardedElement>)(_elements ?? Array.Empty<HttpForwardedElement>())).GetEnumerator();
    ...
    public override int GetHashCode()
    {
        ...
        foreach (HttpForwardedElement element in _elements)
        {
            hash.Add(element);
        }
