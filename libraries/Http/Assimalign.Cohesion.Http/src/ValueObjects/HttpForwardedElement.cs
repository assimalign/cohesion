...
public readonly struct HttpForwardedElement : IEquatable<HttpForwardedElement>
{
    ...
    private readonly HttpForwardedParameter[]? _parameters;
    private readonly HttpForwardedNode _forNode;
    private readonly HttpForwardedNode _byNode;

    private HttpForwardedElement(HttpForwardedParameter[] parameters, HttpForwardedNode forNode, HttpForwardedNode byNode)
    {
        this._parameters = parameters;
        this._forNode = forNode;
        this._byNode = byNode;
    }
    ...
    /// </summary>
    public HttpForwardedNode? For => _forNode.IsEmpty ? null : _forNode;
    ...
    /// </summary>
    public HttpForwardedNode? By => _byNode.IsEmpty ? null : _byNode;
    ...
    public IReadOnlyList<HttpForwardedParameter> Parameters
        => _parameters ?? (IReadOnlyList<HttpForwardedParameter>)Array.Empty<HttpForwardedParameter>();

    /// <summary>Gets a value indicating whether this is the default (unparsed) instance.</summary>
    public bool IsEmpty => _parameters is null;
    ...
    public bool TryGetParameter(string name, out string? value)
    {
        ...
        {
            foreach (HttpForwardedParameter parameter in _parameters)
            {
    ...
    public string Serialize()
    {
        ...
        for (int i = 0; i < _parameters.Length; i++)
        {
            ...
            builder.Append(_parameters[i].Name).Append('=');
            AppendValue(builder, _parameters[i].Value);
        }
    ...
    {
        int count = _parameters?.Length ?? 0;
        if (count != (other._parameters?.Length ?? 0))
        {
            return false;
        }
