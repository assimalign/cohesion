...
public readonly struct HttpForwardedValues : IReadOnlyList<string>, IEquatable<HttpForwardedValues>
{
    ...
    public string this[int index]
        => _entries is null ? throw new ArgumentOutOfRangeException(nameof(index)) : _entries[index];
    ...
    /// </summary>
    public string? Nearest => Count == 0 ? null : _entries![^1];
    ...
    public ReadOnlySpan<string> AsSpan() => _entries;
    ...
    {
        if (_entries is null || _entries.Length <= 1)
        {
            return this;
        }
        var reversed = new string[_entries.Length];
        for (int i = 0; i < _entries.Length; i++)
        {
            reversed[i] = _entries[_entries.Length - 1 - i];
        }
    ...
    public string Serialize()
        => _entries is null || _entries.Length == 0 ? string.Empty : string.Join(", ", _entries);
    ...
    public IEnumerator<string> GetEnumerator()
        => ((IEnumerable<string>)(_entries ?? Array.Empty<string>())).GetEnumerator();
    ...
    public bool Equals(HttpForwardedValues other)
    {
        ...
        {
            if (!string.Equals(_entries![i], other._entries![i], StringComparison.Ordinal))
            {
                return false;
            }
        }
