...
/// <summary>
/// HTTP/2 <see cref="IHttpInterimResponseFeature"/>. Emits an interim (<c>1xx</c>) response as an
/// additional HEADERS block on the exchange's stream, ahead of the final response (RFC 9113 §8.1). The
/// wire emission is delegated to <see cref="Http2ConnectionContext.WriteInterimResponseAsync"/>, which
/// holds the connection write gate so the interim HEADERS never interleave with concurrent frames.
/// </summary>
internal sealed class Http2InterimResponseFeature : IHttpInterimResponseFeature
{
    private readonly Http2ConnectionContext _connection;
    private readonly Http2Context _context;

    public Http2InterimResponseFeature(Http2ConnectionContext connection, Http2Context context)
    {
