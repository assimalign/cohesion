...
/// <summary>
/// HTTP/1.1 <see cref="IHttpInterimResponseWriter"/> — the transport capability the interim-response
/// feature package taps through <see cref="HttpResponseInterceptorContext.InterimResponseWriter"/>.
/// Writes an interim (<c>1xx</c>) status line and its fields directly onto the connection stream ahead
/// of the final response (RFC 9110 §15.2). An HTTP/1.1 exchange owns its whole connection and the
/// handler is the sole writer for its duration, so no frame-interleaving discipline is needed — the
/// interim bytes simply precede the final response bytes on the wire.
/// </summary>
internal sealed class Http1InterimResponseWriter : IHttpInterimResponseWriter
{
    private readonly Http1Context _context;
    private readonly Stream _stream;

    public Http1InterimResponseWriter(Http1Context context, Stream stream)
    {
        _context = context;
        _stream = stream;
    }
