...
/// <summary>
/// HTTP/3 <see cref="IHttpInterimResponseWriter"/> — the transport capability the interim-response
/// feature package taps through <see cref="HttpResponseInterceptorContext.InterimResponseWriter"/>.
/// Emits an interim (<c>1xx</c>) response as an additional QPACK-encoded HEADERS frame on the request
/// stream, ahead of the final HEADERS frame (RFC 9114 §4.1). The wire emission is delegated to
/// <see cref="Http3ConnectionContext.WriteInterimResponseAsync"/>.
/// </summary>
internal sealed class Http3InterimResponseWriter : IHttpInterimResponseWriter
{
    private readonly Http3ConnectionContext _connection;
    private readonly Http3Context _context;

    public Http3InterimResponseWriter(Http3ConnectionContext connection, Http3Context context)
    {
