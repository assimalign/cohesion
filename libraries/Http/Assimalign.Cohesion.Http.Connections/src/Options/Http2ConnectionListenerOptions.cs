...
/// <summary>
/// Configuration for an HTTP/2 listener registered through
/// <see cref="HttpConnectionListenerOptions.UseHttp2(Assimalign.Cohesion.Connections.IConnectionListener, Action{Http2ConnectionListenerOptions})"/>.
/// HTTP/2-specific tunables live here — rather than on the shared
...
/// </summary>
public sealed class Http2ConnectionListenerOptions
{
    /// <summary>
    /// Gets the HTTP/2 abuse-mitigation limits enforced on every connection this registration
    /// accepts (rapid reset, CONTINUATION flood, decoded header-list size, SETTINGS/PING floods,
    /// and the concurrent-stream cap). The bounds carry conservative Kestrel-parity defaults;
    /// mutate the returned instance to tune them.
    /// </summary>
    public Http2Limits Limits { get; } = new Http2Limits();

    /// <summary>
    /// The HTTP/2 limits: extends the shared <see cref="HttpConnectionListenerLimits"/> with the
    /// HTTP/2 abuse-mitigation caps.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// HTTP/2 abuse classes — rapid stream reset (CVE-2023-44487), CONTINUATION flooding,
    /// oversized decoded header lists, and SETTINGS/PING floods — each of which forces the server
    /// to perform unbounded work for a trivially cheap client message. These limits close those
    /// vectors.
    /// </para>
    /// <para>
    ...
    /// or configuration dependency in this package (Lane A guardrail — config binding is a
    /// Web.Hosting builder-time concern). HTTP/3 stream limits are governed by the QUIC transport
    /// and are out of scope for this surface.
    /// </para>
    /// </remarks>
    public sealed class Http2Limits : HttpConnectionListenerLimits
    {
