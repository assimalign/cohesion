...
namespace Assimalign.Cohesion.Dns
{
    ...
    public class DnsClient : IDnsClient
    {
        ...
        public static async Task<DnsPacket> RecursiveResolveAsync(DnsQuestionRecord question, IDnsCache cache = null, NetProxy proxy = null, bool preferIPv6 = false, ushort udpPayloadSize = DnsPacket.EDNS_DEFAULT_UDP_PAYLOAD_SIZE, bool randomizeName = false, bool qnameMinimization = false, bool dnssecValidation = false, NetworkAddress eDnsClientSubnet = null, int retries = 2, int timeout = 2000, int concurrency = 2, int maxStackCount = 16, bool minimalResponse = false, bool asyncNsRevalidation = false, bool asyncNsResolution = false, List<DnsPacket> rawResponses = null, CancellationToken cancellationToken = default)
        {
            ...
            async Task<List<NameServerAddress>> ResolveNameServerAddressesFromCacheAsync(List<NameServerAddress> nameServers)
            {
                ...
                foreach (NameServerAddress nameServer in nameServers)
                {
                    ...
                    if (preferIPv6)
                    {
                        DnsPacket cacheRequest = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, false, DnsResponseCode.NoError, [new DnsQuestionRecord(nameServer.DomainEndPoint.Address, DnsResourceRecordType.AAAA, DnsClass.IN)]);
                    ...
                    {
                        DnsPacket cacheRequest = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, false, DnsResponseCode.NoError, [new DnsQuestionRecord(nameServer.DomainEndPoint.Address, DnsResourceRecordType.A, DnsClass.IN)]);
                        DnsPacket cacheResponse = await cache.QueryAsync(cacheRequest);
            ...
            while (true) //stack loop
            {
                if (resolverStack.Count > maxStackCount)
                {
                    ...
                    DnsPacket failureResponse = new DnsPacket(0, true, DnsOpCode.StandardQuery, false, false, false, false, false, false, DnsResponseCode.ServerFailure, new DnsQuestionRecord[] { question });
                ...
                //query cache
                {
                    ...
                    DnsPacket cacheRequest = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, false, DnsResponseCode.NoError, new DnsQuestionRecord[] { question }, null, null, null, udpPayloadSize, ednsFlags, resolverStack.Count == 0 ? eDnsClientSubnetOption : null);
                                        ...
                                        else
                                        {
                                            ...
                                            if (nextNameServers.Count == 0)
                                            {
                                                ...
                                                while (true)
                                                {
                                                    ...
                                                    DnsPacket cachedNsResponse = await cache.QueryAsync(new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, false, DnsResponseCode.NoError, new DnsQuestionRecord[] { new DnsQuestionRecord(currentDomain, DnsResourceRecordType.NS, DnsClass.IN) }), findClosestNameServers: true);
        ...
        public static Task<DnsPacket> RecursiveResolveQueryAsync(DnsQuestionRecord question, IDnsCache cache = null, NetProxy proxy = null, bool preferIPv6 = false, ushort udpPayloadSize = DnsPacket.EDNS_DEFAULT_UDP_PAYLOAD_SIZE, bool randomizeName = false, bool qnameMinimization = false, bool dnssecValidation = false, NetworkAddress eDnsClientSubnet = null, int retries = 2, int timeout = 2000, int concurrency = 2, int maxStackCount = 16, CancellationToken cancellationToken = default)
        {
            if (cache is null)
                cache = new DnsCache();
        ...
        public static async Task<IReadOnlyList<string>> ResolveMXAsync(IDnsClient dnsClient, string domain, bool resolveIP = false, bool preferIPv6 = false, CancellationToken cancellationToken = default)
        {
            ...
            DnsPacket response = await dnsClient.ResolveAsync(new DnsQuestionRecord(domain, DnsResourceRecordType.MX, DnsClass.IN), cancellationToken);
        ...
        public static IReadOnlyList<IPAddress> ParseResponseA(DnsPacket response)
        {
            ...
            switch (response.ResponseCode)
            {
                ...
                default:
                    throw new DnsClientFailureResponseException("DnsClient failed to resolve the request '" + response.Question[0].ToString() + "'. Received a response with RCODE: " + response.ResponseCode + (response.Metadata is null ? "" : " from Name server: " + response.Metadata.NameServer.ToString()), response);
            }
        }
        ...
        private static async Task<List<NameServerAddress>> GetRootServersUsingRootHintsAsync(IDnsCache cache, NetProxy proxy, bool preferIPv6, ushort udpPayloadSize, bool dnssecValidation, int retries, int timeout, CancellationToken cancellationToken = default)
        {
            ...
            if (response.ResponseCode != DnsResponseCode.NoError)
                throw new DnsClientFailureResponseException("DnsClient failed to resolve the request '" + question.ToString() + "'. Received a response with RCODE: " + response.ResponseCode + (response.Metadata is null ? "" : " from Name server: " + response.Metadata.NameServer.ToString()), response);
            ...
            DnsPacket dnsKeyResponse = await dnsClient.InternalResolveAsync(dnsKeyRequest, async delegate (DnsPacket dnsKeyResponse, CancellationToken cancellationToken1)
            {
                if (dnsKeyResponse.Answer.Count == 0)
                {
                    switch (dnsKeyResponse.ResponseCode)
                    {
                        ...
                        default:
                            dnsKeyResponse.AddDnsClientExtendedError(EDnsExtendedDnsErrorCode.DNSKEYMissing, "Attack detected! " + (dnsKeyResponse.Metadata is null ? "name server" : dnsKeyResponse.Metadata.NameServer.ToString()) + " returned RCODE=" + dnsKeyResponse.ResponseCode.ToString() + " for " + dnsKeyQuestion.ToString());
                            cache.CacheResponse(dnsKeyResponse, true);
                            throw new DnsClientResponseDnssecValidationException("Attack detected! Failed to resolve the request '" + dnsKeyResponse.Question[0].ToString() + "'. Received a response with RCODE: " + dnsKeyResponse.ResponseCode + (dnsKeyResponse.Metadata is null ? "" : " from Name server: " + dnsKeyResponse.Metadata.NameServer.ToString()), dnsKeyResponse);
                    }
                }
            ...
            try
            {
                _ = await dnsClient.InternalResolveAsync(dsRequest, async delegate (DnsPacket dsResponse, CancellationToken cancellationToken1)
                {
                    ...
                    switch (dsResponse.ResponseCode)
                    {
                        ...
                        default:
                            dsResponse.AddDnsClientExtendedError(EDnsExtendedDnsErrorCode.DnssecIndeterminate, "Attack detected! " + (dsResponse.Metadata is null ? "name server" : dsResponse.Metadata.NameServer.ToString()) + " returned RCODE=" + dsResponse.ResponseCode.ToString() + " for " + dsQuestion.ToString());
                            cache.CacheResponse(dsResponse, true);
                            throw new DnsClientResponseDnssecValidationException("Attack detected! Failed to resolve the request '" + dsResponse.Question[0].ToString() + "'. Received a response with RCODE: " + dsResponse.ResponseCode + (dsResponse.Metadata is null ? "" : " from Name server: " + dsResponse.Metadata.NameServer.ToString()), dsResponse);
                    }
                }, false, cancellationToken);
            }
            catch (DnsClientResponseDnssecValidationException ex)
        ...
        private static async Task<DnssecProofOfNonExistence> GetValidatedProofOfNonExistenceAsync(DnsPacket response, string domain, DnsResourceRecordType type, bool wildcardAnswerValidation = false, string wildcardNextCloserName = null)
        {
            foreach (DnsResourceRecord record in response.Authority)
            {
        ...
        private static async Task RevalidateNameServersFromChildSideAsync(string zoneCut, IReadOnlyList<DnsResourceRecord> lastDSRecords, IReadOnlyList<NameServerAddress> parentSideNameServers, IDnsCache cache, NetProxy proxy, bool preferIPv6, ushort udpPayloadSize, bool randomizeName, bool qnameMinimization, bool dnssecValidation, int retries, int timeout, int concurrency, int maxStackCount, CancellationToken cancellationToken)
        {
            ...
            DnsPacket request = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, false, false, false, dnssecValidation, DnsResponseCode.NoError, [new DnsQuestionRecord(zoneCut, DnsResourceRecordType.NS, DnsClass.IN)], null, null, null, udpPayloadSize, dnssecValidation ? EDnsHeaderFlags.DNSSEC_OK : EDnsHeaderFlags.None);
            DnsPacket response;
        ...
        private static async Task<DnsPacket> ResolveQueryAsync(DnsQuestionRecord question, Func<DnsQuestionRecord, Task<DnsPacket>> resolveAsync)
        {
            DnsPacket response = await resolveAsync(question);
            if (response is null)
                return new DnsPacket(0, true, DnsOpCode.StandardQuery, false, false, true, true, false, false, DnsResponseCode.Refused, new DnsQuestionRecord[] { question });
            ...
            if (response.Answer.Count > 0)
            {
                ...
                if ((lastRR.Type != question.Type) && (lastRR.Type == DnsResourceRecordType.CNAME) && (question.Type != DnsResourceRecordType.ANY))
                {
                    ...
                    do
                    {
                        ...
                        {
                            lastResponse = new DnsPacket(0, true, DnsOpCode.StandardQuery, false, false, true, true, false, false, DnsResponseCode.NoError, new DnsQuestionRecord[] { question });
                            break;
                        }
                    ...
                    DnsPacket compositeResponse = new DnsPacket(0, true, DnsOpCode.StandardQuery, false, false, true, true, false, false, lastResponse.ResponseCode, new DnsQuestionRecord[] { question }, newAnswer, authority, lastResponse.Additional);
                                        ...
                                        else
                                        {
                                            ...
                                            switch (response.ResponseCode)
                                            {
                                                ...
                                                case DnsResponseCode.FormatError:
                                                    ...
                                                    {
                                                        server.Metadata.UpdateFailure(_timeout * _retries);
                                                        response.AddDnsClientExtendedError(EDnsExtendedDnsErrorCode.NoReachableAuthority, (response.Metadata is null ? "name server" : response.Metadata.NameServer.ToString()) + " returned RCODE=" + response.ResponseCode.ToString() + " for " + request.Question[0].ToString());
        ...
        private async Task<DnsPacket> InternalDnssecResolveAsync(DnsQuestionRecord question, CancellationToken cancellationToken = default)
        {
        ...
        private async Task<DnsPacket> InternalCachedResolveQueryAsync(DnsQuestionRecord question, CancellationToken cancellationToken)
        {
            return await ResolveQueryAsync(question, async delegate (DnsQuestionRecord q)
            {
                ...
                DnsPacket newRequest = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, _dnssecValidation, DnsResponseCode.NoError, new DnsQuestionRecord[] { q }, null, null, null, _udpPayloadSize, _dnssecValidation ? EDnsHeaderFlags.DNSSEC_OK : EDnsHeaderFlags.None, _advancedForwardingClientSubnet ? null : EDnsClientSubnetOptionData.GetEDnsClientSubnetOption(_eDnsClientSubnet));
                if (_advancedForwardingClientSubnet)
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        //cache as failure
                        DnsPacket failureResponse = new DnsPacket(0, true, DnsOpCode.StandardQuery, false, false, false, false, false, false, DnsResponseCode.ServerFailure, new DnsQuestionRecord[] { q });
        ...
        public Task<DnsPacket> TsigResolveAsync(DnsQuestionRecord question, TsigKey key, ushort fudge = 300, CancellationToken cancellationToken = default)
        {
            DnsPacket request = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, false, DnsResponseCode.NoError, new DnsQuestionRecord[] { question }, null, null, null, _udpPayloadSize, EDnsHeaderFlags.None, _advancedForwardingClientSubnet ? null : EDnsClientSubnetOptionData.GetEDnsClientSubnetOption(_eDnsClientSubnet));
            if (_advancedForwardingClientSubnet)
                request.SetShadowEDnsClientSubnetOption(_eDnsClientSubnet, true);
        ...
        public Task<DnsPacket> TsigResolveAsync(string domain, DnsResourceRecordType type, TsigKey key, ushort fudge = 300, CancellationToken cancellationToken = default)
        {
        ...
        public Task<DnsPacket> ResolveAsync(DnsQuestionRecord question, CancellationToken cancellationToken = default)
        {
        ...
        public Task<DnsPacket> ResolveAsync(string domain, DnsResourceRecordType type, CancellationToken cancellationToken = default)
        {
            if ((type == DnsResourceRecordType.PTR) && IPAddress.TryParse(domain, out IPAddress address))
        ...
        class ResolverData
        {
            ...
            public ResolverData(DnsQuestionRecord question, string zoneCut, EDnsHeaderFlags ednsFlags, IReadOnlyList<DnsResourceRecord> lastDSRecords, IList<NameServerAddress> nameServers, int nameServerIndex, int hopCount, DnsPacket lastResponse, Exception lastException)
            {
                Question = question;
                ZoneCut = zoneCut;
                EDnsFlags = ednsFlags;
