...
public sealed class DnsPacket
{
    ...
    private ushort _id;
    private int _size = -1;
    private byte _questionRecord;
    private DnsOpCode _opcode;
    private byte _isAuthoritativeAnswer;
    private byte _isTruncation;
    private byte _isRecursionDesired;
    private byte _isRecursionAvailable;
    private byte _Z;
    private byte _isAuthenticData;
    private byte _checkingDisabled;
    private DnsResponseCode _responseCode;

    IReadOnlyList<DnsPacketQuestion> _question;
    ...
    DnsPacket _nextDatagram; //used for TCP XFR multiple messages
    ...
    public DnsPacket(ushort ID, bool isResponse, DnsOpCode OPCODE, bool authoritativeAnswer, bool truncation, bool recursionDesired, bool recursionAvailable, bool authenticData, bool checkingDisabled, DnsResponseCode RCODE, IReadOnlyList<DnsPacketQuestion> question, IReadOnlyList<DnsResourceRecord> answer = null, IReadOnlyList<DnsResourceRecord> authority = null, IReadOnlyList<DnsResourceRecord> additional = null, ushort udpPayloadSize = ushort.MinValue, EDnsHeaderFlags ednsFlags = EDnsHeaderFlags.None, IReadOnlyList<EDnsOption> options = null)
    {
        _id = ID;
        ...
        if (authoritativeAnswer)
            _isAuthoritativeAnswer = 1;

        if (truncation)
            _isTruncation = 1;

        if (recursionDesired)
            _isRecursionDesired = 1;

        if (recursionAvailable)
            _isRecursionAvailable = 1;
        ...
        if (_question is null)
            _question = Array.Empty<DnsPacketQuestion>();
        ...
        if (_additional is null)
        {
            ...
            {
                _additional = new DnsResourceRecord[] { DnsPacketEdns.GetOPTFor(udpPayloadSize, RCODE, 0, ednsFlags, options) };
                _edns = new DnsPacketEdns(udpPayloadSize, RCODE, 0, ednsFlags, options);
            }
        }
        ...
        else
        {
            if (udpPayloadSize < 512)
            {
                _edns = DnsPacketEdns.ReadOPTFrom(_additional, RCODE);
            }
            else
            {
                ...
                newAdditional[_additional.Count] = DnsPacketEdns.GetOPTFor(udpPayloadSize, RCODE, 0, ednsFlags, options);
    ...
    public DnsPacketMeta Meta => _meta;

    public DnsPacketEdns EDNS
    { get { return _edns; } }
    ...
    /// 
    /// </summary>
    public DnsOpCode OpCode => _opcode;

    /// <summary>
    /// 
    /// </summary>
    public bool IsResponse { get { return _questionRecord == 1; } }

    /// <summary>
    /// 
    /// </summary>
    public bool IsAuthoritativeAnswer => _isAuthoritativeAnswer == 1;

    /// <summary>
    /// 
    /// </summary>
    public bool IsTruncation => _isTruncation == 1;

    /// <summary>
    /// 
    /// </summary>
    public bool IsRecursionDesired => _isRecursionDesired == 1;

    /// <summary>
    /// 
    /// </summary>
    public bool IsRecursionAvailable =>  _isRecursionAvailable == 1; 
    ...
    /// 
    /// </summary>
    public bool IsAuthenticData => _isAuthenticData == 1;

    /// <summary>
    /// 
    /// </summary>
    public bool IsCheckingDisabled { get { return _checkingDisabled == 1; } }
    ...
    public IReadOnlyList<DnsPacketQuestion> Question
    ...
    { get { return (_additional.Count > 0) && (_additional[_additional.Count - 1].Type == DnsRecordType.TSIG); } }
    ...
    public string TsigKeyName
    {
        get
        {
            if ((_additional.Count > 0) && (_additional[_additional.Count - 1].Type == DnsRecordType.TSIG))
    ...
    { get { return (_question.Count > 0) && ((_question[0].Type == DnsRecordType.IXFR) || (_question[0].Type == DnsRecordType.AXFR)); } }

    public Exception ParsingException
    { get { return _parsingException; } }

    public DnsPacket NextDatagram
    {
    ...
    public static DnsPacket ReadFrom(Stream s)
    {
        ...
        datagram._questionRecord = Convert.ToByte((lB & 0x80) >> 7);
        datagram._opcode = (DnsOpCode)Convert.ToByte((lB & 0x78) >> 3);
        datagram._isAuthoritativeAnswer = Convert.ToByte((lB & 0x4) >> 2);
        datagram._isTruncation = Convert.ToByte((lB & 0x2) >> 1);
        datagram._isRecursionDesired = Convert.ToByte(lB & 0x1);

        int rB = s.ReadByte();
        if (rB < 0)
            throw new EndOfStreamException();

        datagram._isRecursionAvailable = Convert.ToByte((rB & 0x80) >> 7);
        datagram._Z = Convert.ToByte((rB & 0x40) >> 6);
        datagram._isAuthenticData = Convert.ToByte((rB & 0x20) >> 5);
        datagram._checkingDisabled = Convert.ToByte((rB & 0x10) >> 4);
        datagram._responseCode = (DnsResponseCode)(rB & 0xf);
        ...
        try
        {
            if (QDCOUNT == 0)
            {
                datagram._question = Array.Empty<DnsPacketQuestion>();
            }
            else
            {
                DnsPacketQuestion[] question = new DnsPacketQuestion[QDCOUNT];

                for (int i = 0; i < question.Length; i++)
                    question[i] = new DnsPacketQuestion(s);
        ...
        datagram._edns = DnsPacketEdns.ReadOPTFrom(datagram._additional, datagram._responseCode);

        return datagram;
    }

    public static async Task<DnsPacket> ReadFromTcpAsync(Stream stream, int bufferSize = 4096, CancellationToken cancellationToken = default)
    {
    ...
    public static async Task<DnsPacket> ReadFromTcpAsync(Stream stream, MemoryStream sharedBuffer, CancellationToken cancellationToken = default)
    {
    ...
    internal DnsPacket CloneHeadersAndQuestions()
    {
        DnsPacketQuestion[] clonedQuestion = new DnsPacketQuestion[_question.Count];

        for (int i = 0; i < _question.Count; i++)
            clonedQuestion[i] = _question[i].Clone();

        DnsPacket datagram = new DnsPacket(_id, _questionRecord == 1, _opcode, _isAuthoritativeAnswer == 1, _isTruncation == 1, _isRecursionDesired == 1, _isRecursionAvailable == 1, _isAuthenticData == 1, _checkingDisabled == 1, _responseCode, clonedQuestion, _answer, _authority, _additional);

        datagram._meta = _meta;
    ...
    internal void AddDnsClientExtendedErrorFrom(DnsPacket datagram)
    {
    ...
    public DnsPacket Clone(IReadOnlyList<DnsResourceRecord> answer = null, IReadOnlyList<DnsResourceRecord> authority = null, IReadOnlyList<DnsResourceRecord> additional = null)
    {
        if (answer is null)
            answer = _answer;
        ...
        DnsPacket datagram = new DnsPacket(_id, _questionRecord == 1, _opcode, _isAuthoritativeAnswer == 1, _isTruncation == 1, _isRecursionDesired == 1, _isRecursionAvailable == 1, _isAuthenticData == 1, _checkingDisabled == 1, _responseCode, _question, answer, authority, additional);
        ...
        if (additional is null)
            ...
            datagram._edns = DnsPacketEdns.ReadOPTFrom(additional, _responseCode);
    ...
    public DnsPacket CloneWithoutEDns()
    {
        if (_edns is null)
            return this;
    ...
    public DnsPacket CloneWithoutEDnsClientSubnet()
    {
        ...
        newAdditional.Add(DnsPacketEdns.GetOPTFor(_edns.UdpPayloadSize, _edns.ExtendedRCODE, _edns.Version, _edns.Flags, options));

        return Clone(null, null, newAdditional);
    }

    public DnsPacket CloneWithoutGlueRecords()
    {
    ...
    {
        if ((_isRecursionDesired == 0) || (_isAuthoritativeAnswer == 1))
            return false;
        ...
        if ((_responseCode == DnsResponseCode.NoError) && (_question.Count > 0) && (_answer.Count > 0))
        {
                ...
                case DnsRecordType.A:
                    {
                        ...
                        return (lastRR.Type == DnsRecordType.A) && (lastRR.RDATA is DnsARecordData aRecord) && aRecord.Address.Equals(IPAddress.Any);
                    }

                case DnsRecordType.AAAA:
                    {
                        DnsResourceRecord lastRR = _answer[_answer.Count - 1];
                        return (lastRR.Type == DnsRecordType.AAAA) && (lastRR.RDATA is DnsAAAARecordData aaaaRecord) && aaaaRecord.Address.Equals(IPAddress.IPv6Any);
                    }
            }
        }
    ...
    {
        return (_responseCode == DnsResponseCode.NoError) && (_answer.Count == 0) && (FindFirstAuthorityType() == DnsRecordType.NS);
    }

    public void SetMetadata(DnsNsAddress server, double rtt = 0.0)
    {
        if (_meta is not null)
            throw new InvalidOperationException("Cannot overwrite existing Metadata.");

        _meta = new DnsPacketMeta(server, _size, rtt);
    }
    ...
    public DnsResourceRecord GetLastAnswerRecord()
    {
        ...
        DnsPacketQuestion question = _question[0];
    ...
    public bool IsFirstAuthoritySOA()
    {
        ...
        return (firstAuthority is not null) && (firstAuthority.Type == DnsRecordType.SOA);
    }

    public DnsRecordType FindFirstAuthorityType()
    {
        DnsResourceRecord firstAuthority = FindFirstAuthorityRecord();
    ...
    public void WriteTo(Stream s)
    {
        ...
        s.WriteByte(Convert.ToByte((_questionRecord << 7) | ((byte)_opcode << 3) | (_isAuthoritativeAnswer << 2) | (_isTruncation << 1) | _isRecursionDesired));
        s.WriteByte(Convert.ToByte((_isRecursionAvailable << 7) | (_Z << 6) | (_isAuthenticData << 5) | (_checkingDisabled << 4) | (byte)((int)_responseCode & 0xf)));
        ...
        foreach (DnsPacketQuestion question in _question)
    ...
    public async Task WriteToTcpAsync(Stream s, MemoryStream sharedBuffer, CancellationToken cancellationToken = default)
    {
        ...
        do
        {
            ...
            if (datagramLength > ushort.MaxValue)
            {
                ...
                if (_edns is not null)
                    additional = new DnsResourceRecord[] { DnsPacketEdns.GetOPTFor(_edns.UdpPayloadSize, _edns.ExtendedRCODE, _edns.Version, _edns.Flags, _edns.Options) };

                DnsPacket truncted = new DnsPacket(_id, _questionRecord == 1, _opcode, _isAuthoritativeAnswer == 1, true, _isRecursionDesired == 1, _isRecursionAvailable == 1, _isAuthenticData == 1, _checkingDisabled == 1, _responseCode, _question, Array.Empty<DnsResourceRecord>(), Array.Empty<DnsResourceRecord>(), additional);
    ...
    public void SerializeTo(Utf8JsonWriter jsonWriter)
    {
        ...
        jsonWriter.WriteBoolean("AuthoritativeAnswer", IsAuthoritativeAnswer);
        jsonWriter.WriteBoolean("Truncation", IsTruncation);
        jsonWriter.WriteBoolean("RecursionDesired", IsRecursionDesired);
        jsonWriter.WriteBoolean("RecursionAvailable", IsRecursionAvailable);
        jsonWriter.WriteNumber("Z", _Z);
        jsonWriter.WriteBoolean("AuthenticData", IsAuthenticData);
        jsonWriter.WriteBoolean("CheckingDisabled", IsCheckingDisabled);
        jsonWriter.WriteString("RCODE", ResponseCode.ToString());
    ...
    public DnsPacket Split()
    {
        ...
        do
        {
            ...
            IReadOnlyList<DnsPacketQuestion> question = null;
            ...
            DnsPacket datagram = new DnsPacket(_id, _questionRecord == 1, _opcode, _isAuthoritativeAnswer == 1, _isTruncation == 1, _isRecursionDesired == 1, _isRecursionAvailable == 1, _isAuthenticData == 1, _checkingDisabled == 1, _responseCode, question, answer, authority, additional);
    ...
    public DnsPacket Join()
    {
        ...
        DnsPacket joinedDatagram = new DnsPacket(_id, _questionRecord == 1, _opcode, _isAuthoritativeAnswer == 1, _isTruncation == 1, _isRecursionDesired == 1, _isRecursionAvailable == 1, _isAuthenticData == 1, _checkingDisabled == 1, _responseCode, _question, answer, _authority, _additional);
        joinedDatagram._size = size;
        joinedDatagram.SetMetadata(_meta.NameServer, _meta.RoundTripTime);
        joinedDatagram.Tag = Tag;

        return joinedDatagram;
    }

    public DnsPacket SignRequest(TsigKey key, ushort fudge)
    {
        if (IsResponse)
        ...
        DnsResourceRecord tsigRecord = new DnsResourceRecord(key.KeyName, DnsRecordType.TSIG, DnsClass.Any, 0, new DnsTSIGRecordData(key.AlgorithmName, timeSigned, fudge, mac, _id, error, otherData));
        ...
        DnsPacket signedDatagram = new DnsPacket(_id, _questionRecord == 1, _opcode, _isAuthoritativeAnswer == 1, _isTruncation == 1, _isRecursionDesired == 1, _isRecursionAvailable == 1, _isAuthenticData == 1, _checkingDisabled == 1, _responseCode, _question, _answer, _authority, additional);
        signedDatagram._meta = _meta;
        signedDatagram.Tag = Tag;

        return signedDatagram;
    }

    public bool VerifySignedRequest(IReadOnlyDictionary<string, TsigKey> keys, out DnsPacket unsignedRequest, out DnsPacket errorResponse)
    {
        if (IsResponse)
        ...
        if ((expectedMacSize == ushort.MinValue) || (keys is null) || !keys.TryGetValue(tsigRecord.Name.ToLowerInvariant(), out TsigKey key) || !key.AlgorithmName.Equals(tsig.AlgorithmName, StringComparison.OrdinalIgnoreCase))
        {
            ...
            DnsResourceRecord errorTsigRecord = new DnsResourceRecord(tsigRecord.Name, DnsRecordType.TSIG, DnsClass.Any, 0, errorTsig);
            errorResponse = new DnsPacket(_id, true, _opcode, _isAuthoritativeAnswer == 1, _isTruncation == 1, _isRecursionDesired == 1, _isRecursionAvailable == 1, _isAuthenticData == 1, _checkingDisabled == 1, DnsResponseCode.NotAuth, _question, null, null, new DnsResourceRecord[] { errorTsigRecord });
        ...
        if ((tsig.MAC.Length > expectedMacSize) || (tsig.MAC.Length < Math.Max(10, expectedMacSize / 2)))
        {
            ...
            //unsigned error response
            errorResponse = new DnsPacket(_id, true, _opcode, _isAuthoritativeAnswer == 1, _isTruncation == 1, _isRecursionDesired == 1, _isRecursionAvailable == 1, _isAuthenticData == 1, _checkingDisabled == 1, DnsResponseCode.FormatError, _question);
        ...
        if (!BinaryNumber.Equals(computedMac, tsig.MAC))
        {
            ...
            DnsResourceRecord errorTsigRecord = new DnsResourceRecord(key.KeyName, DnsRecordType.TSIG, DnsClass.Any, 0, errorTsig);
            errorResponse = new DnsPacket(_id, true, _opcode, _isAuthoritativeAnswer == 1, _isTruncation == 1, _isRecursionDesired == 1, _isRecursionAvailable == 1, _isAuthenticData == 1, _checkingDisabled == 1, DnsResponseCode.NotAuth, _question, null, null, new DnsResourceRecord[] { errorTsigRecord });
        ...
        if ((utcNow < startTime) || (utcNow > endTime))
        {
            ...
            errorResponse = new DnsPacket(_id, true, _opcode, _isAuthoritativeAnswer == 1, _isTruncation == 1, _isRecursionDesired == 1, _isRecursionAvailable == 1, _isAuthenticData == 1, _checkingDisabled == 1, DnsResponseCode.NotAuth, _question);
        ...
        unsignedRequest = new DnsPacket(tsig.OriginalID, _questionRecord == 1, _opcode, _isAuthoritativeAnswer == 1, _isTruncation == 1, _isRecursionDesired == 1, _isRecursionAvailable == 1, _isAuthenticData == 1, _checkingDisabled == 1, _responseCode, _question, _answer, _authority, additional);
        unsignedRequest._meta = _meta;
    ...
    public DnsPacket SignResponse(DnsPacket signedRequest, IReadOnlyDictionary<string, TsigKey> keys, DnsTsigError error = DnsTsigError.NoError)
    {
        if (!IsResponse)
        ...
        DnsResourceRecord tsigRecord = new DnsResourceRecord(key.KeyName, DnsRecordType.TSIG, DnsClass.Any, 0, tsig);
        ...
        DnsPacket signedDatagram = new DnsPacket(signedRequest._id, _questionRecord == 1, _opcode, _isAuthoritativeAnswer == 1, _isTruncation == 1, _isRecursionDesired == 1, _isRecursionAvailable == 1, _isAuthenticData == 1, _checkingDisabled == 1, _responseCode, _question, _answer, _authority, additional);
        signedDatagram._meta = _meta;
        signedDatagram.Tag = Tag;
        ...
        if (_nextDatagram is not null)
        {
            DnsPacket current = _nextDatagram;
            DnsTSIGRecordData priorTsig = tsig;
            DnsPacket lastSigned = signedDatagram;

            while (current is not null)
            {
                byte[] currentMac = ComputeTsigNextResponseMac(priorTsig, new DnsPacket[] { current }, timeSigned, requestTsig.Fudge, key.SharedSecret, 0);
                ...
                DnsResourceRecord currentTsigRecord = new DnsResourceRecord(key.KeyName, DnsRecordType.TSIG, DnsClass.Any, 0, currentTsig);
                ...
                DnsPacket signedCurrent = new DnsPacket(signedRequest._id, _questionRecord == 1, _opcode, _isAuthoritativeAnswer == 1, _isTruncation == 1, _isRecursionDesired == 1, _isRecursionAvailable == 1, _isAuthenticData == 1, _checkingDisabled == 1, _responseCode, current._question, current._answer, current._authority, currentAdditional);
    ...
    public bool VerifySignedResponse(DnsPacket signedRequest, TsigKey key, out DnsPacket unsignedResponse, out bool requestFailed, out DnsResponseCode rCode, out DnsTsigError error)
    {
        if (!IsResponse)
        ...
        unsignedResponse = new DnsPacket(tsig.OriginalID, _questionRecord == 1, _opcode, _isAuthoritativeAnswer == 1, _isTruncation == 1, _isRecursionDesired == 1, _isRecursionAvailable == 1, _isAuthenticData == 1, _checkingDisabled == 1, _responseCode, _question, _answer, _authority, additional);
        unsignedResponse._meta = _meta;
        unsignedResponse.Tag = Tag;

        //verify next messages
        if (_nextDatagram is not null)
        {
            DnsPacket unsignedPrior = unsignedResponse;
            DnsPacket current = _nextDatagram;
            DnsTSIGRecordData priorTsig = tsig;
            List<DnsPacket> dnsMessages = new List<DnsPacket>();
            ...
            while (current is not null)
            {
                ...
                DnsPacket unsignedCurrentResponse = new DnsPacket(currentTsig.OriginalID, _questionRecord == 1, _opcode, _isAuthoritativeAnswer == 1, _isTruncation == 1, _isRecursionDesired == 1, _isRecursionAvailable == 1, _isAuthenticData == 1, _checkingDisabled == 1, _responseCode, current._question, current._answer, current._authority, currentAdditional);
    ...
    private static byte[] ComputeTsigNextResponseMac(DnsTSIGRecordData priorTsig, IReadOnlyList<DnsPacket> dnsMessages, ulong timeSigned, ushort fudge, string sharedSecret, int truncationLength)
    {
        using (MemoryStream mS = new MemoryStream(256))
        {
            ...
            foreach (DnsPacket dnsMessage in dnsMessages)
            {
