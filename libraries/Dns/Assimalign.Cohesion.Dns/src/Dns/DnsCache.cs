...
namespace Assimalign.Cohesion.Dns
{
    ...
    public class DnsCache : IDnsCache
    {
        ...
        protected virtual void CacheRecords(IReadOnlyList<DnsResourceRecord> resourceRecords, NetworkAddress eDnsClientSubnet, DnsPacketMeta responseMetadata)
        {
            if (resourceRecords.Count == 1)
            {
                DnsResourceRecord resourceRecord = resourceRecords[0];

                if (resourceRecord.Type == DnsRecordType.DNAME)
            ...
            {
                Dictionary<string, Dictionary<DnsRecordType, List<DnsResourceRecord>>> cacheEntries = DnsResourceRecord.GroupRecords(resourceRecords);

                //add grouped entries into cache
                foreach (KeyValuePair<string, Dictionary<DnsRecordType, List<DnsResourceRecord>>> cacheEntry in cacheEntries)
                {
                    bool foundDNAME = false;

                    foreach (KeyValuePair<DnsRecordType, List<DnsResourceRecord>> cacheTypeEntry in cacheEntry.Value)
                    {
                        if (cacheTypeEntry.Key == DnsRecordType.DNAME)
                        {
                            foundDNAME = true;
                            break;
                        }
        ...
        private void InternalCacheRecords(IReadOnlyList<DnsResourceRecord> resourceRecords, NetworkAddress eDnsClientSubnet, DnsPacketMeta responseMetadata)
        {
        ...
        private IReadOnlyList<DnsResourceRecord> GetClosestNameServers(string domain, bool dnssecOk)
        {
            ...
            do
            {
                ...
                {
                    IReadOnlyList<DnsResourceRecord> records = entry.QueryRecords(DnsRecordType.NS, true);
                    if ((records.Count > 0) && (records[0].Type == DnsRecordType.NS))
                    {
                        if (dnssecOk)
                        {
        ...
        {
            IReadOnlyList<DnsResourceRecord> records = entry.QueryRecords(DnsRecordType.DS, true);
            if ((records.Count > 0) && (records[0].Type == DnsRecordType.DS))
            {
        ...
        private void ResolveCNAME(DnsPacketQuestion question, DnsResourceRecord lastCNAME, List<DnsResourceRecord> answerRecords)
        {
            int queryCount = 0;
        ...
        private void ResolveAdditionalRecords(DnsResourceRecord refRecord, string domain, List<DnsResourceRecord> additionalRecords)
        {
            ...
            {
                IReadOnlyList<DnsResourceRecord> glueAs = entry.QueryRecords(DnsRecordType.A, true);
                if ((glueAs.Count > 0) && (glueAs[0].Type == DnsRecordType.A))
                    additionalRecords.AddRange(glueAs);

                IReadOnlyList<DnsResourceRecord> glueAAAAs = entry.QueryRecords(DnsRecordType.AAAA, true);
                if ((glueAAAAs.Count > 0) && (glueAAAAs[0].Type == DnsRecordType.AAAA))
        ...
        public virtual Task<DnsPacket> QueryClosestDelegationAsync(DnsPacket request)
        {
            ...
            if (closestAuthority is null)
                return Task.FromResult<DnsPacket>(null);

            IReadOnlyList<DnsResourceRecord> additional = GetAdditionalRecords(closestAuthority);

            return Task.FromResult(new DnsPacket(request.Id, true, DnsOpCode.StandardQuery, false, false, request.IsRecursionDesired, true, false, false, DnsResponseCode.NoError, request.Question, null, closestAuthority, additional));
        }

        public virtual Task<DnsPacket> QueryAsync(DnsPacket request, bool serveStale = false, bool findClosestNameServers = false, bool resetExpiry = false)
        {
            ...
            if (_cache.TryGetValue(question.Name.ToLowerInvariant(), out DnsCacheEntry entry))
            {
                ...
                if (answers.Count > 0)
                {
                    ...
                    if (firstRR.RDATA is DnsSpecialCacheRecordData dnsSpecialCacheRecord)
                    {
                        ...
                        {
                            if (request.IsCheckingDisabled)
                                return Task.FromResult(new DnsPacket(request.Id, true, DnsOpCode.StandardQuery, false, false, request.IsRecursionDesired, true, false, true, dnsSpecialCacheRecord.OriginalRCODE, request.Question, dnsSpecialCacheRecord.OriginalNoDnssecAnswer, dnsSpecialCacheRecord.OriginalNoDnssecAuthority, dnsSpecialCacheRecord.OriginalAdditional, request.EDNS is null ? ushort.MinValue : request.EDNS.UdpPayloadSize, EDnsHeaderFlags.None, dnsSpecialCacheRecord.EDnsOptions));
                            else
                                return Task.FromResult(new DnsPacket(request.Id, true, DnsOpCode.StandardQuery, false, false, request.IsRecursionDesired, true, false, false, dnsSpecialCacheRecord.RCODE, request.Question, dnsSpecialCacheRecord.NoDnssecAnswer, dnsSpecialCacheRecord.NoDnssecAuthority, null, request.EDNS is null ? ushort.MinValue : request.EDNS.UdpPayloadSize, EDnsHeaderFlags.None, dnsSpecialCacheRecord.EDnsOptions));
                        }
                    }
                    ...
                    return Task.FromResult(new DnsPacket(request.Id, true, DnsOpCode.StandardQuery, false, false, request.IsRecursionDesired, true, answers[0].DnssecStatus == DnssecStatus.Secure, request.IsCheckingDisabled, DnsResponseCode.NoError, request.Question, answers, authority, additional));
                }
            }
            ...
            if (findClosestNameServers)
            {
                ...
                if (question.Type == DnsRecordType.DS)
                {
                    //find parent zone NS
                    domain = GetParentZone(question.Name);
                    if (domain is null)
                        return Task.FromResult<DnsPacket>(null); //dont find NS for root
                }
                else
                {
                ...
                if (closestAuthority is not null)
                {
                    ...
                    return Task.FromResult(new DnsPacket(request.Id, true, DnsOpCode.StandardQuery, false, false, request.IsRecursionDesired, true, closestAuthority[0].DnssecStatus == DnssecStatus.Secure, request.IsCheckingDisabled, DnsResponseCode.NoError, request.Question, null, closestAuthority, additionalRecords));
                }
            }
        ...
        {
            if (!response.IsResponse || response.IsTruncation || (response.Question.Count == 0))
            ...
            if (isDnssecBadCache)
            {
                ...
                foreach (DnsPacketQuestion question in response.Question)
                {
                    ...
                    InternalCacheRecords(new DnsResourceRecord[] { record }, eDnsClientSubnet, response.Meta);
                }
            ...
            if (response.IsBlockedResponse())
            {
                ...
                foreach (DnsPacketQuestion question in response.Question)
                {
                    ...
                    InternalCacheRecords([record], eDnsClientSubnet, response.Meta);
                }
                ...
                foreach (DnsResourceRecord answer in response.Answer)
                {
                    if (answer.Name.Equals(qName, StringComparison.OrdinalIgnoreCase))
                    {
                        switch (answer.Type)
                        {
                            ...
                            case DnsRecordType.NS:
                                if ((question.Type == DnsRecordType.NS) || (question.Type == DnsRecordType.ANY))
                                {
                                    cachableRecords.Add(answer);
                            ...
                            case DnsRecordType.MX:
                                if ((question.Type == DnsRecordType.MX) || (question.Type == DnsRecordType.ANY))
                                {
                                    cachableRecords.Add(answer);
                            ...
                            case DnsRecordType.SRV:
                                if ((question.Type == DnsRecordType.SRV) || (question.Type == DnsRecordType.ANY))
                                {
                                    cachableRecords.Add(answer);
                            ...
                            case DnsRecordType.HTTPS:
                                if ((question.Type == DnsRecordType.SVCB) || (question.Type == DnsRecordType.HTTPS) || (question.Type == DnsRecordType.ANY))
                                {
                                    cachableRecords.Add(answer);
                            ...
                            case DnsRecordType.RRSIG:
                                if ((question.Type == DnsRecordType.RRSIG) || (question.Type == DnsRecordType.ANY))
                                    cachableRecords.Add(answer);
                ...
                switch (firstAuthority.Type)
                {
                    case DnsRecordType.SOA:
                        ...
                        else if (zoneCut is null)
                        {
                            ...
                            if (lastAnswer.Type == DnsRecordType.CNAME)
                            {
                                foreach (DnsPacketQuestion question in response.Question)
                                {
                    ...
                    case DnsRecordType.NS:
                        if (response.Answer.Count == 0)
                        {
                            ...
                            foreach (DnsPacketQuestion question in response.Question)
                            {
                                foreach (DnsResourceRecord authority in response.Authority)
                                {
                                    if (authority.Type == DnsRecordType.NS)
                                    {
            ...
            if (cachableRecords.Count > 0)
                InternalCacheRecords(cachableRecords, eDnsClientSubnet, response.Meta);
        }
        ...
        public class DnsSpecialCacheRecordData : DnsResourceRecordData
        {
            ...
            public DnsSpecialCacheRecordData(DnsSpecialCacheRecordType type, DnsPacket response)
                : this(type, response.ResponseCode, response.Question, response.Answer, response.Authority, response.Additional, response.EDNS, response.DnsClientExtendedErrors)
            { }

            public DnsSpecialCacheRecordData(DnsSpecialCacheRecordType type, DnsResponseCode rcode, IReadOnlyList<DnsPacketQuestion> question, IReadOnlyList<DnsResourceRecord> answer, IReadOnlyList<DnsResourceRecord> authority, IReadOnlyList<DnsResourceRecord> additional, DnsPacketEdns edns, IReadOnlyList<EDnsExtendedDnsErrorOptionData> dnsClientExtendedErrors)
            {
                _type = type;
                _rcode = rcode;
                _answer = answer;
                ...
                if ((_additional.Count == 1) && (_additional[0].Type == DnsRecordType.OPT))
                {
                ...
                else if (_additional.Count > 0)
                {
                    ...
                    if (foundOpt)
                    {
                        ...
                        {
                            if (record2.Type == DnsRecordType.OPT)
                                continue;
        ...
        class DnsCacheEntry
        {
            ...
            readonly ConcurrentDictionary<DnsRecordType, IReadOnlyList<DnsResourceRecord>> _entries;
            ...
            {
                _entries = new ConcurrentDictionary<DnsRecordType, IReadOnlyList<DnsResourceRecord>>(1, capacity);
            }
            ...
            public static DnsCacheEntry GetRootCacheEntry()
            {
                ...
                foreach (DnsNsAddress ipv4Hint in DnsClient.IPv4RootHints)
                {
                    string nsDomain = ipv4Hint.Host;

                    DnsResourceRecord nsRecord = new DnsResourceRecord("", DnsRecordType.NS, DnsClass.Internet, 518400, new DnsNSRecordData(nsDomain));

                    DnsResourceRecord ipv4Glue = new DnsResourceRecord(nsDomain, DnsRecordType.A, DnsClass.Internet, 518400, new DnsARecordData(ipv4Hint.IPEndPoint.Address));
                    AddGlueRecordTo(nsRecord, ipv4Glue);

                    foreach (DnsNsAddress ipv6Hint in DnsClient.IPv6RootHints)
                    {
                        if (ipv6Hint.Host.Equals(nsDomain, StringComparison.OrdinalIgnoreCase))
                        {
                            DnsResourceRecord ipv6Glue = new DnsResourceRecord(nsDomain, DnsRecordType.AAAA, DnsClass.Internet, 518400, new DnsAAAARecordData(ipv6Hint.IPEndPoint.Address));
                ...
                entry._entries[DnsRecordType.NS] = rrset;
            ...
            private static IReadOnlyList<DnsResourceRecord> ValidateRRSet(DnsRecordType type, IReadOnlyList<DnsResourceRecord> records, bool skipSpecialCacheRecord)
            {
                foreach (DnsResourceRecord record in records)
                {
                    if (record.IsStale)
            ...
            public void SetRecords(DnsRecordType type, IReadOnlyList<DnsResourceRecord> records)
            {
                if (records.Count == 0)
                    return;
                ...
                else if ((type == DnsRecordType.NS) && (records[0].RDATA is DnsNSRecordData ns) && !ns.IsParentSideTtlSet)
                {
                    //for ns revalidation
                    if (_entries.TryGetValue(DnsRecordType.NS, out IReadOnlyList<DnsResourceRecord> existingNSRecords))
                    {
            ...
            public IReadOnlyList<DnsResourceRecord> QueryRecords(DnsRecordType type, bool skipSpecialCacheRecord)
            {
                switch (type)
                {
                    case DnsRecordType.DS:
                    ...
                    case DnsRecordType.DNSKEY:
                        {
                            ...
                            if (_entries.TryGetValue(DnsRecordType.CNAME, out IReadOnlyList<DnsResourceRecord> existingCNAMERecords))
                            {
                                IReadOnlyList<DnsResourceRecord> rrset = ValidateRRSet(type, existingCNAMERecords, skipSpecialCacheRecord);
                                if (rrset.Count > 0)
                                {
                                    if ((type == DnsRecordType.CNAME) || (rrset[0].RDATA is DnsCNAMERecordData))
                                        return rrset;
                                }
                            }
                        }
                    ...
                    case DnsRecordType.ANY:
                        ...
                        foreach (KeyValuePair<DnsRecordType, IReadOnlyList<DnsResourceRecord>> entry in _entries)
                        {
                            if (entry.Key == DnsRecordType.DS)
                                continue;
