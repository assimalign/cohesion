...
#pragma warning disable CA1416 // Validate platform compatibility
    ...
    public class QuicClientConnection : DnsClientConnection
    {
        ...
        public QuicClientConnection(DnsNsAddress server, NetProxy proxy)
            : base(server, proxy)
        {
        ...
        private static async Task<DnsPacket> QuicQueryAsync(DnsPacket request, QuicConnection quicConnection, CancellationToken cancellationToken)
        {
            await using (QuicStream quicStream = await quicConnection.OpenOutboundStreamAsync(QuicStreamType.Bidirectional, cancellationToken))
            {
                ...
                using (MemoryStream mS = new MemoryStream(64))
                {
                    ...
                    mS.Position = 0;
                    DnsPacket.WriteUInt16NetworkOrder(Convert.ToUInt16(datagramLength), mS);
                    mS.Position = 0;
                ...
                if ((request.Question.Count > 0) && (request.Question[0].Type == DnsRecordType.AXFR))
                {
                    //read zone transfer response
                    DnsPacket firstResponse = null;
                    ...
                    while (true)
                    {
                        DnsPacket response = await DnsPacket.ReadFromTcpAsync(quicStream, sharedBuffer, cancellationToken);
                        ...
                        if ((response.Answer.Count == 0) || ((response.Answer[response.Answer.Count - 1].Type == DnsRecordType.SOA) && ((response.Answer.Count > 1) || !isFirstResponse)))
                            break;
                    }
                ...
                else
                {
                    ...
                    return await DnsPacket.ReadFromTcpAsync(quicStream, 512, cancellationToken);
                }
            }
        }
