...
namespace Assimalign.Cohesion.Dns
{
    ...
    public class DnsClient : IDnsClient
    {
        ...
        static IReadOnlyList<DnsNsAddress> IPv4_ROOT_HINTS;
        static IReadOnlyList<DnsNsAddress> IPv6_ROOT_HINTS;
        ...
        readonly IReadOnlyList<DnsNsAddress> _servers;

        IDnsCache _cache;
        NetProxy _proxy;
        bool _preferIPv6;
        ushort _udpPayloadSize = DnsPacket.EDNS_DEFAULT_UDP_PAYLOAD_SIZE;
        bool _randomizeName;
        ...
        static DnsClient()
        {
            ...
            IPv4_ROOT_HINTS =
            [
                new DnsNsAddress("a.root-servers.net", IPAddress.Parse("198.41.0.4")), //VeriSign, Inc.
                new DnsNsAddress("b.root-servers.net", IPAddress.Parse("170.247.170.2")), //University of Southern California (ISI)
                new DnsNsAddress("c.root-servers.net", IPAddress.Parse("192.33.4.12")), //Cogent Communications
                new DnsNsAddress("d.root-servers.net", IPAddress.Parse("199.7.91.13")), //University of Maryland
                new DnsNsAddress("e.root-servers.net", IPAddress.Parse("192.203.230.10")), //NASA (Ames Research Center)
                new DnsNsAddress("f.root-servers.net", IPAddress.Parse("192.5.5.241")), //Internet Systems Consortium, Inc.
                new DnsNsAddress("g.root-servers.net", IPAddress.Parse("192.112.36.4")), //US Department of Defense (NIC)
                new DnsNsAddress("h.root-servers.net", IPAddress.Parse("198.97.190.53")), //US Army (Research Lab)
                new DnsNsAddress("i.root-servers.net", IPAddress.Parse("192.36.148.17")), //Netnod
                new DnsNsAddress("j.root-servers.net", IPAddress.Parse("192.58.128.30")), //VeriSign, Inc.
                new DnsNsAddress("k.root-servers.net", IPAddress.Parse("193.0.14.129")), //RIPE NCC
                new DnsNsAddress("l.root-servers.net", IPAddress.Parse("199.7.83.42")), //ICANN
                new DnsNsAddress("m.root-servers.net", IPAddress.Parse("202.12.27.33")) //WIDE Project
            ];

            IPv6_ROOT_HINTS =
            [
                new DnsNsAddress("a.root-servers.net", IPAddress.Parse("2001:503:ba3e::2:30")), //VeriSign, Inc.
                new DnsNsAddress("b.root-servers.net", IPAddress.Parse("2801:1b8:10::b")), //University of Southern California (ISI)
                new DnsNsAddress("c.root-servers.net", IPAddress.Parse("2001:500:2::c")), //Cogent Communications
                new DnsNsAddress("d.root-servers.net", IPAddress.Parse("2001:500:2d::d")), //University of Maryland
                new DnsNsAddress("e.root-servers.net", IPAddress.Parse("2001:500:a8::e")), //NASA (Ames Research Center)
                new DnsNsAddress("f.root-servers.net", IPAddress.Parse("2001:500:2f::f")), //Internet Systems Consortium, Inc.
                new DnsNsAddress("g.root-servers.net", IPAddress.Parse("2001:500:12::d0d")), //US Department of Defense (NIC)
                new DnsNsAddress("h.root-servers.net", IPAddress.Parse("2001:500:1::53")), //US Army (Research Lab)
                new DnsNsAddress("i.root-servers.net", IPAddress.Parse("2001:7fe::53")), //Netnod
                new DnsNsAddress("j.root-servers.net", IPAddress.Parse("2001:503:c27::2:30")), //VeriSign, Inc.
                new DnsNsAddress("k.root-servers.net", IPAddress.Parse("2001:7fd::1")), //RIPE NCC
                new DnsNsAddress("l.root-servers.net", IPAddress.Parse("2001:500:9f::42")), //ICANN
                new DnsNsAddress("m.root-servers.net", IPAddress.Parse("2001:dc3::35")) //WIDE Project
            ];

            //set default root trust anchors
            ROOT_TRUST_ANCHORS =
            [
                new DnsResourceRecord("", DnsRecordType.DS, DnsClass.Internet, 0, new DnsDSRecordData(20326, DnssecAlgorithm.RSASHA256, DnssecDigestType.SHA256, Convert.FromHexString("E06D44B80B8F1D39A95C0B0D7C65D08458E880409BBC683457104237C7F8EC8D"))),
                new DnsResourceRecord("", DnsRecordType.DS, DnsClass.Internet, 0, new DnsDSRecordData(38696, DnssecAlgorithm.RSASHA256, DnssecDigestType.SHA256, Convert.FromHexString("683D2D0ACB8C9B712A1948B27F741219298D0A450D612C483AF444A4C0FB2B16")))
            ];
        ...
        {
            _servers = [new DnsNsAddress(dohEndPoint)];
        }
        ...
        public DnsClient(Uri[] dohEndPoints)
        {
            ...
            DnsNsAddress[] servers = new DnsNsAddress[dohEndPoints.Length];

            for (int i = 0; i < dohEndPoints.Length; i++)
                servers[i] = new DnsNsAddress(dohEndPoints[i]);
        ...
        public DnsClient(bool preferIPv6 = false)
        {
            ...
            DnsNsAddress[] servers = new DnsNsAddress[systemDnsServers.Count];

            for (int i = 0; i < systemDnsServers.Count; i++)
                servers[i] = new DnsNsAddress(systemDnsServers[i]);
        ...
        public DnsClient(IPAddress[] servers)
        {
            ...
            DnsNsAddress[] nameServers = new DnsNsAddress[servers.Length];

            for (int i = 0; i < servers.Length; i++)
                nameServers[i] = new DnsNsAddress(servers[i]);

            _servers = nameServers;
        }

        public DnsClient(IPAddress server)
            : this(new DnsNsAddress(server))
        { }
        ...
        public DnsClient(string addresses)
            : this(addresses.Split(DnsNsAddress.Parse, ','))
        { }

        public DnsClient(params string[] addresses)
            : this(addresses.Convert(DnsNsAddress.Parse))
        { }

        public DnsClient(string address, DnsTransportProtocol protocol)
            : this(DnsNsAddress.Parse(address, protocol))
        { }

        public DnsClient(DnsNsAddress server)
        {
            _servers = [server];
        }

        public DnsClient(params DnsNsAddress[] servers)
        {
            if (servers.Length == 0)
                throw new DnsClientException("At least one name server must be available for DnsClient.");
        ...
        public DnsClient(IReadOnlyList<DnsNsAddress> servers)
        {
            if (servers.Count == 0)
                throw new DnsClientException("At least one name server must be available for DnsClient.");
        ...
        public static async Task ReloadRootHintsAsync()
        {
            ...
            List<DnsNsAddress> ipv4RootHints = new List<DnsNsAddress>(13);
            List<DnsNsAddress> ipv6RootHints = new List<DnsNsAddress>(13);
                    ...
                    switch (record.Type)
                    {
                        case DnsRecordType.A:
                            if (name.Equals(record.Name, StringComparison.OrdinalIgnoreCase))
                                ipv4RootHints.Add(new DnsNsAddress(name, (record.RDATA as DnsARecordData).Address));

                            break;

                        case DnsRecordType.AAAA:
                            if (name.Equals(record.Name, StringComparison.OrdinalIgnoreCase))
                                ipv6RootHints.Add(new DnsNsAddress(name, (record.RDATA as DnsAAAARecordData).Address));
        ...
        public static void ReloadRootTrustAnchors()
        {
            ...
            foreach (XmlNode keyDigestNode in nodeList)
            {
                ...
                rootTrustAnchors.Add(new DnsResourceRecord("", DnsRecordType.DS, DnsClass.Internet, 0, new DnsDSRecordData(keyTag, algorithm, digestType, Convert.FromHexString(digest))));
            }

            ROOT_TRUST_ANCHORS = rootTrustAnchors;
        }

        public static async Task<DnsPacket> RecursiveResolveAsync(DnsPacketQuestion question, IDnsCache cache = null, NetProxy proxy = null, bool preferIPv6 = false, ushort udpPayloadSize = DnsPacket.EDNS_DEFAULT_UDP_PAYLOAD_SIZE, bool randomizeName = false, bool qnameMinimization = false, bool dnssecValidation = false, NetworkAddress eDnsClientSubnet = null, int retries = 2, int timeout = 2000, int concurrency = 2, int maxStackCount = 16, bool minimalResponse = false, bool asyncNsRevalidation = false, bool asyncNsResolution = false, List<DnsPacket> rawResponses = null, CancellationToken cancellationToken = default)
        {
            ...
            void TriggerNsResolution()
            {
                ...
                _ = Task.Factory.StartNew(delegate ()
                {
                    return TaskExtensions.TimeoutAsync(async delegate (CancellationToken cancellationToken1)
                    {
                        ...
                        {
                            if (preferIPv6)
                                tasks.Add(RecursiveResolveAsync(new DnsPacketQuestion(entry.Key, DnsRecordType.AAAA, DnsClass.Internet), cache, proxy, preferIPv6, udpPayloadSize, randomizeName, qnameMinimization, dnssecValidation, null, retries, timeout, concurrency, maxStackCount, cancellationToken: cancellationToken1));

                            tasks.Add(RecursiveResolveAsync(new DnsPacketQuestion(entry.Key, DnsRecordType.A, DnsClass.Internet), cache, proxy, preferIPv6, udpPayloadSize, randomizeName, qnameMinimization, dnssecValidation, null, retries, timeout, concurrency, maxStackCount, cancellationToken: cancellationToken1));
                        }
            ...
            IList<DnsNsAddress> nameServers = null;
            int nameServerIndex = 0;
            int hopCount = 0;
            DnsPacket lastResponse = null;
            Exception lastException = null;

            void PushStack(string nextQName, DnsRecordType nextQType)
            {
                ...
                question = new DnsPacketQuestion(nextQName, nextQType, question.Class);
            ...
            void InspectCacheNameServersForLoops(List<DnsNsAddress> cacheNameServers)
            {
                bool allCacheNameServersHaveGlue = true;

                foreach (DnsNsAddress cacheNameServer in cacheNameServers)
                {
                ...
                {
                    foreach (DnsNsAddress stackNameServer in stack.NameServers)
                    {
            ...
            async Task<List<DnsNsAddress>> ResolveNameServerAddressesFromCacheAsync(List<DnsNsAddress> nameServers)
            {
                List<DnsNsAddress> newNameServers = new List<DnsNsAddress>(preferIPv6 ? nameServers.Count * 2 : nameServers.Count);

                foreach (DnsNsAddress nameServer in nameServers)
                {
                    ...
                    if (preferIPv6)
                    {
                        DnsPacket cacheRequest = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, false, DnsResponseCode.NoError, [new DnsPacketQuestion(nameServer.DomainEndPoint.Address, DnsRecordType.AAAA, DnsClass.Internet)]);
                        DnsPacket cacheResponse = await cache.QueryAsync(cacheRequest);
                        if ((cacheResponse is not null) && (cacheResponse.Answer.Count > 0) && (cacheResponse.Answer[0].Type == DnsRecordType.AAAA))
                        {
                            resolved = true;
                    ...
                    {
                        DnsPacket cacheRequest = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, false, DnsResponseCode.NoError, [new DnsPacketQuestion(nameServer.DomainEndPoint.Address, DnsRecordType.A, DnsClass.Internet)]);
                        DnsPacket cacheResponse = await cache.QueryAsync(cacheRequest);
                        if ((cacheResponse is not null) && (cacheResponse.Answer.Count > 0) && (cacheResponse.Answer[0].Type == DnsRecordType.A))
                        {
                            resolved = true;
            ...
            while (true) //stack loop
            {
                if (resolverStack.Count > maxStackCount)
                {
                    ...
                    DnsPacket failureResponse = new DnsPacket(0, true, DnsOpCode.StandardQuery, false, false, false, false, false, false, DnsResponseCode.ServerFailure, new DnsPacketQuestion[] { question });
                ...
                //query cache
                {
                    ...
                    DnsPacket cacheRequest = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, false, DnsResponseCode.NoError, new DnsPacketQuestion[] { question }, null, null, null, udpPayloadSize, ednsFlags, resolverStack.Count == 0 ? eDnsClientSubnetOption : null);
                    DnsPacket cacheResponse = await cache.QueryAsync(cacheRequest, findClosestNameServers: true);
                            ...
                            case DnsResponseCode.NoError:
                                {
                                                ...
                                                switch (answer.Type)
                                                {
                                                    case DnsRecordType.AAAA:
                                                        ...
                                                        for (int j = i + 1; j < cacheResponse.Answer.Count; j++)
                                                        {
                                                            ...
                                                            if (answer.Type == DnsRecordType.AAAA)
                                    ...
                                    else if (cacheResponse.Authority.Count > 0)
                                    {
                                        ...
                                        if (firstAuthority.Type == DnsRecordType.SOA)
                                        {
                                            if (resolverStack.Count == 0)
                                            {
                                                TriggerNsRevalidation();
                                        ...
                                        else
                                        {
                                            ...
                                            List<DnsNsAddress> nextNameServers = null;

                                            nextNameServers = DnsNsAddress.GetNameServersFromResponse(cacheResponse, preferIPv6, false);
                                            ...
                                            if (nextNameServers.Count == 0)
                                            {
                                                ...
                                                while (true)
                                                {
                                                    ...
                                                    DnsPacket cachedNsResponse = await cache.QueryAsync(new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, false, DnsResponseCode.NoError, new DnsPacketQuestion[] { new DnsPacketQuestion(currentDomain, DnsRecordType.NS, DnsClass.Internet) }), findClosestNameServers: true);
                                                    if (cachedNsResponse is null)
                                                        continue;

                                                    nextNameServers = DnsNsAddress.GetNameServersFromResponse(cachedNsResponse, preferIPv6, false);
                                                    InspectCacheNameServersForLoops(nextNameServers);
                ...
                while (true) //resolver loop
                {
                    if ((lastDSRecords is not null) && !lastDSRecords[0].Name.Equals(zoneCut, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        PushStack(zoneCut, DnsRecordType.DS);
                        goto stackLoop;
                    }
                    ...
                    for (; nameServerIndex < referralLimit; nameServerIndex++) //try next server loop
                    {
                        ...
                        List<DnsNsAddress> resolvedNameServers = new List<DnsNsAddress>(referralLimit - nameServerIndex);
                        ...
                        if (resolvedNameServers.Count > 0)
                        {
                            ...
                            if (preferIPv6)
                            {
                                ...
                                List<DnsNsAddress> nameServersToAdd = new List<DnsNsAddress>();

                                foreach (DnsNsAddress nameServerWithIpv4Glue in nameServers)
                                {
                                ...
                                foreach (DnsNsAddress nameServer in nameServersToAdd)
                                {
                        ...
                        else
                        {
                            ...
                            DnsNsAddress currentNameServer = nameServers[nameServerIndex];
                            ...
                            if (preferIPv6)
                            {
                                ...
                                {
                                    DnsNsAddress attemptedNameServer = nameServers[i];
                                ...
                                if (wasIPv6Attempted)
                                {
                                    PushStack(currentNameServer.DomainEndPoint.Address, DnsRecordType.A);
                                }
                                else
                                {
                                    if (!wasIPv4Attempted)
                                    ...
                                    PushStack(currentNameServer.DomainEndPoint.Address, DnsRecordType.AAAA);
                                }
                            }
                            else
                            {
                        ...
                        DnsPacket request = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, false, false, false, false, DnsResponseCode.NoError, [question], null, null, null, udpPayloadSize, ednsFlags, (resolverStack.Count == 0) && zoneCut.Contains('.') ? eDnsClientSubnetOption : null);
                        DnsPacket response;

                        try
                        {
                            response = await dnsClient.InternalResolveAsync(request, async delegate (DnsPacket response, CancellationToken cancellationToken1)
                            {
                                ...
                                if ((response.ResponseCode == DnsResponseCode.NoError) && (response.Answer.Count == 0) && (response.Authority.Count > 0))
                                {
                                    foreach (DnsResourceRecord authorityRecord in response.Authority)
                                    {
                                        if ((authorityRecord.Type == DnsRecordType.NS) && authorityRecord.Name.Equals(zoneCut, StringComparison.OrdinalIgnoreCase))
                        ...
                        if ((response.ResponseCode != DnsResponseCode.NoError) && (extendedDnsErrors.Count > 0))
                            ...
                            case DnsResponseCode.NoError:
                                {
                                    if (response.Answer.Count > 0)
                                    {
                                        ...
                                        if (!qnameMatches)
                                        {
                                            ...
                                            {
                                                if ((answer.Type == DnsRecordType.DNAME) && question.Name.EndsWith("." + answer.Name, StringComparison.OrdinalIgnoreCase))
                                                {
                                                    foundDNAME = true;
                                                    break;
                                                }
                                    ...
                                    else if (response.Authority.Count > 0)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            List<DnsNsAddress> nextNameServers = DnsNsAddress.GetNameServersFromResponse(response, preferIPv6, true);
                                            ...
                                            if (nextNameServers.Count > 0)
                                            {
                                                ...
                                                if (asyncNsResolution)
                                                {
                                                    ...
                                                    foreach (DnsNsAddress nextNameServer in nextNameServers)
                                                    {
                    ...
                    {
                        DnsPacketQuestion lastQuestion = question;
                        PopStack();
        ...
        public static Task<DnsPacket> RecursiveResolveQueryAsync(DnsPacketQuestion question, IDnsCache cache = null, NetProxy proxy = null, bool preferIPv6 = false, ushort udpPayloadSize = DnsPacket.EDNS_DEFAULT_UDP_PAYLOAD_SIZE, bool randomizeName = false, bool qnameMinimization = false, bool dnssecValidation = false, NetworkAddress eDnsClientSubnet = null, int retries = 2, int timeout = 2000, int concurrency = 2, int maxStackCount = 16, CancellationToken cancellationToken = default)
        {
            if (cache is null)
                cache = new DnsCache();

            return ResolveQueryAsync(question, delegate (DnsPacketQuestion q)
            {
        ...
        public static async Task<IReadOnlyList<IPAddress>> RecursiveResolveIPAsync(string domain, IDnsCache cache = null, NetProxy proxy = null, bool preferIPv6 = false, ushort udpPayloadSize = DnsPacket.EDNS_DEFAULT_UDP_PAYLOAD_SIZE, bool randomizeName = false, bool qnameMinimization = false, bool dnssecValidation = false, NetworkAddress eDnsClientSubnet = null, int retries = 2, int timeout = 2000, int concurrency = 2, int maxStackCount = 16, CancellationToken cancellationToken = default)
        {
            if (cache is null)
                cache = new DnsCache();
            ...
            if (preferIPv6)
            {
                IReadOnlyList<IPAddress> addresses = ParseResponseAAAA(await RecursiveResolveQueryAsync(new DnsPacketQuestion(domain, DnsRecordType.AAAA, DnsClass.Internet), cache, proxy, preferIPv6, udpPayloadSize, randomizeName, qnameMinimization, dnssecValidation, eDnsClientSubnet, retries, timeout, concurrency, maxStackCount, cancellationToken));
                if (addresses.Count > 0)
                    return addresses;
            }

            return ParseResponseA(await RecursiveResolveQueryAsync(new DnsPacketQuestion(domain, DnsRecordType.A, DnsClass.Internet), cache, proxy, preferIPv6, udpPayloadSize, randomizeName, qnameMinimization, dnssecValidation, eDnsClientSubnet, retries, timeout, concurrency, maxStackCount, cancellationToken));
        }
        ...
        public static async Task<IReadOnlyList<IPAddress>> ResolveIPAsync(IDnsClient dnsClient, string domain, bool preferIPv6 = false, CancellationToken cancellationToken = default)
        {
            if (preferIPv6)
            {
                IReadOnlyList<IPAddress> addresses = ParseResponseAAAA(await dnsClient.ResolveAsync(new DnsPacketQuestion(domain, DnsRecordType.AAAA, DnsClass.Internet), cancellationToken));
                if (addresses.Count > 0)
                    return addresses;
            }

            return ParseResponseA(await dnsClient.ResolveAsync(new DnsPacketQuestion(domain, DnsRecordType.A, DnsClass.Internet), cancellationToken));
        }
        ...
        public static async Task<IReadOnlyList<string>> ResolveMXAsync(IDnsClient dnsClient, string domain, bool resolveIP = false, bool preferIPv6 = false, CancellationToken cancellationToken = default)
        {
            ...
            DnsPacket response = await dnsClient.ResolveAsync(new DnsPacketQuestion(domain, DnsRecordType.MX, DnsClass.Internet), cancellationToken);
        ...
        public static IReadOnlyList<IPAddress> ParseResponseA(DnsPacket response)
        {
            ...
            switch (response.ResponseCode)
            {
                ...
                case DnsResponseCode.NxDomain:
                    throw new DnsClientNxDomainException("Domain does not exists: " + domain.ToLowerInvariant() + (response.Meta is null ? "" : "; Name server: " + response.Meta.NameServer.ToString()));

                default:
                    throw new DnsClientFailureResponseException("DnsClient failed to resolve the request '" + response.Question[0].ToString() + "'. Received a response with RCODE: " + response.ResponseCode + (response.Meta is null ? "" : " from Name server: " + response.Meta.NameServer.ToString()), response);
            }
        }

        public static IReadOnlyList<IPAddress> ParseResponseAAAA(DnsPacket response)
        {
            string domain = response.Question[0].Name;
        ...
        public static IReadOnlyList<string> ParseResponseTXT(DnsPacket response)
        {
            string domain = response.Question[0].Name;
        ...
        public static IReadOnlyList<string> ParseResponsePTR(DnsPacket response)
        {
            string domain = response.Question[0].Name;
        ...
        public static IReadOnlyList<string> ParseResponseMX(DnsPacket response)
        {
            string domain = response.Question[0].Name;
        ...
        public static IReadOnlyList<DnsDSRecordData> ParseResponseDS(DnsPacket response)
        {
            string domain = response.Question[0].Name;
        ...
        public static IReadOnlyList<DnsTLSARecordData> ParseResponseTLSA(DnsPacket response)
        {
            string domain = response.Question[0].Name;
        ...
        public static IReadOnlyList<DnsZONEMDRecordData> ParseResponseZONEMD(DnsPacket response)
        {
            string domain = response.Question[0].Name;
        ...
        public static DnsSOARecordData ParseResponseSOA(DnsPacket response)
        {
            string domain = response.Question[0].Name;
        ...
        private static int CompareNameServersToPreferOnesWithIPAddress(DnsNsAddress x, DnsNsAddress y)
        {
        ...
        private static int CompareNameServersToPreferIPv6(DnsNsAddress x, DnsNsAddress y)
        {
        ...
        private static int CompareNameServersToPreferPerformance(DnsNsAddress x, DnsNsAddress y)
        {
        ...
        private static List<DnsNsAddress> GetOrderedNameServersToPreferPerformance(IReadOnlyCollection<DnsNsAddress> nameServers, bool prioritizeOnesWithIPAddress, bool preferIPv6)
        {
            ...
            List<DnsNsAddress> nameServersList = new List<DnsNsAddress>(nameServers);
        ...
        private static async Task<List<DnsNsAddress>> GetRootServersUsingRootHintsAsync(IDnsCache cache, NetProxy proxy, bool preferIPv6, ushort udpPayloadSize, bool dnssecValidation, int retries, int timeout, CancellationToken cancellationToken = default)
        {
            //create copy of root name servers array so that the values in original array are not messed due to shuffling feature
            List<DnsNsAddress> rootHints;

            if (preferIPv6)
            {
                List<DnsNsAddress> nameServersList = new List<DnsNsAddress>(IPv6_ROOT_HINTS.Count + IPv4_ROOT_HINTS.Count);
            ...
            {
                List<DnsNsAddress> nameServersList = new List<DnsNsAddress>(IPv4_ROOT_HINTS);
                nameServersList.Shuffle();
            ...
            DnsPacketQuestion question = new DnsPacketQuestion("", DnsRecordType.NS, DnsClass.Internet);
            DnsPacket response;

            if (dnssecValidation)
                response = await dnsClient.InternalDnssecResolveAsync(question, cancellationToken);
            else
                response = await dnsClient.InternalNoDnssecResolveAsync(new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, dnssecValidation, DnsResponseCode.NoError, [question], null, null, null, udpPayloadSize, dnssecValidation ? EDnsHeaderFlags.DNSSEC_OK : EDnsHeaderFlags.None, null), cancellationToken);

            //validate priming query response
            if (response.ResponseCode != DnsResponseCode.NoError)
                throw new DnsClientFailureResponseException("DnsClient failed to resolve the request '" + question.ToString() + "'. Received a response with RCODE: " + response.ResponseCode + (response.Meta is null ? "" : " from Name server: " + response.Meta.NameServer.ToString()), response);

            if (!response.IsAuthoritativeAnswer)
                throw new DnsClientFailureResponseException("DnsClient failed to resolve the request '" + question.ToString() + "'. Received a response without AuthoritativeAnswer flag set from Name server: " + response.Meta.NameServer.ToString(), response);

            if ((response.Answer.Count == 0) || (response.Authority.Count > 0) || (response.Additional.Count == 0))
                throw new DnsClientFailureResponseException("DnsClient failed to resolve the request '" + question.ToString() + "'. Received a response without any answer from Name server: " + response.Meta.NameServer.ToString(), response);

            cache.CacheResponse(response);

            List<DnsNsAddress> rootServers = DnsNsAddress.GetNameServersFromResponse(response, preferIPv6, true);
            rootServers.Shuffle();

            return rootServers;
        }

        private static async Task DnssecValidateResponseAsync(DnsPacket response, IReadOnlyList<DnsResourceRecord> lastDSRecords, DnsClient dnsClient, IDnsCache cache, ushort udpPayloadSize, CancellationToken cancellationToken = default)
        {
            //find current DNSKEY
                ...
                switch (response.ResponseCode)
                {
                    case DnsResponseCode.NoError:
                        if (response.Answer.Count > 0)
                        {
                            ...
                            {
                                if (rrsigRecord.Type != DnsRecordType.RRSIG)
                                    continue;
                                ...
                                if (DnsRRSIGRecordData.IsWildcard(rrsigRecord, out string nextCloserName))
                                {
                                    ...
                                    //validate wildcard
                                    DnsRecordType typeCovered = (rrsigRecord.RDATA as DnsRRSIGRecordData).TypeCovered;
                        ...
                        {
                            DnsPacketQuestion question = response.Question[0];
                            DnsResourceRecord firstAuthority = response.FindFirstAuthorityRecord();
                                ...
                                case DnsRecordType.NS:
                                    {
                                        ...
                                        DnssecProofOfNonExistence proofOfNonExistence = await GetValidatedProofOfNonExistenceAsync(response, question.Name, DnsRecordType.DS);
                                        switch (proofOfNonExistence)
                                        {
        ...
        private static async Task DnssecValidateSignatureAsync(DnsPacket response, IReadOnlyList<DnsResourceRecord> dnsKeyRecords, IReadOnlyList<string> unsignedZones)
        {
            ...
            {
                if (dnsKeyRecord.Type != DnsRecordType.DNSKEY)
                    continue;
            ...
            if (response.Answer.Count > 0)
            {
                ...
                if (response.Question[0].Type == DnsRecordType.DNSKEY)
        ...
        private static async Task DnssecValidateSignatureAsync(DnsPacket response, IReadOnlyList<DnsResourceRecord> records, IReadOnlyList<DnsResourceRecord> dnsKeyRecords, IReadOnlyList<string> unsignedZones, DnssecValidateSignatureParameters parameters, bool isAuthoritySection, bool isAdditionalSection)
        {
            Dictionary<string, Dictionary<DnsRecordType, List<DnsResourceRecord>>> groupedRecords = DnsResourceRecord.GroupRecords(records, true);

            foreach (KeyValuePair<string, Dictionary<DnsRecordType, List<DnsResourceRecord>>> groupedRecord in groupedRecords)
            {
                string ownerName = groupedRecord.Key;
                Dictionary<DnsRecordType, List<DnsResourceRecord>> rrsets = groupedRecord.Value;

                if (IsDomainUnsigned(ownerName, unsignedZones))
                {
                    foreach (KeyValuePair<DnsRecordType, List<DnsResourceRecord>> rrset in rrsets)
                ...
                foreach (KeyValuePair<DnsRecordType, List<DnsResourceRecord>> rrset in rrsets)
                {
                    DnsRecordType rrsetType = rrset.Key;
                    ...
                    if (isAuthoritySection && (response.Answer.Count == 0) && (rrsetType == DnsRecordType.NS))
                    {
                    ...
                    else if (isAuthoritySection && (rrsetType == DnsRecordType.NS) && (lastExtendedDnsErrorCode == EDnsExtendedDnsErrorCode.RRSIGsMissing))
                    {
                    ...
                    else
                    {
                        switch (lastExtendedDnsErrorCode)
                        {
                            ...
                            case EDnsExtendedDnsErrorCode.RRSIGsMissing:
                                ...
                                if (rrsetType == DnsRecordType.CNAME)
                                {
                                    ...
                                    {
                                        if (dnameRecord.Type != DnsRecordType.DNAME)
                                            continue;
        ...
        private static async Task<IReadOnlyList<DnsResourceRecord>> FindDnsKeyForAsync(string ownerName, DnsClass @class, IReadOnlyList<DnsResourceRecord> currentDnsKeyRecords, DnsClient dnsClient, IDnsCache cache, ushort udpPayloadSize, DnsPacket originalResponse, CancellationToken cancellationToken)
        {
        ...
        private static async Task<IReadOnlyList<DnsResourceRecord>> GetDnsKeyForAsync(IReadOnlyList<DnsResourceRecord> lastDSRecords, DnsClient dnsClient, IDnsCache cache, ushort udpPayloadSize, CancellationToken cancellationToken)
        {
            ...
            DnsPacketQuestion dnsKeyQuestion = new DnsPacketQuestion(lastDSRecord.Name, DnsRecordType.DNSKEY, lastDSRecord.Class);

            //query cache without CD & DO flags
            DnsPacket cacheDnsKeyRequest = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, false, DnsResponseCode.NoError, [dnsKeyQuestion], null, null, null, udpPayloadSize, EDnsHeaderFlags.None);
            DnsPacket cacheDnsKeyResponse = await QueryCacheAsync(cache, cacheDnsKeyRequest);
            ...
            //query name server
            DnsPacket dnsKeyRequest = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, true, DnsResponseCode.NoError, [dnsKeyQuestion], null, null, null, udpPayloadSize, EDnsHeaderFlags.DNSSEC_OK);
            DnsPacket dnsKeyResponse = await dnsClient.InternalResolveAsync(dnsKeyRequest, async delegate (DnsPacket dnsKeyResponse, CancellationToken cancellationToken1)
            {
                if (dnsKeyResponse.Answer.Count == 0)
                {
                    switch (dnsKeyResponse.ResponseCode)
                    {
                        case DnsResponseCode.NoError:
                            dnsKeyResponse.AddDnsClientExtendedError(EDnsExtendedDnsErrorCode.DNSKEYMissing, "Attack detected! " + (dnsKeyResponse.Meta is null ? "name server" : dnsKeyResponse.Meta.NameServer.ToString()) + " returned no DNSKEYs for " + dnsKeyQuestion.Name.ToLowerInvariant());
                        ...
                        default:
                            dnsKeyResponse.AddDnsClientExtendedError(EDnsExtendedDnsErrorCode.DNSKEYMissing, "Attack detected! " + (dnsKeyResponse.Meta is null ? "name server" : dnsKeyResponse.Meta.NameServer.ToString()) + " returned RCODE=" + dnsKeyResponse.ResponseCode.ToString() + " for " + dnsKeyQuestion.ToString());
                            cache.CacheResponse(dnsKeyResponse, true);
                            throw new DnsClientResponseDnssecValidationException("Attack detected! Failed to resolve the request '" + dnsKeyResponse.Question[0].ToString() + "'. Received a response with RCODE: " + dnsKeyResponse.ResponseCode + (dnsKeyResponse.Meta is null ? "" : " from Name server: " + dnsKeyResponse.Meta.NameServer.ToString()), dnsKeyResponse);
                    }
                }
        ...
        private static async Task<IReadOnlyList<DnsResourceRecord>> GetDSForAsync(string ownerName, DnsClass @class, IReadOnlyList<DnsResourceRecord> currentDnsKeyRecords, DnsClient dnsClient, IDnsCache cache, ushort udpPayloadSize, DnsPacket originalResponse, CancellationToken cancellationToken)
        {
            string dnsKeyOwnerName = currentDnsKeyRecords[0].Name;
            ...
            DnsPacketQuestion dsQuestion = new DnsPacketQuestion(ownerName, DnsRecordType.DS, @class);
            ...
            DnsPacket cacheDSRequest = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, false, DnsResponseCode.NoError, [dsQuestion], null, null, null, udpPayloadSize, EDnsHeaderFlags.DNSSEC_OK);
            DnsPacket cacheDSResponse = await QueryCacheAsync(cache, cacheDSRequest);
            ...
            //query dns server
            DnsPacket dsRequest = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, true, DnsResponseCode.NoError, [dsQuestion], null, null, null, udpPayloadSize, EDnsHeaderFlags.DNSSEC_OK);
            IReadOnlyList<DnsResourceRecord> dsRecords = []; //default NO DATA response so that DNSKEY remains same

            try
            {
                _ = await dnsClient.InternalResolveAsync(dsRequest, async delegate (DnsPacket dsResponse, CancellationToken cancellationToken1)
                {
                    ...
                    switch (dsResponse.ResponseCode)
                    {
                        case DnsResponseCode.NoError:
                        case DnsResponseCode.NxDomain:
                            dsResponse.AddDnsClientExtendedError(EDnsExtendedDnsErrorCode.DnssecIndeterminate, (dsResponse.Meta is null ? "name server" : dsResponse.Meta.NameServer.ToString()) + " returned no DS for " + ownerName.ToLowerInvariant());
                        ...
                        default:
                            dsResponse.AddDnsClientExtendedError(EDnsExtendedDnsErrorCode.DnssecIndeterminate, "Attack detected! " + (dsResponse.Meta is null ? "name server" : dsResponse.Meta.NameServer.ToString()) + " returned RCODE=" + dsResponse.ResponseCode.ToString() + " for " + dsQuestion.ToString());
                            cache.CacheResponse(dsResponse, true);
                            throw new DnsClientResponseDnssecValidationException("Attack detected! Failed to resolve the request '" + dsResponse.Question[0].ToString() + "'. Received a response with RCODE: " + dsResponse.ResponseCode + (dsResponse.Meta is null ? "" : " from Name server: " + dsResponse.Meta.NameServer.ToString()), dsResponse);
                    }
                }, false, cancellationToken);
            }
            catch (DnsClientResponseDnssecValidationException ex)
            {
                ...
                {
                    if ((record.Type == DnsRecordType.CNAME) && record.Name.Equals(ownerName, StringComparison.OrdinalIgnoreCase))
                    {
                        if (record.DnssecStatus == DnssecStatus.Secure)
        ...
        private static async Task<Tuple<bool, IReadOnlyList<DnsResourceRecord>>> TryGetDSFromResponseAsync(DnsPacket response, string ownerName)
        {
            ...
            switch (response.ResponseCode)
            {
                ...
                case DnsResponseCode.NoError:
                    if ((response.Question[0].Type == DnsRecordType.DS) && response.Question[0].Name.Equals(ownerName, StringComparison.OrdinalIgnoreCase))
                    {
                        //DS query response
                        if (response.Answer.Count > 0)
                        {
                        ...
                        {
                            //validate if DS records are really missing
                            DnssecProofOfNonExistence proofOfNonExistence = await GetValidatedProofOfNonExistenceAsync(response, ownerName, DnsRecordType.DS);
                            switch (proofOfNonExistence)
                            {
                                case DnssecProofOfNonExistence.InsecureDelegation:
                                case DnssecProofOfNonExistence.OptOut:
                            ...
                            DnsResourceRecord firstAuthority = response.FindFirstAuthorityRecord();
                            if ((firstAuthority.Type == DnsRecordType.SOA) && (firstAuthority.DnssecStatus == DnssecStatus.Insecure))
                            {
                    ...
                    else
                    {
                        //referral response
                        DnsResourceRecord firstAuthority = response.FindFirstAuthorityRecord();
                        if ((firstAuthority is not null) && (firstAuthority.Type == DnsRecordType.NS) && (firstAuthority.DnssecStatus == DnssecStatus.Insecure))
                        {
        ...
        private static List<DnsResourceRecord> GetFilterdDSRecords(IReadOnlyList<DnsResourceRecord> records, string ownerName)
        {
            ...
            {
                if ((record.Type == DnsRecordType.DS) && record.Name.Equals(ownerName, StringComparison.OrdinalIgnoreCase))
        ...
        private static async Task<DnssecProofOfNonExistence> GetValidatedProofOfNonExistenceAsync(DnsPacket response, string domain, DnsRecordType type, bool wildcardAnswerValidation = false, string wildcardNextCloserName = null)
        {
            foreach (DnsResourceRecord record in response.Authority)
            {
                ...
                if (record.Type == DnsRecordType.NSEC3)
                {
                    ...
                    if (proof == DnssecProofOfNonExistence.UnsupportedNSEC3IterationsValue)
                    {
                        ...
                        {
                            if ((authority.Type == DnsRecordType.SOA) || (authority.Type == DnsRecordType.NS))
                            {
        ...
        private static IReadOnlyCollection<string> FindSignersNames(DnsPacket response)
        {
        ...
        private static void FindSignersNames(IReadOnlyList<DnsResourceRecord> records, List<string> signersNames, bool isAuthoritySection)
        {
            foreach (DnsResourceRecord record in records)
            {
                ...
                foreach (DnsResourceRecord rrsigRecord in records)
                {
                    ...
                    else if ((record.Type == DnsRecordType.CNAME) && record.Name.EndsWith("." + rrsigRecord.Name, StringComparison.OrdinalIgnoreCase) && ((rrsigRecord.RDATA as DnsRRSIGRecordData).TypeCovered == DnsRecordType.DNAME))
                    {
                        isRecordCovered = true;
                        break;
                    }
                ...
                if (!isRecordCovered)
                {
                    if (isAuthoritySection && (record.Type == DnsRecordType.NS))
                    {
                        //ignore record
                    }
                    else
        ...
        private static DnsPacket SanitizeResponseAnswerForQName(DnsPacket response)
        {
            bool fixAnswer = false;

            foreach (DnsPacketQuestion question in response.Question)
            {
                ...
                foreach (DnsResourceRecord answer in response.Answer)
                {
                    if (qName.Equals(answer.Name, StringComparison.OrdinalIgnoreCase))
                    {
                        switch (answer.Type)
                        {
                            ...
                            default:
                                if ((question.Type == answer.Type) || (question.Type == DnsRecordType.ANY))
                                    continue;
            ...
            foreach (DnsPacketQuestion question in response.Question)
            {
                ...
                do
                {
                    ...
                    foreach (DnsResourceRecord answer in response.Answer)
                    {
                        ...
                        else if ((answer.Type == DnsRecordType.DNAME) && qName.EndsWith("." + answer.Name, StringComparison.OrdinalIgnoreCase))
                        {
                            //found DNAME
                            newAnswers.Add(answer);
                        }
        ...
        private static DnsPacket SanitizeResponseAnswerForZoneCut(DnsPacket response, string zoneCut)
        {
            ...
            for (int i = 0; i < response.Answer.Count; i++)
            {
                ...
                if ((answer.Type == DnsRecordType.DNAME) && qName.EndsWith("." + answer.Name, StringComparison.OrdinalIgnoreCase))
                    continue; //found DNAME, continue next
        ...
        private static DnsPacket SanitizeResponseAuthorityForZoneCut(DnsPacket response, string zoneCut)
        {
            if (zoneCut.Length == 0)
            {
        ...
        private static DnsPacket SanitizeResponseAdditionalForZoneCut(DnsPacket response, string zoneCut)
        {
            if (zoneCut.Length == 0)
            {
                //zone cut is root, do nothing
        ...
        private static DnsPacket SanitizeResponseAfterDnssecValidation(DnsPacket response)
        {
        ...
        private static DnsPacket GetMinimalResponseWithoutNSAndGlue(DnsPacket response)
        {
            bool foundNS = false;
            ...
            if (foundNS || foundIndeterminate)
            {
                ...
                if ((response.Additional.Count == 0) || ((response.Additional.Count == 1) && (response.Additional[0].Type == DnsRecordType.OPT)))
                {
                ...
                else
                {
                    ...
                    foreach (DnsResourceRecord additionalRecord in response.Additional)
                    {
                        switch (additionalRecord.DnssecStatus)
                        {
                            ...
                            default:
                                if (additionalRecord.Type == DnsRecordType.OPT)
                                    break;
                        ...
                        switch (additionalRecord.Type)
                        {
                            ...
                            case DnsRecordType.RRSIG:
                                if (foundNS)
                                {
                                    ...
                                    {
                                        if ((nsRecord.Type == DnsRecordType.NS) && additionalRecord.Name.Equals((nsRecord.RDATA as DnsNSRecordData).NameServer, StringComparison.OrdinalIgnoreCase))
                                        {
                                            foundGlue = true;
                                            break;
                                        }
        ...
        private static async Task RevalidateNameServersFromChildSideAsync(string zoneCut, IReadOnlyList<DnsResourceRecord> lastDSRecords, IReadOnlyList<DnsNsAddress> parentSideNameServers, IDnsCache cache, NetProxy proxy, bool preferIPv6, ushort udpPayloadSize, bool randomizeName, bool qnameMinimization, bool dnssecValidation, int retries, int timeout, int concurrency, int maxStackCount, CancellationToken cancellationToken)
        {
            ...
            List<DnsNsAddress> nameServers = new List<DnsNsAddress>(parentSideNameServers.Count);

            foreach (DnsNsAddress nameServer in parentSideNameServers)
            {
            ...
            DnsPacket request = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, false, false, false, dnssecValidation, DnsResponseCode.NoError, [new DnsPacketQuestion(zoneCut, DnsRecordType.NS, DnsClass.Internet)], null, null, null, udpPayloadSize, dnssecValidation ? EDnsHeaderFlags.DNSSEC_OK : EDnsHeaderFlags.None);
            DnsPacket response;
            ...
            if (response.Answer.Count > 0)
            {
                ...
                List<DnsNsAddress> revalidatedNameServers = DnsNsAddress.GetNameServersFromResponse(response, preferIPv6, true);
                List<Task> tasks = new List<Task>(revalidatedNameServers.Count);

                foreach (DnsNsAddress revalidatedNameServer in revalidatedNameServers)
                {
                    if (revalidatedNameServer.IPEndPoint is null)
                    {
                        if (preferIPv6)
                            tasks.Add(RecursiveResolveAsync(new DnsPacketQuestion(revalidatedNameServer.DomainEndPoint.Address, DnsRecordType.AAAA, DnsClass.Internet), cache, proxy, preferIPv6, udpPayloadSize, randomizeName, qnameMinimization, dnssecValidation, null, retries, timeout, concurrency, maxStackCount, cancellationToken: cancellationToken));

                        tasks.Add(RecursiveResolveAsync(new DnsPacketQuestion(revalidatedNameServer.DomainEndPoint.Address, DnsRecordType.A, DnsClass.Internet), cache, proxy, preferIPv6, udpPayloadSize, randomizeName, qnameMinimization, dnssecValidation, null, retries, timeout, concurrency, maxStackCount, cancellationToken: cancellationToken));
                    }
                }
        ...
        private static Task<DnsPacket> RevalidateNameServersFromParentSideAsync(string zoneCut, IDnsCache cache, NetProxy proxy, bool preferIPv6, ushort udpPayloadSize, bool randomizeName, bool qnameMinimization, bool dnssecValidation, int retries, int timeout, int concurrency, int maxStackCount, CancellationToken cancellationToken)
        {
            DnsPacketQuestion question = new DnsPacketQuestion(zoneCut, DnsRecordType.NS, DnsClass.Internet);
        ...
        private static async Task<DnsPacket> ResolveQueryAsync(DnsPacketQuestion question, Func<DnsPacketQuestion, Task<DnsPacket>> resolveAsync)
        {
            DnsPacket response = await resolveAsync(question);
            if (response is null)
                return new DnsPacket(0, true, DnsOpCode.StandardQuery, false, false, true, true, false, false, DnsResponseCode.Refused, new DnsPacketQuestion[] { question });

            if (response.Answer.Count > 0)
            {
                DnsResourceRecord lastRR = response.GetLastAnswerRecord();

                if ((lastRR.Type != question.Type) && (lastRR.Type == DnsRecordType.CNAME) && (question.Type != DnsRecordType.ANY))
                {
                    ...
                    do
                    {
                        ...
                        lastResponse = await resolveAsync(new DnsPacketQuestion(cnameDomain, question.Type, question.Class));
                        if (lastResponse is null)
                        {
                            lastResponse = new DnsPacket(0, true, DnsOpCode.StandardQuery, false, false, true, true, false, false, DnsResponseCode.NoError, new DnsPacketQuestion[] { question });
                            break;
                        }
                        ...
                        if (lastRR.Type != DnsRecordType.CNAME)
                        {
                        ...
                        {
                            if (answerRecord.Type != DnsRecordType.CNAME)
                                continue;
                    ...
                    DnsPacket compositeResponse = new DnsPacket(0, true, DnsOpCode.StandardQuery, false, false, true, true, false, false, lastResponse.ResponseCode, new DnsPacketQuestion[] { question }, newAnswer, authority, lastResponse.Additional);

                    if (lastResponse.Meta is not null)
                        compositeResponse.SetMetadata(lastResponse.Meta.NameServer, lastResponse.Meta.RoundTripTime);
        ...
        protected virtual async Task<DnsPacket> InternalResolveAsync(DnsPacket request, Func<DnsPacket, CancellationToken, Task<DnsPacket>> getValidatedResponseAsync = null, bool doNotReorderNameServers = false, CancellationToken cancellationToken = default)
        {
            //get servers
            IReadOnlyList<DnsNsAddress> servers;
            int concurrency;
            ...
            async Task<DnsPacket> DoResolveAsync(CancellationToken cancellationToken)
            {
                DnsPacket asyncRequest = request.CloneHeadersAndQuestions(); //clone request (headers + question section) so that qname randomization does not pollute request question section and does not cause issue with parallel tasks
                DnsPacket lastResponse = null;
                Exception lastException = null;
                ...
                while (true) //next server loop
                {
                    ...
                    DnsNsAddress server = GetNextServer();
                    if (server is null)
                    {
                        if (lastResponse is not null)
                            return lastResponse;
                    ...
                    try
                    {
                        ...
                        do //retry request loop
                        {
                            ...
                            {
                                if ((asyncRequest.Question.Count > 0) && (asyncRequest.Question[0].Type == DnsRecordType.AXFR))
                                {
                                    //use TCP for AXFR
                                    server = server.ChangeProtocol(DnsTransportProtocol.Tcp);
                                }
                                ...
                                {
                                    foreach (DnsPacketQuestion question in asyncRequest.Question)
                            ...
                            await using (DnsClientConnection connection = DnsClientConnection.GetConnection(server, proxy))
                            {
                                try
                                {
                                    DnsPacket response = await connection.QueryAsync(asyncRequest, _timeout, _retries, cancellationToken);
                                    if (response.IsTruncation)
                                    {
                                        if (server.Protocol == DnsTransportProtocol.Udp)
                                        {
                                            ...
                                            if (response.Meta is not null)
                                                server.Metadata.UpdateSuccess(response.Meta.RoundTripTime);
                                        ...
                                        else
                                        {
                                            ...
                                            switch (response.ResponseCode)
                                            {
                                                ...
                                                case DnsResponseCode.YXDomain:
                                                    ...
                                                    response.SetIdentifier(request.Id);
                                                    return response;
                                                ...
                                                case DnsResponseCode.NxDomain:
                                                    ...
                                                    if (request.IsRecursionDesired && !response.IsRecursionAvailable && !response.IsAuthoritativeAnswer)
                                                        response.AddDnsClientExtendedError(EDnsExtendedDnsErrorCode.Blocked, response.Question[0].Name.ToLowerInvariant() + " was blocked by " + (response.Meta is null ? "upstream server" : response.Meta.NameServer.ToString()));
                                                ...
                                                case DnsResponseCode.FormatError:
                                                    ...
                                                    {
                                                        server.Metadata.UpdateFailure(_timeout * _retries);
                                                        response.AddDnsClientExtendedError(EDnsExtendedDnsErrorCode.NoReachableAuthority, (response.Meta is null ? "name server" : response.Meta.NameServer.ToString()) + " returned RCODE=" + response.ResponseCode.ToString() + " for " + request.Question[0].ToString());
                    ...
                    await using (CancellationTokenRegistration ctr = cancellationToken.Register(cancellationTokenSource.Cancel))
                    {
                        ...
                        while (true)
                        {
                            ...
                            if (completedTask.Status == TaskStatus.RanToCompletion)
                            {
                                ...
                                DnsPacket response = await (completedTask as Task<DnsPacket>); //await to get response
                            ...
                            if (tasks.Count == 2)
                            {
                                ...
                                return await (completedTask as Task<DnsPacket>); //await throw error
                            }
        ...
        private async Task<DnsPacket> InternalNoDnssecResolveAsync(DnsPacket request, CancellationToken cancellationToken = default)
        {
            if ((_conditionalForwardingZoneCut is not null) && (request.Question.Count == 1))
            {
                DnsPacketQuestion question = request.Question[0];
            ...
            DnsPacket response = await InternalResolveAsync(request, cancellationToken: cancellationToken);
            ...
            if (_conditionalForwardingZoneCut is not null)
            {
                ...
                foreach (DnsResourceRecord answer in response.Answer)
                {
                    if ((answer.Type == DnsRecordType.CNAME) && (answer.Name.Equals(_conditionalForwardingZoneCut, StringComparison.OrdinalIgnoreCase) || answer.Name.EndsWith("." + _conditionalForwardingZoneCut, StringComparison.OrdinalIgnoreCase)))
                    {
        ...
        private async Task<DnsPacket> InternalDnssecResolveAsync(DnsPacketQuestion question, CancellationToken cancellationToken = default)
        {
            if ((_conditionalForwardingZoneCut is not null) && !question.Name.Equals(_conditionalForwardingZoneCut, StringComparison.OrdinalIgnoreCase) && !question.Name.EndsWith("." + _conditionalForwardingZoneCut, StringComparison.OrdinalIgnoreCase))
                return new DnsPacket(0, true, DnsOpCode.StandardQuery, false, false, true, true, false, false, DnsResponseCode.Refused, [question]);
            ...
            DnsPacket request = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, true, DnsResponseCode.NoError, [question], null, null, null, _udpPayloadSize, EDnsHeaderFlags.DNSSEC_OK, _advancedForwardingClientSubnet ? null : EDnsClientSubnetOptionData.GetEDnsClientSubnetOption(_eDnsClientSubnet));
            ...
            while (true)
            {
                try
                {
                    return await InternalResolveAsync(request, async delegate (DnsPacket response, CancellationToken cancellationToken1)
                    {
                        //sanitize response
                        response = SanitizeResponseAnswerForQName(response);
        ...
        private IReadOnlyList<DnsResourceRecord> GetTrustAnchorsFor(DnsPacket response)
        {
        ...
        private async Task<DnsPacket> InternalCachedResolveQueryAsync(DnsPacketQuestion question, CancellationToken cancellationToken)
        {
            return await ResolveQueryAsync(question, async delegate (DnsPacketQuestion q)
            {
                ...
                DnsPacket newRequest = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, _dnssecValidation, DnsResponseCode.NoError, new DnsPacketQuestion[] { q }, null, null, null, _udpPayloadSize, _dnssecValidation ? EDnsHeaderFlags.DNSSEC_OK : EDnsHeaderFlags.None, _advancedForwardingClientSubnet ? null : EDnsClientSubnetOptionData.GetEDnsClientSubnetOption(_eDnsClientSubnet));
                if (_advancedForwardingClientSubnet)
                    newRequest.SetShadowEDnsClientSubnetOption(_eDnsClientSubnet, true);

                DnsPacket cacheResponse = await QueryCacheAsync(_cache, newRequest);
                if (cacheResponse is not null)
                    return cacheResponse;

                try
                {
                    DnsPacket newResponse;
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        //cache as failure
                        DnsPacket failureResponse = new DnsPacket(0, true, DnsOpCode.StandardQuery, false, false, false, false, false, false, DnsResponseCode.ServerFailure, new DnsPacketQuestion[] { q });
        ...
        private static async Task<DnsPacket> QueryCacheAsync(IDnsCache cache, DnsPacket request)
        {
            DnsPacket cacheResponse = await cache.QueryAsync(request);
            if (cacheResponse is not null)
            {
                if ((cacheResponse.ResponseCode != DnsResponseCode.NoError) || (cacheResponse.Answer.Count > 0) || (cacheResponse.Authority.Count == 0) || cacheResponse.IsFirstAuthoritySOA())
                    return cacheResponse;
            }
        ...
        public Task<DnsPacket> RawResolveAsync(DnsPacket request, CancellationToken cancellationToken = default)
        {
            return InternalResolveAsync(request, cancellationToken: cancellationToken);
        }

        public async Task<DnsPacket> TsigResolveAsync(DnsPacket request, TsigKey key, ushort fudge = 300, CancellationToken cancellationToken = default)
        {
            if (request.Id == 0)
                request.SetRandomIdentifier();

            DnsPacket signedRequest = request.SignRequest(key, fudge);

            return await InternalResolveAsync(signedRequest, delegate (DnsPacket signedResponse, CancellationToken cancellationToken1)
            {
                if (!signedResponse.VerifySignedResponse(signedRequest, key, out DnsPacket unsignedResponse, out bool requestFailed, out DnsResponseCode rCode, out DnsTsigError error))
                {
                    if (requestFailed)
        ...
        public Task<DnsPacket> TsigResolveAsync(DnsPacketQuestion question, TsigKey key, ushort fudge = 300, CancellationToken cancellationToken = default)
        {
            DnsPacket request = new DnsPacket(0, false, DnsOpCode.StandardQuery, false, false, true, false, false, false, DnsResponseCode.NoError, new DnsPacketQuestion[] { question }, null, null, null, _udpPayloadSize, EDnsHeaderFlags.None, _advancedForwardingClientSubnet ? null : EDnsClientSubnetOptionData.GetEDnsClientSubnetOption(_eDnsClientSubnet));
            if (_advancedForwardingClientSubnet)
                request.SetShadowEDnsClientSubnetOption(_eDnsClientSubnet, true);
        ...
        public Task<DnsPacket> TsigResolveAsync(string domain, DnsRecordType type, TsigKey key, ushort fudge = 300, CancellationToken cancellationToken = default)
        {
            if ((type == DnsRecordType.PTR) && IPAddress.TryParse(domain, out IPAddress address))
                return TsigResolveAsync(new DnsPacketQuestion(address, DnsClass.Internet), key, fudge, cancellationToken);
            else
                return TsigResolveAsync(new DnsPacketQuestion(domain, type, DnsClass.Internet), key, fudge, cancellationToken);
        }

        public Task<DnsPacket> ResolveAsync(DnsPacketQuestion question, CancellationToken cancellationToken = default)
        {
        ...
        public Task<DnsPacket> ResolveAsync(string domain, DnsRecordType type, CancellationToken cancellationToken = default)
        {
            if ((type == DnsRecordType.PTR) && IPAddress.TryParse(domain, out IPAddress address))
                return ResolveAsync(new DnsPacketQuestion(address, DnsClass.Internet), cancellationToken);
            else
                return ResolveAsync(new DnsPacketQuestion(domain, type, DnsClass.Internet), cancellationToken);
        }
        ...
        {
            return ParseResponsePTR(await ResolveAsync(new DnsPacketQuestion(ip, DnsClass.Internet), cancellationToken));
        }

        public async Task<IReadOnlyList<string>> ResolveTXTAsync(string domain, CancellationToken cancellationToken = default)
        {
            return ParseResponseTXT(await ResolveAsync(new DnsPacketQuestion(domain, DnsRecordType.TXT, DnsClass.Internet), cancellationToken));
        }
        ...
        public void AddTrustAnchor(string domain, ushort keyTag, DnssecAlgorithm algorithm, DnssecDigestType digestType, byte[] digest)
        {
            ...
            DnsResourceRecord dsRecord = new DnsResourceRecord(domain, DnsRecordType.DS, DnsClass.Internet, 0, new DnsDSRecordData(keyTag, algorithm, digestType, digest));
        ...
        public async Task ValidateDaneAsync(SslStream sslStream, X509Certificate certificate, X509Chain chain, SslPolicyErrors sslPolicyErrors, CancellationToken cancellationToken = default)
        {
            ...
            IReadOnlyList<DnsTLSARecordData> tlsaRecords = ParseResponseTLSA(await ResolveAsync("_" + networkStream.Socket.RemoteEndPoint.GetPort() + "._tcp." + sslStream.TargetHostName, DnsRecordType.TLSA, cancellationToken));
        ...
        internal static IReadOnlyList<DnsNsAddress> IPv4RootHints
        { get { return IPv4_ROOT_HINTS; } }

        internal static IReadOnlyList<DnsNsAddress> IPv6RootHints
        { get { return IPv6_ROOT_HINTS; } }

        public IReadOnlyList<DnsNsAddress> Servers
        { get { return _servers; } }
        ...
        class ResolverData
        {
            public readonly DnsPacketQuestion Question;
            ...
            public readonly IList<DnsNsAddress> NameServers;
            public readonly int NameServerIndex;
            public readonly int HopCount;
            public readonly DnsPacket LastResponse;
            public readonly Exception LastException;

            public ResolverData(DnsPacketQuestion question, string zoneCut, EDnsHeaderFlags ednsFlags, IReadOnlyList<DnsResourceRecord> lastDSRecords, IList<DnsNsAddress> nameServers, int nameServerIndex, int hopCount, DnsPacket lastResponse, Exception lastException)
            {
                Question = question;
                ZoneCut = zoneCut;
                EDnsFlags = ednsFlags;
        ...
        {
            public readonly IReadOnlyList<DnsResourceRecord> LastDSRecords;
            public readonly IReadOnlyList<DnsNsAddress> NameServers;

            public NsRevalidationTask(IReadOnlyList<DnsResourceRecord> lastDSRecords, IReadOnlyList<DnsNsAddress> nameServers)
            {
                LastDSRecords = lastDSRecords;
                NameServers = nameServers;
            }
