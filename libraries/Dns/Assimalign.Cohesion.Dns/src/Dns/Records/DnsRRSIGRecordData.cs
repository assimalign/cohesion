...
namespace Assimalign.Cohesion.Dns.ResourceRecords
{
    public class DnsRRSIGRecordData : DnsResourceRecordData
    {
        ...
        public DnsRRSIGRecordData(DnsRecordType typeCovered, DnssecAlgorithm algorithm, byte labels, uint originalTtl, uint signatureExpiration, uint signatureInception, ushort keyTag, string signersName, byte[] signature)
        {
            ...
            using (MemoryStream mS = new MemoryStream(512))
            {
                ...
                string name;
                {
                    ...
                    else if (rrsigRecord._labels < labelCount)
                    {
                        ...
                        if (rrsigRecord._typeCovered == DnsRecordType.NSEC)
                        {
        ...
        {
            using (MemoryStream mS = new MemoryStream(2 + 1 + 1 + 4 + 4 + 4 + 2 + DnsPacket.GetSerializeDomainNameLength(_signersName) + _signature.Length))
            {
                WriteTo(mS, true);
        ...
        protected override void ReadRecordData(Stream s)
        {
            ...
            _typeCovered = (DnsRecordType)DnsPacket.ReadUInt16NetworkOrder(s);
            _algorithm = (DnssecAlgorithm)s.ReadByteValue();
            _labels = s.ReadByteValue();
            _originalTtl = DnsPacket.ReadUInt32NetworkOrder(s);
            _signatureExpiration = DnsPacket.ReadUInt32NetworkOrder(s);
            _signatureInception = DnsPacket.ReadUInt32NetworkOrder(s);
            _keyTag = DnsPacket.ReadUInt16NetworkOrder(s);
            _signersName = DnsPacket.DeserializeDomainName(s);
        ...
        internal static async Task<DnsRRSIGRecordData> FromZoneFileEntryAsync(ZoneFile zoneFile)
        {
            ...
            DnsRecordType typeCovered = Enum.Parse<DnsRecordType>(await zoneFile.PopItemAsync(), true);
        ...
        {
            DnsPacket.WriteUInt16NetworkOrder((ushort)_typeCovered, s);
            s.WriteByte((byte)_algorithm);
            s.WriteByte(_labels);
            DnsPacket.WriteUInt32NetworkOrder(_originalTtl, s);
            DnsPacket.WriteUInt32NetworkOrder(_signatureExpiration, s);
            DnsPacket.WriteUInt32NetworkOrder(_signatureInception, s);
            DnsPacket.WriteUInt16NetworkOrder(_keyTag, s);
            DnsPacket.SerializeDomainName(_signersName.ToLowerInvariant(), s);
        ...
        public bool IsSignatureValid(IReadOnlyList<DnsResourceRecord> records, IReadOnlyList<DnsResourceRecord> dnsKeyRecords, ref int maxCryptoFailures, out EDnsExtendedDnsErrorCode extendedDnsErrorCode)
        {
            ...
            foreach (DnsResourceRecord dnsKeyRecord in dnsKeyRecords)
            {
                ...
                if (dnsKey.Flags.HasFlag(DnsDnsKeyFlag.ZoneKey))
                {
                    ...
                    if (dnsKey.Flags.HasFlag(DnsDnsKeyFlag.Revoke) && (_typeCovered != DnsRecordType.DNSKEY))
        ...
        { get { return 2 + 1 + 1 + 4 + 4 + 4 + 2 + DnsPacket.GetSerializeDomainNameLength(_signersName) + _signature.Length; } }
