...
namespace Assimalign.Cohesion.Dns.ResourceRecords
{
    ...
    public class DnsForwarderRecordData : DnsResourceRecordData
    {
        ...
        public DnsForwarderRecordData(DnsTransportProtocol protocol, string forwarder, bool dnssecValidation, DnsForwarderRecordProxyType proxyType, string proxyAddress, ushort proxyPort, string proxyUsername, string proxyPassword, byte priority)
        {
            ...
            _nameServer = DnsNsAddress.Parse(_forwarder);
            ...
            if (bytesRead < _rdLength)
            {
                ...
                switch (_proxyType)
                {
                    ...
                    case DnsForwarderRecordProxyType.Socks5:
                        ...
                        _proxyPort = DnsPacket.ReadUInt16NetworkOrder(s);
        ...
        protected override void WriteRecordData(Stream s, List<DnsDomainOffset> domainEntries, bool canonicalForm)
        {
            ...
            switch (_proxyType)
            {
                ...
                case DnsForwarderRecordProxyType.Socks5:
                    ...
                    DnsPacket.WriteUInt16NetworkOrder(_proxyPort, s);
        ...
        {
            string str = _protocol.ToString() + " " + DnsPacket.EncodeCharacterString(_forwarder) + " " + _dnssecValidation + " " + _proxyType.ToString();

            switch (_proxyType)
            {
                case DnsForwarderRecordProxyType.Http:
                case DnsForwarderRecordProxyType.Socks5:
                    str += " " + DnsPacket.EncodeCharacterString(_proxyAddress) + " " + _proxyPort;

                    if (string.IsNullOrEmpty(_proxyUsername))
                        str += " -";
                    else
                        str += " " + DnsPacket.EncodeCharacterString(_proxyUsername);

                    if (string.IsNullOrEmpty(_proxyPassword))
                        str += " -";
                    else
                        str += " " + DnsPacket.EncodeCharacterString(_proxyPassword);
