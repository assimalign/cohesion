...
namespace Assimalign.Cohesion.Dns.ResourceRecords
{
    ...
    public class DnsSVCBRecordData : DnsResourceRecordData
    {
        ...
        protected override void ReadRecordData(Stream s)
        {
            ...
            {
                _svcPriority = DnsPacket.ReadUInt16NetworkOrder(mS);
                _targetName = DnsPacket.DeserializeDomainName(mS);

                Dictionary<DnsSvcParamKey, DnsSvcParamValue> svcParams = new Dictionary<DnsSvcParamKey, DnsSvcParamValue>();

                while (mS.Position < mS.Length)
                {
                    DnsSvcParamKey svcParamKey = (DnsSvcParamKey)DnsPacket.ReadUInt16NetworkOrder(mS);
        ...
        protected override void WriteRecordData(Stream s, List<DnsDomainOffset> domainEntries, bool canonicalForm)
        {
            if (_rData is null)
            {
                ...
                {
                    DnsPacket.WriteUInt16NetworkOrder(_svcPriority, mS);
                    DnsPacket.SerializeDomainName(canonicalForm ? _targetName.ToLowerInvariant() : _targetName, mS, null); //no compression for domain name as per RFC

                    foreach (KeyValuePair<DnsSvcParamKey, DnsSvcParamValue> svcParam in svcParams)
                    {
                        DnsPacket.WriteUInt16NetworkOrder((ushort)svcParam.Key, mS);
        ...
        public override int UncompressedLength
        {
            get
            {
                ...
                return 2 + DnsPacket.GetSerializeDomainNameLength(_targetName) + scvParamLength;
            }
        }
    ...
    public abstract class DnsSvcParamValue
    {
        ...
        {
            _length = DnsPacket.ReadUInt16NetworkOrder(s);
        ...
        public void WriteTo(Stream s)
        {
            ...
            DnsPacket.WriteUInt16NetworkOrder(length, s);
    ...
    public class DnsSvcMandatoryParamValue : DnsSvcParamValue
    {
        ...
        protected override void ReadSvcParamValue(Stream s)
        {
            ...
            for (int i = 0; i < count; i++)
                keys[i] = (DnsSvcParamKey)DnsPacket.ReadUInt16NetworkOrder(s);
        ...
        protected override void WriteSvcParamValue(Stream s)
        {
            ...
            foreach (DnsSvcParamKey key in keys)
                DnsPacket.WriteUInt16NetworkOrder((ushort)key, s);
        }
    ...
    public class DnsSvcPortParamValue : DnsSvcParamValue
    {
        ...
        protected override void ReadSvcParamValue(Stream s)
        {
            _port = DnsPacket.ReadUInt16NetworkOrder(s);
        }

        protected override void WriteSvcParamValue(Stream s)
        {
            DnsPacket.WriteUInt16NetworkOrder(_port, s);
        }
