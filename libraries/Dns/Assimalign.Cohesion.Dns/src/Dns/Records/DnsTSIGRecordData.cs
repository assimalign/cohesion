...
namespace Assimalign.Cohesion.Dns.ResourceRecords
{
    ...
    public class DnsTSIGRecordData : DnsResourceRecordData
    {
        ...
        {
            _algorithmName = DnsPacket.DeserializeDomainName(s);
            _timeSigned = DnsPacket.ReadUInt48NetworkOrder(s);
            _fudge = DnsPacket.ReadUInt16NetworkOrder(s);

            ushort macSize = DnsPacket.ReadUInt16NetworkOrder(s);
            _mac = s.ReadExactly(macSize);

            _originalID = DnsPacket.ReadUInt16NetworkOrder(s);
            _error = (DnsTsigError)DnsPacket.ReadUInt16NetworkOrder(s);

            ushort otherLen = DnsPacket.ReadUInt16NetworkOrder(s);
            _otherData = s.ReadExactly(otherLen);
        }

        protected override void WriteRecordData(Stream s, List<DnsDomainOffset> domainEntries, bool canonicalForm)
        {
            DnsPacket.SerializeDomainName(canonicalForm ? _algorithmName.ToLowerInvariant() : _algorithmName, s); //MUST NOT be compressed
            DnsPacket.WriteUInt48NetworkOrder(_timeSigned, s);
            DnsPacket.WriteUInt16NetworkOrder(_fudge, s);

            DnsPacket.WriteUInt16NetworkOrder(Convert.ToUInt16(_mac.Length), s);
            s.Write(_mac);

            DnsPacket.WriteUInt16NetworkOrder(_originalID, s);
            DnsPacket.WriteUInt16NetworkOrder((ushort)_error, s);

            DnsPacket.WriteUInt16NetworkOrder(Convert.ToUInt16(_otherData.Length), s);
            s.Write(_otherData);
        }
        ...
        { get { return DnsPacket.GetSerializeDomainNameLength(_algorithmName) + 6 + 2 + 2 + _mac.Length + 2 + 2 + 2 + _otherData.Length; } }
