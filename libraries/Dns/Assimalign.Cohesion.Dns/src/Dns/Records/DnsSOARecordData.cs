...
namespace Assimalign.Cohesion.Dns.ResourceRecords
{
    public class DnsSOARecordData : DnsResourceRecordData
    {
        ...
        {
            _primaryNameServer = DnsPacket.DeserializeDomainName(s);
            _responsiblePerson = DnsPacket.DeserializeDomainName(s, 10, false, true);
            _serial = DnsPacket.ReadUInt32NetworkOrder(s);
            _refresh = DnsPacket.ReadUInt32NetworkOrder(s);
            _retry = DnsPacket.ReadUInt32NetworkOrder(s);
            _expire = DnsPacket.ReadUInt32NetworkOrder(s);
            _minimum = DnsPacket.ReadUInt32NetworkOrder(s);
        }

        protected override void WriteRecordData(Stream s, List<DnsDomainOffset> domainEntries, bool canonicalForm)
        {
            DnsPacket.SerializeDomainName(canonicalForm ? _primaryNameServer.ToLowerInvariant() : _primaryNameServer, s, domainEntries);
            DnsPacket.SerializeDomainName(canonicalForm ? _responsiblePerson.ToLowerInvariant() : _responsiblePerson, s, domainEntries, true);
            DnsPacket.WriteUInt32NetworkOrder(_serial, s);
            DnsPacket.WriteUInt32NetworkOrder(_refresh, s);
            DnsPacket.WriteUInt32NetworkOrder(_retry, s);
            DnsPacket.WriteUInt32NetworkOrder(_expire, s);
            DnsPacket.WriteUInt32NetworkOrder(_minimum, s);
        }
        ...
        { get { return DnsPacket.GetSerializeDomainNameLength(_primaryNameServer) + DnsPacket.GetSerializeDomainNameLength(_responsiblePerson) + 4 + 4 + 4 + 4 + 4; } }
