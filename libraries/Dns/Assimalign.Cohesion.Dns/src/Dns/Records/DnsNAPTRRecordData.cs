...
namespace Assimalign.Cohesion.Dns.ResourceRecords
{
    public class DnsNAPTRRecordData : DnsResourceRecordData
    {
        ...
        private void Serialize()
        {
            ...
            {
                DnsPacket.WriteUInt16NetworkOrder(_order, mS);
                DnsPacket.WriteUInt16NetworkOrder(_preference, mS);
                ...
                DnsPacket.SerializeDomainName(_replacement, mS);
        ...
        protected override void ReadRecordData(Stream s)
        {
            ...
            {
                _order = DnsPacket.ReadUInt16NetworkOrder(mS);
                _preference = DnsPacket.ReadUInt16NetworkOrder(mS);
                ...
                _replacement = DnsPacket.DeserializeDomainName(mS);
            }
        }
        ...
        internal static async Task<DnsNAPTRRecordData> FromZoneFileEntryAsync(ZoneFile zoneFile)
        {
            ...
            string flags = DnsPacket.DecodeCharacterString(await zoneFile.PopItemAsync());
            string services = DnsPacket.DecodeCharacterString(await zoneFile.PopItemAsync());
            string regexp = DnsPacket.DecodeCharacterString(await zoneFile.PopItemAsync());
        ...
        {
            return _order + " " + _preference + " " + DnsPacket.EncodeCharacterString(_flags) + " " + DnsPacket.EncodeCharacterString(_services) + " " + DnsPacket.EncodeCharacterString(_regexp) + " " + DnsResourceRecord.GetRelativeDomainName(_replacement, originDomain);
        }
        ...
        { get { return 2 + 2 + 1 + _flags.Length + 1 + _services.Length + 1 + _regexp.Length + DnsPacket.GetSerializeDomainNameLength(_replacement); } }
