...
namespace Assimalign.Cohesion.Dns.ResourceRecords
{
    public class DnsALIASRecordData : DnsANAMERecordData
    {
        ...
        DnsRecordType _type; //alias for record type

        #endregion

        #region constructor

        public DnsALIASRecordData(DnsRecordType type, string domain)
            : base(domain)
        {
            _type = type;
        }
        ...
        internal new static async Task<DnsANAMERecordData> FromZoneFileEntryAsync(ZoneFile zoneFile)
        {
            ...
            DnsRecordType type = (DnsRecordType)ushort.Parse(await zoneFile.PopItemAsync());
