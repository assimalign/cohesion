...
namespace Assimalign.Cohesion.Dns.ResourceRecords
{
    ...
    public class DnsNSECRecordData : DnsResourceRecordData
    {
        ...
        IReadOnlyList<DnsRecordType> _types;
        ...
        public DnsNSECRecordData(string nextDomainName, IReadOnlyList<DnsRecordType> types)
        {
        ...
        internal static DnssecProofOfNonExistence GetValidatedProofOfNonExistence(IReadOnlyList<DnsResourceRecord> nsecRecords, string domain, DnsRecordType type, bool wildcardAnswerValidation)
        {
            ...
            {
                if (nsecRecord.Type != DnsRecordType.NSEC)
                    continue;
                ...
                if (nsecRecord.Name.Equals(domain, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if ((type != DnsRecordType.DS) && nsec._isAncestorDelegation)
        ...
        private DnssecProofOfNonExistence GetProofOfNonExistenceFromRecordTypes(DnsRecordType checkType)
        {
            if ((checkType == DnsRecordType.DS) && _isInsecureDelegation)
                return DnssecProofOfNonExistence.InsecureDelegation;

            //find if record set exists
            foreach (DnsRecordType type in _types)
            {
                if ((type == checkType) || (type == DnsRecordType.CNAME))
        ...
        internal static IReadOnlyList<DnsRecordType> ReadTypeBitMapsFrom(Stream s, int length)
        {
                ...
                for (int i = 0; i < bitmapLength; i++)
                {
                    ...
                    for (int count = 0, bitMask = 0x80; count < 8; count++, bitMask >>= 1)
                    {
                        if ((currentByte & bitMask) > 0)
                            types.Add((DnsRecordType)(windowBlockNumber | (currentPosition + count)));
                    }
                }
        ...
        internal static void WriteTypeBitMapsTo(IReadOnlyList<DnsRecordType> types, Stream s)
        {
        ...
        private void Serialize()
        {
            ...
            {
                DnsPacket.SerializeDomainName(_nextDomainName, mS); //RFC6840: DNS names in the RDATA section of NSEC resource records are not converted to lowercase
        ...
        protected override void ReadRecordData(Stream s)
        {
            ...
            {
                _nextDomainName = DnsPacket.DeserializeDomainName(mS);
        ...
        internal static async Task<DnsNSECRecordData> FromZoneFileEntryAsync(ZoneFile zoneFile)
        {
            ...
            do
            {
                ...
                types.Add(Enum.Parse<DnsRecordType>(type, true));
            }
            while (true);
        ...
        public IReadOnlyList<DnsRecordType> Types
        { get { return _types; } }
