...
namespace Assimalign.Cohesion.Dns.ResourceRecords
{
    public class DnsANAMERecordData : DnsResourceRecordData
    {
        ...
        {
            DnsPacket.SerializeDomainName(canonicalForm ? _domain.ToLowerInvariant() : _domain, s, null);
        }
        ...
        internal static async Task<DnsANAMERecordData> FromZoneFileEntryAsync(ZoneFile zoneFile)
        {
            Stream rdata = await zoneFile.GetRData();
            if (rdata is not null)
                return new DnsANAMERecordData(rdata);

            return new DnsANAMERecordData(await zoneFile.PopDomainAsync());
        }
