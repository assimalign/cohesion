...
namespace Assimalign.Cohesion.Dns.ResourceRecords
{
    public class DnsApplicationRecordData : DnsResourceRecordData
    {
        ...
        {
            return DnsPacket.EncodeCharacterString(_appName) + " " + DnsPacket.EncodeCharacterString(_classPath) + " " + (_data.Length == 0 ? "-" : Convert.ToBase64String(Encoding.UTF8.GetBytes(_data)));
        }
