...
namespace Assimalign.Cohesion.Dns.ResourceRecords
{
    ...
    public class DnsNSEC3RecordData : DnsResourceRecordData
    {
        ...
        public DnsNSEC3RecordData(DnssecNSEC3HashAlgorithm hashAlgorithm, DnssecNSEC3Flags flags, ushort iterations, byte[] salt, byte[] nextHashedOwnerName, IReadOnlyList<DnsRecordType> types)
        {
        ...
        internal static async Task<DnssecProofOfNonExistence> GetValidatedProofOfNonExistenceAsync(IReadOnlyList<DnsResourceRecord> nsec3Records, string domain, DnsRecordType type, bool wildcardAnswerValidation, string wildcardNextCloserName)
        {
            ...
            while (!foundClosestEncloserProof)
            {
                ...
                {
                    if (nsec3Record.Type != DnsRecordType.NSEC3)
                        continue;
                    ...
                    if (hashedOwnerName.Equals(hashedClosestEncloser, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (closestEncloser.Equals(domain, StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            if ((type != DnsRecordType.DS) && nsec3._isAncestorDelegation)
        ...
        internal static byte[] ComputeHashedOwnerName(string ownerName, DnssecNSEC3HashAlgorithm hashAlgorithm, ushort iterations, byte[] salt)
        {
            switch (hashAlgorithm)
            {
                case DnssecNSEC3HashAlgorithm.SHA1:
                    ...
                    {
                        DnsPacket.SerializeDomainName(ownerName.ToLowerInvariant(), mS);
                        mS.Write(salt);
        ...
        private void Serialize()
        {
            using (MemoryStream mS = new MemoryStream())
            {
                ...
                DnsPacket.WriteUInt16NetworkOrder(_iterations, mS);
        ...
        protected override void ReadRecordData(Stream s)
        {
            ...
            using (MemoryStream mS = new MemoryStream(_rData))
            {
                ...
                _iterations = DnsPacket.ReadUInt16NetworkOrder(mS);
