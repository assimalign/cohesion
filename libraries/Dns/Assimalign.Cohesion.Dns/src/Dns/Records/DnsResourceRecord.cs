...
namespace Assimalign.Cohesion.Dns.ResourceRecords
{
    ...
    public sealed class DnsResourceRecord : IComparable<DnsResourceRecord>
    {
        ...
        public DnsResourceRecord(string name, DnsRecordType type, DnsClass @class, uint ttl, DnsResourceRecordData rData)
        {
        ...
        public DnsResourceRecord(Stream s)
        {
            ...
            _name = DnsPacket.DeserializeDomainName(s);
            _type = (DnsRecordType)DnsPacket.ReadUInt16NetworkOrder(s);
            _class = (DnsClass)DnsPacket.ReadUInt16NetworkOrder(s);
            _ttl = DnsPacket.ReadUInt32NetworkOrder(s);
        ...
        public static DnsResourceRecord ReadCacheRecordFrom(BinaryReader bR, Action<DnsResourceRecord> readTagInfo)
        {
            ...
            switch (version)
            {
                ...
                case 2:
                    ...
                    record._type = (DnsRecordType)bR.ReadUInt16();
        ...
        public static Dictionary<string, Dictionary<DnsRecordType, List<DnsResourceRecord>>> GroupRecords(IReadOnlyCollection<DnsResourceRecord> records, bool deduplicate = false)
        {
            Dictionary<string, Dictionary<DnsRecordType, List<DnsResourceRecord>>> groupedByDomainRecords = new Dictionary<string, Dictionary<DnsRecordType, List<DnsResourceRecord>>>();

            foreach (DnsResourceRecord record in records)
            {
                string recordName = record.Name.ToLowerInvariant();

                if (!groupedByDomainRecords.TryGetValue(recordName, out Dictionary<DnsRecordType, List<DnsResourceRecord>> groupedByTypeRecords))
                {
                    groupedByTypeRecords = new Dictionary<DnsRecordType, List<DnsResourceRecord>>();
        ...
        public static DnsResourceRecordData ReadRecordDataFrom(DnsRecordType type, byte[] rdata)
        {
            using MemoryStream mS = new MemoryStream(2 + rdata.Length);
            {
                DnsPacket.WriteUInt16NetworkOrder((ushort)rdata.Length, mS);
                mS.Write(rdata);
                mS.Position = 0;
        ...
        private static DnsResourceRecordData ReadRecordDataFrom(Stream s, DnsRecordType type)
        {
            switch (type)
            {
                ...
                case DnsRecordType.ANAME:
                    return new DnsANAMERecordData(s);

                case DnsRecordType.FWD:
                    return new DnsForwarderRecordData(s);

                case DnsRecordType.APP:
        ...
        {
            DnsPacket.SerializeDomainName(_name, s, domainEntries);
            DnsPacket.WriteUInt16NetworkOrder((ushort)_type, s);
            DnsPacket.WriteUInt16NetworkOrder((ushort)_class, s);
            DnsPacket.WriteUInt32NetworkOrder(TTL, s);
        ...
        { get { return DnsPacket.GetSerializeDomainNameLength(_name) + 2 + 2 + 4 + 2 + _rData.UncompressedLength; } }
