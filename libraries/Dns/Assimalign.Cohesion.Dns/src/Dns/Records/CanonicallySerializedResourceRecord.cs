...
namespace Assimalign.Cohesion.Dns.ResourceRecords
{
    public class CanonicallySerializedResourceRecord : IComparable<CanonicallySerializedResourceRecord>
    {
        ...
        public static CanonicallySerializedResourceRecord Create(string name, DnsRecordType type, DnsClass @class, uint originalTtl, DnsResourceRecordData rData, MemoryStream buffer)
        {
            byte[] firstPart;
            byte[] rdataPart;
            ...
            buffer.SetLength(0);
            DnsPacket.SerializeDomainName(name.ToLowerInvariant(), buffer);
            DnsPacket.WriteUInt16NetworkOrder((ushort)type, buffer);
            DnsPacket.WriteUInt16NetworkOrder((ushort)@class, buffer);
            DnsPacket.WriteUInt32NetworkOrder(originalTtl, buffer);
            DnsPacket.WriteUInt16NetworkOrder(Convert.ToUInt16(rdataPart.Length), buffer);
