...
namespace Assimalign.Cohesion.Dns.ResourceRecords
{
    public class DnsSRVRecordData : DnsResourceRecordData
    {
        ...
        {
            _priority = DnsPacket.ReadUInt16NetworkOrder(s);
            _weight = DnsPacket.ReadUInt16NetworkOrder(s);
            _port = DnsPacket.ReadUInt16NetworkOrder(s);
            _target = DnsPacket.DeserializeDomainName(s);
        }

        protected override void WriteRecordData(Stream s, List<DnsDomainOffset> domainEntries, bool canonicalForm)
        {
            DnsPacket.WriteUInt16NetworkOrder(_priority, s);
            DnsPacket.WriteUInt16NetworkOrder(_weight, s);
            DnsPacket.WriteUInt16NetworkOrder(_port, s);
            DnsPacket.SerializeDomainName(canonicalForm ? _target.ToLowerInvariant() : _target, s, null); //no compression for domain name as per RFC
        }
        ...
        {
            return _priority + " " + _weight + " " + _port + " " + DnsResourceRecord.GetRelativeDomainName(_target, originDomain);
        }
        ...
        { get { return 2 + 2 + 2 + DnsPacket.GetSerializeDomainNameLength(_target); } }
