...
namespace Assimalign.Cohesion.Dns.ResourceRecords
{
    public class DnsPTRRecordData : DnsResourceRecordData
    {
        ...
        {
            _domain = DnsPacket.DeserializeDomainName(s);
        }

        protected override void WriteRecordData(Stream s, List<DnsDomainOffset> domainEntries, bool canonicalForm)
        {
            DnsPacket.SerializeDomainName(canonicalForm ? _domain.ToLowerInvariant() : _domain, s, domainEntries);
        }
        ...
        internal static async Task<DnsPTRRecordData> FromZoneFileEntryAsync(ZoneFile zoneFile)
        {
            Stream rdata = await zoneFile.GetRData();
            if (rdata is not null)
                return new DnsPTRRecordData(rdata);

            return new DnsPTRRecordData(await zoneFile.PopDomainAsync());
        }
        ...
        { get { return DnsPacket.GetSerializeDomainNameLength(_domain); } }
