...
internal sealed class RetryResilienceStrategy : ResilienceStrategy
{
    ...
    public override ValueTask<TResult> ExecuteAsync<TResult, TState>(ResilienceStrategyCallback<TResult, TState> callback, IResilienceContext context, TState state)
    {
        throw new NotImplementedException();
    }

    protected override async ValueTask ExecuteAsync<TState>(Func<ResilienceContextO, TState, ValueTask<OutcomeO<T>>> callback, ResilienceContextO context, TState state)
    {
        double retryState = 0;
...
#pragma warning restore S3236 // Remove this argument from the method call; it hides the caller information.
            ...
            {
                return OutcomeO.FromException<T>(e);
            }
