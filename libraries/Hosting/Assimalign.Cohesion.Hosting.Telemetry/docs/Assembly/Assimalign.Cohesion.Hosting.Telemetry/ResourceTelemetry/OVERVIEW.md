...
Configuration errors raise InvalidOperationException; reserved gRPC raises NotSupportedException. Null context raises ArgumentNullException. Off returns false/null without a builder mutation. Register the out lifetime for host shutdown; the no-out overload transfers disposal responsibility to the caller.
