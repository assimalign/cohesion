...
/// <summary>
/// A context assigned to a single execution of <see cref="ResiliencePipelineO"/>. It is created manually or automatically
/// when the user calls the various extensions on top of <see cref="ResiliencePipelineO"/>. After every execution the context should be discarded and returned to the pool.
/// </summary>
/// <remarks>
/// Do not re-use an instance of <see cref="ResilienceContextO"/> across more than one execution. The <see cref="ResilienceContextO"/> is retrieved from the pool
/// by calling the <see cref="ResilienceContextPool.Rent(CancellationToken)"/> method. After you are done with it you should return it to the pool
/// by calling the <see cref="ResilienceContextPool.Return(ResilienceContextO)"/> method.
/// </remarks>
public sealed class ResilienceContextO
{
    internal ResilienceContextO()
    {
    }
...
//#pragma warning restore S3236 // Remove this argument from the method call; it hides the caller information.
    ...
    internal ResilienceContextO Initialize<TResult>(bool isSynchronous)
    {
