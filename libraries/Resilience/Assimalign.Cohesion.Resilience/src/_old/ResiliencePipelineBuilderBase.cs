...
/// The resulting instance of <see cref="ResiliencePipelineO"/> executes the strategies in the same order they were added to the builder.
...
#pragma warning restore S1694 // An abstract class should have both abstract and concrete methods
{
    ...
    /// <summary>
    /// Gets or sets the <see cref="Telemetry.ResilienceTelemetryListener"/> that is used by Polly to report resilience events.
    /// </summary>
    /// <remarks>
    ...
    public ResilienceTelemetryListener? TelemetryListener { get; set; }
    ...
    {
        Validator(new(this, $"The '{nameof(ResiliencePipelineBuilderO)}' configuration is invalid."));
        ...
        var source = new ResilienceEventSource(Name, InstanceName, null);
    ...
    {
        var source = new ResilienceEventSource(Name, InstanceName, entry.Options.Name);
