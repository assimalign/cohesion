...
internal sealed class CompositeComponent : PipelineComponent
{
    ...
    private ValueTask<OutcomeO<TResult>> ExecuteCoreWithoutTelemetry<TResult, TState>(
        Func<ResilienceContextO, TState, ValueTask<OutcomeO<TResult>>> callback,
        ResilienceContextO context,
        TState state)
    {
        ...
        {
            return FirstComponent.ExecuteAsync(callback, context, state);
        }
    }

    private async ValueTask<OutcomeO<TResult>> ExecuteCoreWithTelemetry<TResult, TState>(
        Func<ResilienceContextO, TState, ValueTask<OutcomeO<TResult>>> callback,
        ResilienceContextO context,
        TState state)
    {
