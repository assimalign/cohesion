...
/// </remarks>
public readonly struct OutcomeO<TResult>
{
    // TODO: Making Public, review exposure later.
    public OutcomeO(Exception exception)
        : this() => ExceptionDispatchInfo = ExceptionDispatchInfo.Capture(ArgumentNullException.ThrowIfNull<Exception>(exception));

    public OutcomeO(TResult? result)
        : this() => Result = result;

    public OutcomeO(ExceptionDispatchInfo exceptionDispatchInfo)
        : this() => ExceptionDispatchInfo = ArgumentNullException.ThrowIfNull<ExceptionDispatchInfo>(exceptionDispatchInfo);
