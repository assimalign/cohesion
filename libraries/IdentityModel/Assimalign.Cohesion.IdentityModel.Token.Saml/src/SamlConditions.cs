...
/// <summary>
/// Represents a SAML 2.0 <c>Conditions</c> element (SAML Core §2.5.1) on a materialized SAML
/// token: the constraints under which the assertion may be relied upon. This is the
/// authoritative audience surface for a SAML token — the base
/// <see cref="IIdentityToken.Audiences" /> is only a lossy union projection, because it cannot
/// express the AND-across / OR-within audience-restriction rule.
/// </summary>
/// <remarks>
/// Audience restrictions follow the SAML rule: an assertion satisfies the audience constraint
/// when, for <em>every</em> <c>AudienceRestriction</c>, the relying party matches at least one
/// of its audiences (AND across restrictions, OR within one). Each entry in
/// <see cref="AudienceRestrictions" /> is one restriction's set of audiences; an empty set is a
/// wire malformation treated as unsatisfiable, never vacuously true.
/// </remarks>
public sealed class SamlConditions
{
    ...
    /// <summary>
    /// Determines whether the provided relying party satisfies every audience restriction. An
    /// empty restriction set (a malformed restriction with no audiences) is never satisfied.
    /// When there are no restrictions the assertion is unrestricted.
    /// </summary>
