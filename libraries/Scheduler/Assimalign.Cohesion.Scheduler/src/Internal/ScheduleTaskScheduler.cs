...
{
    internal class ScheduleTaskScheduler : TaskScheduler, IDisposable
    {
        ...
        public ScheduleTaskScheduler(int threadCount)
        {
        ...
        protected override void QueueTask(Task task)
        {
            ...
            {
                throw new ObjectDisposedException(nameof(ScheduleTaskScheduler),
                    "Cannot queue tasks after the scheduler is disposed.");
            }

            if (task.CreationOptions == TaskCreationOptions.HideScheduler)
            {
                _taskDict.TryAdd(task.Id, new TaskWithPriority(task, SchedulePriority.Normal));
            }
            else
            {
        ...
        public void SetQueuedTaskPriority(int taskId, SchedulePriority tickerTaskPriority)
        {
        ...
        {
            public readonly SchedulePriority Priority;
            public readonly Task Task;

            public TaskWithPriority(Task task, SchedulePriority priority)
            {
                Task = task;
                Priority = priority;
            }
